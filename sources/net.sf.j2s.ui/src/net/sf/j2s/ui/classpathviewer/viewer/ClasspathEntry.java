...
//	}
	...
	public boolean hasChildren() {
		...
		if (rpe instanceof CompositeResources) {
			//CompositeResources comp = (CompositeResources) rpe;
			return true;
		} else if (rpe instanceof ContactedClasses) {
			return true;
		}
...
//		 ((IRuntimeClasspathEntry2)rpe).isComposite();
	...
	public IClasspathEntry[] getChildren(ILaunchConfiguration configuration) {
		...
		if (rpe instanceof CompositeResources) {
			...
			ClasspathEntry[] entries = new ClasspathEntry[children.length];
			for (int i = 0; i < entries.length; i++) {
				entries[i] = new ClasspathEntry(children[i], this);
			}
			return entries;
		} else if (rpe instanceof ContactedClasses) {
			ContactedClasses cc = (ContactedClasses) rpe;
			Resource[] children = cc.getChildren();
			ClasspathEntry[] entries = new ClasspathEntry[children.length];
			for (int i = 0; i < entries.length; i++) {
				entries[i] = new ClasspathEntry(children[i], this);
			}
