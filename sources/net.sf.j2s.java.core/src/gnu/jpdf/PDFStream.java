   ...
   */
    ...
    public void writeStream(OutputStream os) throws IOException {
        if(deflate) {
            ...
            write(os, "/Filter /FlateDecode\n");
            write(os, "/Length ");
            write(os, Integer.toString(b.size()+1));
            write(os, "\n>>\nstream\n");
            b.writeTo(os);
            write(os, "\n");
        } else {
        	...
        	write(os, "/Length ");
        	write(os, Integer.toString(buf.size()));
        	write(os, "\n>>\nstream\n");
            buf.writeTo(os);
        }
    ...
    static void write(OutputStream os, String s) throws IOException {
    	// Current SwingJS does not implement os.write(byte[]) for speed
    	byte[] b = s.getBytes();
