...
public class PDFAnnot extends PDFObject implements Serializable
{
  ...
  @Override
    ...
    // handle the border
    if(border==null) {
      PDFStream.write(os, "/Border [0 0 0]\n");
      //if(pdf.defaultOutlineBorder==null)
      //pdf.add(pdf.defaultOutlineBorder = new border(SOLID,0.0));
      //PDFStream.write(os, pdf.defaultOutlineBorder.toString());
    } else {
      PDFStream.write(os, "/BS ");
      PDFStream.write(os, border.toString());
      PDFStream.write(os, "\n");
    }
    ...
    if(subtype.equals("/Text")) {
      ...
      PDFStream.write(os, PDFStringHelper.makePDFString(s));
      PDFStream.write(os, "\n");
    } else if(subtype.equals("/Link")) {
      PDFStream.write(os, "/Dest [");
      PDFStream.write(os, dest.toString());
      if(fl==FULL_PAGE)
        PDFStream.write(os, " /Fit]");
      else {
        PDFStream.write(os, " /FitR ");
        PDFStream.write(os, Integer.toString(fl));
        PDFStream.write(os, " ");
        PDFStream.write(os, Integer.toString(fb));
        PDFStream.write(os, " ");
        PDFStream.write(os, Integer.toString(fr));
        PDFStream.write(os, " ");
        PDFStream.write(os, Integer.toString(ft));
        PDFStream.write(os, "]");
      }
      PDFStream.write(os, "\n");
    }
