...
//    protected Vector xobjects;
    ...
    public void write(OutputStream os) throws IOException
    {
        ...
        PDFStream.write(os, "/Parent ");
        PDFStream.write(os, pdfPageList.toString());
        PDFStream.write(os, "\n");

        // the /MediaBox for the page size
        PDFStream.write(os, "/MediaBox [");
        PDFStream.write(os, Integer.toString(0));
        PDFStream.write(os, " ");
        PDFStream.write(os, Integer.toString(0));
        PDFStream.write(os, " ");
        PDFStream.write(os, Integer.toString((int)pageFormat.getWidth()));
        PDFStream.write(os, " ");
        PDFStream.write(os, Integer.toString((int)pageFormat.getHeight()));
        PDFStream.write(os, "]\n");

        // Rotation (if not zero)
//        if(rotate!=0) {
//            PDFStream.write(os, "/Rotate ");
//            PDFStream.write(os, Integer.toString(rotate));
//            PDFStream.write(os, "\n");
//        }

        // Now the resources
        PDFStream.write(os, "/Resources << ");
        // fonts
        if(fonts.size()>0) {
          //PDFStream.write(os, "/Font << ");
            PDFStream.write(os, "\n/Font << ");
            for(PDFFont font : fonts) {
                PDFStream.write(os, font.getName());
                PDFStream.write(os, " ");
                PDFStream.write(os, font.toString());
                PDFStream.write(os, " ");
            }
            PDFStream.write(os, ">> ");
        }
	...
	if(imageResources.size() > 0) {
	  ...
	  PDFStream.write(os, " >> ");
        ...
        // The thumbnail
        if(thumbnail!=null) {
            PDFStream.write(os, "/Thumb ");
            PDFStream.write(os, thumbnail.toString());
            PDFStream.write(os, "\n");
        }
        ...
        if(contents.size()>0) {
            if(contents.size()==1) {
                ...
                PDFStream.write(os, "/Contents ");
                PDFStream.write(os, ob.toString());
                PDFStream.write(os, "\n");
            } else {
                PDFStream.write(os, "/Contents [");
                PDFStream.write(os, PDFObject.toArray(contents));
                PDFStream.write(os, "\n");
            }
        }
        ...
        if(annotations.size()>0) {
            PDFStream.write(os, "/Annots ");
            PDFStream.write(os, PDFObject.toArray(annotations));
            PDFStream.write(os, "\n");
        }
