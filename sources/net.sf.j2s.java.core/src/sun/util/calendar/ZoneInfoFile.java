...
public final class ZoneInfoFile {
    ...
    private static void checkHaveData() {
    	if (regions != null)
    		return;
		DataInputStream dis = new DataInputStream(ZoneInfoFile.class.getResourceAsStream("tzdb.dat"));
        try {
			load(dis);
    ...
    public static ZoneInfo getZoneInfo(String zoneId) {
        ...
        if (zoneId.startsWith("GMT"))
        	return null;
        checkHaveData();
        ZoneInfo zi = getZoneInfo0(zoneId.replace(' ', '_'));
        if (zi != null) {
