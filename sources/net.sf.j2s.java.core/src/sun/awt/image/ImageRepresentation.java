...
//    }
	...
	/**
	 * Create an ImageRepresentation for the given Image. The width and height are
	 * unknown at this point. The color model is a hint as to the color model to use
	 * when creating the buffered image. If null, the src color model will be used.
	 */
	...
	/**
	 * Returns the BufferedImage that will be used as the representation of the
	 * pixel data. Subclasses can override this method to return platform specific
	 * subclasses of BufferedImage that may or may not be accelerated.
	 *
	 * It is subclass' responsibility to propagate acceleration priority to the
	 * newly created image.
	 */
...
//				JSUtil.notImplemented("setDiffICM is a native method");
//				IndexColorModel icm = (IndexColorModel) model;///
//				ByteComponentRaster bct = (ByteComponentRaster) biRaster;
	...
	public void setPixels(int x, int y, int w, int h, ColorModel model, int pix[], int off, int scansize) {
		...
		synchronized (this) {
			...
			if ((model == cmodel) && (biRaster instanceof IntegerComponentRaster)) {
				...
				if (model.getTransparency() != Transparency.OPAQUE && cmodel.getTransparency() == Transparency.OPAQUE) {
					convertToRGB();
				}
