...
/**
 * This class defines a Raster with pixels consisting of one or more 32-bit data
 * elements stored in close proximity to each other in a integer array. The bit
 * precision per data element is that of the data type (that is, the bit
 * precision for this raster is 32). There is only one pixel stride and one
 * scanline stride for all bands. For a given pixel, all samples fit in N data
 * elements and these N data elements hold samples for only one pixel. This type
 * of Raster can be used with a PackedColorModel.
 * <p>
 * For example, if there is only one data element per pixel, a
 * SinglePixelPackedSampleModel can be used to represent multiple bands with a
 * PackedColorModel (including a DirectColorModel) for color interpretation.
 * 
 */
public class IntegerInterleavedRaster extends IntegerComponentRaster {
	...
	/**
	 * Constructs a IntegerInterleavedRaster with the given SampleModel. The
	 * Raster's upper left corner is origin and it is the same size as the
	 * SampleModel. A DataBuffer large enough to describe the Raster is
	 * automatically created. SampleModel must be of type
	...
	public IntegerInterleavedRaster(SampleModel sampleModel, Point origin) {
		setIntInterRaster(
				...
				new Rectangle(origin.x, origin.y, sampleModel.getWidth(), sampleModel
						.getHeight()), origin, null);
	}

	/**
	 * Constructs a IntegerInterleavedRaster with the given SampleModel and
	 * DataBuffer. The Raster's upper left corner is origin and it is the same
	 * sizes the SampleModel. The DataBuffer is not initialized and must be a
	 * DataBufferInt compatible with SampleModel. SampleModel must be of type
	 * SinglePixelPackedSampleModel.
	 * 
	 * @param sampleModel
	 *          The SampleModel that specifies the layout.
	...
	public IntegerInterleavedRaster(SampleModel sampleModel,
		...
		setParams(sampleModel, dataBuffer, origin);
	}

	public IntegerInterleavedRaster() {
		// for reflection
	}
	...
	public void setParams(SampleModel sampleModel, DataBuffer dataBuffer,
		...
		setIntInterRaster(sampleModel, dataBuffer, new Rectangle(origin.x,
				origin.y, sampleModel.getWidth(), sampleModel.getHeight()), origin,
				null);
	}

	/**
	 * Constructs a IntegerInterleavedRaster with the given SampleModel,
	 * DataBuffer, and parent. DataBuffer must be a DataBufferInt and SampleModel
	 * must be of type SinglePixelPackedSampleModel. When translated into the base
	 * Raster's coordinate system, aRegion must be contained by the base Raster.
	 * Origin is the coodinate in the new Raster's coordinate system of the origin
	 * of the base Raster. (The base Raster is the Raster's ancestor which has no
	 * parent.)
	 * 
	...
	public IntegerInterleavedRaster(SampleModel sampleModel,
		...
		setIntInterRaster(sampleModel, dataBuffer, aRegion, origin, parent);
	}

	private void setIntInterRaster(SampleModel sampleModel,
			DataBuffer dataBuffer, Rectangle aRegion, Point origin,
			IntegerInterleavedRaster parent) {
	...
	/**
	 * Returns data offset for the specified band. The data offset is the index
	 ...
	 * @param outData
	 *          An object reference to an array of type defined by
	 *          getTransferType() and length w*h*getNumDataElements(). If null an
	 *          array of appropriate type and size will be allocated.
	 * @return An object reference to an array of type defined by
	 *         getTransferType() with the request pixel data.
	 */
	...
	/**
	 * Stores the Raster data at the specified location. The transferType of the
	 * inputRaster must match this raster. An ArrayIndexOutOfBoundsException will
	 * be thrown at runtime if the pixel coordinates are out of bounds.
	 * 
	 * @param x
	 *          The X coordinate of the pixel location.
	 * @param y
	 ...
	 *          Raster of data to place at x,y location.
	 */
	private void setDataElements(int dstX, int dstY, int width, int height,
			Raster inRaster) {
		...
		for (int startY = 0; startY < height; startY++) {
			...
			odata = inRaster.getDataElements(srcOffX, srcOffY + startY, width, 1,
					odata);
	 ...
	 * That is, a data element for the nth band at location (x2, y2) would be
	 * found at:
	 * 
	 * <pre>
	 ...
	 * bands of the parent Raster may be specified. If this is null, then all the
	 * bands are present in the subRaster. A translation to the subRaster may also
	 * be specified. Note that the subraster will reference the same DataBuffer as
	 * the parent raster, but using different offsets.
	 * 
	 * @param x
	 *          X offset.
	 * @param y
	...
	public WritableRaster createWritableChild(int x, int y, int width,
		...
		return new IntegerInterleavedRaster(sm, dataBuffer, new Rectangle(x0, y0,
	 ...
	 * this raster to the upper-left corner of the subraster. A subset of the
	 * bands of the parent raster may be specified. If this is null, then all the
	 * bands are present in the subRaster. Note that the subraster will reference
	 * the same DataBuffer as the parent raster, but using different offsets.
	 * 
	 * @param x
	 *          X offset.
	 * @param y
	...
	/**
	 * Creates a raster with the same data layout and the same width and height,
