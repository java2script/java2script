...
/**
 * This class defines a Raster with pixels consisting of one or more 8-bit data
 * elements stored in close proximity to each other in a single byte array.
 * <p>
 * The bit precision per data element is that of the data type (that is, the bit
 * precision for this Raster is 8). There is only one pixel stride and one
 * scanline stride for all bands. This type of Raster can be used with a
 * ComponentColorModel if there are multiple bands, or an IndexColorModel if
...
public class ByteInterleavedRaster extends ByteComponentRaster {
	...
	/**
	 * Constructs a ByteInterleavedRaster with the given SampleModel. The Raster's
	 * upper left corner is origin and it is the same size as the SampleModel. A
	 * DataBuffer large enough to describe the Raster is automatically created.
	 * SampleModel must be of type SinglePixelPackedSampleModel or
	...
	/**
	 * Constructs a ByteInterleavedRaster with the given SampleModel and DataBuffer.
	 * The Raster's upper left corner is origin and it is the same size as the
	 * SampleModel. The DataBuffer is not initialized and must be a DataBufferByte
	 * compatible with SampleModel. SampleModel must be of type
	 ...
	 * PixelInterleavedSampleModel. In order to do so, it must use only bank 0 of
	 * its DataBuffer, and the data offsets must span a range of less than
	 * pixelStride.
	 *
	 * <p>
	...
	/**
	 * Constructs a ByteInterleavedRaster with the given SampleModel, DataBuffer,
	 * and parent. DataBuffer must be a DataBufferByte and SampleModel must be of
	 * type SinglePixelPackedSampleModel or InterleavedSampleModel. When translated
	 * into the base Raster's coordinate system, aRegion must be contained by the
	 * base Raster. Origin is the coordinate in the new Raster's coordinate system
	 * of the origin of the base Raster. (The base Raster is the Raster's ancestor
	 * which has no parent.)
	 *
	...
	/**
	 * Returns the data offset for the specified band. The data offset is the index
	 ...
	 * given sample and the sample in the same column of the next row in the same
	 * band.
	 */
	@Override
	...
	/**
	 * Returns pixel stride -- the number of data array elements between two samples
	 ...
	 * coordinate is out of bounds. A ClassCastException will be thrown if the input
	 * object is non null and references anything other than an array of
	 * transferType.
	 * 
	 ...
	 * @return An object reference to an array of type defined by getTransferType()
	 *         with the request pixel data.
	 */
	@Override
	 ...
	 * System.arraycopy(bandData, ((y2 - y) * w + (x2 - x)) * numDataElements, pixel, 0, numDataElements);
	 * </pre>
	 * 
	 ...
	 * for the specified band. An ArrayIndexOutOfBounds exception will be thrown at
	 * runtime if the pixel coordinates are out of bounds.
	 * 
	 * <pre>
	 ...
	 * @param outData If non-null, data elements for all bands at the specified
	...
	/**
	 * Stores the Raster data at the specified location. An ArrayIndexOutOfBounds
	 * exception will be thrown at runtime if the pixel coordinates are out of
	 * bounds.
	 * 
	 * @param x        The X coordinate of the pixel location.
	 * @param y        The Y coordinate of the pixel location.
	 ...
	 * 
	 * @param dstX     The absolute X coordinate of the destination pixel that will
	 *                 receive a copy of the upper-left pixel of the inRaster
	 * @param dstY     The absolute Y coordinate of the destination pixel that will
	 *                 receive a copy of the upper-left pixel of the inRaster
	 * @param srcX     The absolute X coordinate of the upper-left source pixel that
	 *                 will be copied into this Raster
	 * @param srcY     The absolute Y coordinate of the upper-left source pixel that
	 *                 will be copied into this Raster
	 ...
	 * coordinates are out of bounds. A ClassCastException will be thrown if the
	 * input object is non null and references anything other than an array of
	 * transferType. The data elements in the data array are assumed to be packed.
	 * That is, a data element for the nth band at location (x2, y2) would be found
	 * at:
	 * 
	 * <pre>
	 ...
	 * for the specified band. An ArrayIndexOutOfBounds exception will be thrown at
	 * runtime if the pixel coordinates are out of bounds. The data elements in the
	 * data array are assumed to be packed. That is, a data element at location (x2,
	 * y2) would be found at:
	 * 
	 * <pre>
	 ...
	 * Stores a byte array of data elements into the specified rectangular region.
	 * An ArrayIndexOutOfBounds exception will be thrown at runtime if the pixel
	 * coordinates are out of bounds. The data elements in the data array are
	 * assumed to be packed. That is, a data element for the nth band at location
	 * (x2, y2) would be found at:
	 * 
	 * <pre>
	 * inData[((y2 - y) * w + (x2 - x)) * numDataElements + n]
	 * </pre>
	 ...
	 * this raster to the upper-left corner of the subraster. A subset of the bands
	 * of the parent Raster may be specified. If this is null, then all the bands
	 * are present in the subRaster. A translation to the subRaster may also be
	 * specified. Note that the subraster will reference the same DataBuffer as the
	 * parent raster, but using different offsets.
	 * 
	 * @param x        X offset.
	 * @param y        Y offset.
	 ...
	 * @exception RasterFormatException if the specified bounding box is outside of
	 *                                  the parent raster.
	 */
	@Override
	 ...
	 * coordinates specify the horizontal and vertical offsets from the upper-left
	 * corner of this Raster to the upper-left corner of the subRaster. A subset of
	 * the bands of the parent Raster may be specified. If this is null, then all
	 * the bands are present in the subRaster. A translation to the subRaster may
	 * also be specified. Note that the subRaster will reference the same DataBuffer
	 ...
	 * and with new zeroed data arrays.
	 */
	@Override
	...
	/**
	 * Creates a Raster with the same layout and the same width and height, and with
	 * new zeroed data arrays. If the Raster is a subRaster, this will call
	...
	public String toString() {
		return new String("ByteInterleavedRaster: width = " + width + " height = " + height + " #numDataElements "
				+ numDataElements
