...
import static sun.font.EAttribute.EBASELINE_TRANSFORM;
...
//    }
    ...
    private boolean i_validate(EAttribute a) {
        ...
        case EJUSTIFICATION: justification = Math.max(0, Math.min (justification, 1));
            return true;
    ...
    public static float getJustification(Map<?, ?> map) {
        if (map != null) {
            ...
            if (obj != null && obj instanceof Number) {
                return Math.max(0, Math.min(1, ((Number)obj).floatValue()));
            }
        }
