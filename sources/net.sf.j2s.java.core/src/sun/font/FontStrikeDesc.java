...
import static sun.awt.SunHints.INTVAL_FRACTIONALMETRICS_OFF;
import static sun.awt.SunHints.INTVAL_FRACTIONALMETRICS_ON;
import static sun.awt.SunHints.INTVAL_TEXT_ANTIALIAS_LCD_HBGR;
import static sun.awt.SunHints.INTVAL_TEXT_ANTIALIAS_LCD_HRGB;
import static sun.awt.SunHints.INTVAL_TEXT_ANTIALIAS_LCD_VBGR;
import static sun.awt.SunHints.INTVAL_TEXT_ANTIALIAS_LCD_VRGB;
import static sun.awt.SunHints.INTVAL_TEXT_ANTIALIAS_OFF;
import static sun.awt.SunHints.INTVAL_TEXT_ANTIALIAS_ON;
...
public class FontStrikeDesc {
	...
	/*
	 * Values to use as a mask that is used for faster comparison of two strikes
	 * using just an int equality test. The ones we don't use are listed here but
	 * commented out. ie style is already built and hint "OFF" values are zero. Note
	 * that this is used as a strike key and the same strike is used for HRGB and
	 * HBGR, so only the orientation needed (H or V) is needed to construct and
	 * distinguish a FontStrikeDesc. The rgb ordering needed for rendering is stored
...
//     static final int FRAC_METRICS_OFF  = 0x0000;
	...
	/*
	 * devTx is to get an inverse transform to get user space values for metrics.
	 * Its not used otherwise, as the glyphTx is the important one. But it does mean
	 * that a strike representing a 6pt font and identity graphics transform is not
	 * equal to one for a 12 pt font and 2x scaled graphics transform. Its likely to
	 * be very rare that this causes duplication.
	 */
	...
	public boolean equals(Object obj) {
		try {
			...
			/*
			 * class cast or NP exceptions should not happen often, if ever, and I am hoping
			 * that this is faster than an instanceof check.
			 */
			return false;
		}
	}
	...
	/*
	 * This maps a public text AA hint value into one of the subset of values used
	 * to index strikes. For the purpose of the strike cache there are only 4 values
	 * : OFF, ON, LCD_HRGB, LCD_VRGB. Font and ptSize are needed to resolve the
	 * 'gasp' table. The ptSize must therefore include device and font transforms.
	 */
...
//        }
	 ...
	 * This maps a public text AA hint value into one of the subset of values used
	 * to index strikes. For the purpose of the strike cache there are only 4 values
	 * : OFF, ON, LCD_HRGB, LCD_VRGB. Font and FontRenderContext are needed to
	 * resolve the 'gasp' table. This is similar to the method above, but used by
	 * callers which have not already calculated the glyph device point size.
	 */
