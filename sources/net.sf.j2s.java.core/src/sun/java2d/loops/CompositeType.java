...
/**
 * A CompositeType object provides a chained description of a type of algorithm
 * for color compositing. The object will provide a single String constant
 * descriptor which is one way of describing a particular compositing algorithm
 * as well as a pointer to another CompositeType which describes a more general
 * algorithm for achieving the same result.
 * <p>
 * A description of a more specific algorithm is considered a "subtype" and a
 * description of a more general algorithm is considered a "supertype". Thus,
 * the deriveSubType method provides a way to create a new CompositeType that is
 * related to but more specific than an existing CompositeType and the
 * getSuperType method provides a way to ask a given CompositeType for a more
 * general algorithm to achieve the same result.
 * <p>
 ...
 * the Any node provided here using the deriveSubType() method. The presence of
 * this common Any node on every chain ensures that all chains end with the
 * DESC_ANY descriptor so that a suitable General GraphicsPrimitive object can
 * be obtained for the indicated algorithm if all of the more specific searches
 * fail.
 */
public final class CompositeType {
	/*
	 * CONSTANTS USED BY ALL PRIMITIVES TO DESCRIBE THE COMPOSITING ALGORITHMS
	 * THEY CAN PERFORM
	 */

	/**
	 * algorithm is a general algorithm that uses a CompositeContext to do the
	 * rendering.
	 */
	...
	/**
	 * constants used to describe the two common cases of AlphaComposite
	 * algorithms that are simpler if there is not extraAlpha.
	 */
	...
	/**
	 * constant used to describe an algorithm that implements all 8 of the
	 * Porter-Duff rules in one Primitive.
	 */
	...
	public static final CompositeType AnyAlpha = General
			.deriveSubType(DESC_ANY_ALPHA);
	...
	public static final CompositeType SrcOver = AnyAlpha
			.deriveSubType(DESC_SRC_OVER);
	public static final CompositeType DstOver = AnyAlpha
			.deriveSubType(DESC_DST_OVER);
	...
	public static final CompositeType SrcOut = AnyAlpha
			.deriveSubType(DESC_SRC_OUT);
	public static final CompositeType DstOut = AnyAlpha
			.deriveSubType(DESC_DST_OUT);
	public static final CompositeType SrcAtop = AnyAlpha
			.deriveSubType(DESC_SRC_ATOP);
	public static final CompositeType DstAtop = AnyAlpha
			.deriveSubType(DESC_DST_ATOP);
	public static final CompositeType AlphaXor = AnyAlpha
			.deriveSubType(DESC_ALPHA_XOR);

	public static final CompositeType SrcNoEa = Src.deriveSubType(DESC_SRC_NO_EA);
	public static final CompositeType SrcOverNoEa = SrcOver
			.deriveSubType(DESC_SRC_OVER_NO_EA);
	...
	/**
	 * Return a new CompositeType object which uses this object as its more
	 * general "supertype" descriptor. If no operation can be found that
	 * implements the algorithm described more exactly by desc, then this object
	 * will define the more general compositing algorithm that can be used
	 * instead.
	 */
