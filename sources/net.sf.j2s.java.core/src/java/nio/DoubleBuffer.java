...
#if[byte]
 *
 * content, or by {@link #wrap(double[]) <i>wrapping</i>} an
 * existing double array {#if[char]?or&#32;string} into a buffer.
 *
#else[byte]
 *
 ...
 * <i>get</i> and <i>put</i> methods for each type.  For 32-bit doubleing-point
 * values, for example, this class defines:
 *
 * <blockquote><pre>
 * double  {@link #getDouble()}
 * double  {@link #getDouble(int) getDouble(int index)}
 *  void  {@link #putDouble(double) putDouble(double f)}
 *  void  {@link #putDouble(int,double) putDouble(int index, double f)}</pre></blockquote>
 *
 ...
 * #asDoubleBuffer() asDoubleBuffer} method, for example, creates an instance of
 * the {@link DoubleBuffer} class that is backed by the byte buffer upon which
...
public abstract class DoubleBuffer
     ...
     */
    public abstract DoubleBuffer put(double $x$);
