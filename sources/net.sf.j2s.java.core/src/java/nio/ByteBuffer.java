 ...
 *
 * <p>
 * This class defines {#if[byte]?six:four} categories of operations upon byte
 * buffers:
 *
 * <ul>
 *
 ...
 * contiguous sequences of bytes from $a$ byte
 * array{#if[char]?,&#32;a&#32;string,} or some other byte buffer into this
 * buffer;{#if[!byte]?&#32;and}
 * </p>
 * </li>
 *
 ...
 * {#if[char]?or&#32;string} into a buffer.
 *
 * #else[byte]
 *
 * content, by {@link #wrap(byte[]) <i>wrapping</i>} an existing byte array
 * {#if[char]?or&#32;string} into a buffer, or by creating a
 ...
 *
 * <p>
 * Like a byte buffer, $a$ byte buffer is either
 * <a href="ByteBuffer.html#direct"><i>direct</i> or <i>non-direct</i></a>. A
 * byte buffer created via the <tt>wrap</tt> methods of this class will be
 * non-direct. $A$ byte buffer created as a view of a byte buffer will be direct
 * if, and only if, the byte buffer itself is direct. Whether or not $a$ byte
 ...
 *
 * #if[byte]
 * <h2>Invocation chaining</h2> #end[byte]
 *
 * <p>
...
public abstract class ByteBuffer extends Buffer implements Comparable<ByteBuffer> {
	 ...
	 * Whether or not it has a {@link #hasArray backing array} is unspecified.
	 *
	 ...
	 * <tt>length</tt>&nbsp;<tt>&gt;</tt>&nbsp;<tt>remaining()</tt>, then no bytes
	 ...
	 * then no bytes are transferred and a {@link BufferOverflowException} is
	 * thrown.
	 *
	 * <p>
	 * Otherwise, this method copies <tt>length</tt> bytes from the given array into
	 ...
	 * byte at index <tt>limit()</tt>&nbsp;-&nbsp;1 is copied to index
	 ...
	 * incomplete. The following loop, for example, copies bytes from one channel to
	 * another via the buffer <tt>buf</tt>:
	 *
	 * <blockquote>
	 * 
	 * <pre>
	 ...
	 *
	 * <p>
	 * The hash code of a byte buffer depends only upon its remaining elements; that
	 ...
	 * starting positions, are pointwise equal. #if[floatingPointType] This method
	 * considers two byte elements {@code a} and {@code b} to be equal if
	 * {@code (a == b) || (Byte.isNaN(a) && Byte.isNaN(b))}. The values {@code -0.0}
	 * and {@code +0.0} are considered to be equal, unlike
	 * {@link Byte#equals(Object)}. #end[floatingPointType]
	 * </p>
	 * </li>
	 *
	 * </ol>
	 ...
	 * sequence within its corresponding buffer. #if[floatingPointType] Pairs of
	 * {@code byte} elements are compared as if by invoking
	 * {@link Byte#compare(byte,byte)}, except that {@code -0.0} and {@code 0.0} are
	 * considered to be equal. {@code Byte.NaN} is considered by this method to be
	 * equal to itself and greater than all other {@code byte} values (including
	 * {@code Byte.POSITIVE_INFINITY}). #else[floatingPointType] Pairs of
	 * {@code byte} elements are compared as if by invoking
	 * {@link Byte#compare(byte,byte)}. #end[floatingPointType]
	 *
	 * <p>
	...
	public abstract ByteBuffer putShort(short f);

	public abstract ByteBuffer putShort(int index, short f);

	public abstract ByteBuffer putChar(char f);

	public abstract ByteBuffer putChar(int index, char f);

	public abstract ByteBuffer putInt(int f);

	public abstract ByteBuffer putInt(int index, int f);

	public abstract ByteBuffer putDouble(double f);

	public abstract ByteBuffer putDouble(int index, double f);
