...
//    private static final PlatformLogger log = PlatformLogger.getLogger("java.awt.KeyboardFocusManager");
	 ...
	 * current KeyboardFocusManager. If this permission is not granted, this method
	 * will throw a SecurityException, and the current KeyboardFocusManager will be
	 * unchanged.
	 *
...
//		checkReplaceKFMPermission();
	...
	/**
	 * The Component in an application that will regain focus when an outstanding
	 * temporary focus transfer has completed, or the focus owner, if no outstanding
	 * temporary transfer exists.
	 */
	 ...
	 * highlighted title bar. The active Window is always either the focused Window,
	 * or the first Frame or Dialog which is an owner of the focused Window.
	 */
	private static Window activeWindow;

	/**
	 * The default FocusTraversalPolicy for all Windows that have no policy of their
	 * own set. If those Windows have focus-cycle-root children that have no
	...
	private static final AWTKeyStroke[][] defaultFocusTraversalKeyStrokes = {
			{ AWTKeyStroke.getAWTKeyStroke(KeyEvent.VK_TAB, 0, false),
					AWTKeyStroke.getAWTKeyStroke(KeyEvent.VK_TAB, InputEvent.CTRL_DOWN_MASK | InputEvent.CTRL_MASK,
							false), },
			{ AWTKeyStroke.getAWTKeyStroke(KeyEvent.VK_TAB, InputEvent.SHIFT_DOWN_MASK | InputEvent.SHIFT_MASK, false),
					AWTKeyStroke.getAWTKeyStroke(KeyEvent.VK_TAB, InputEvent.SHIFT_DOWN_MASK | InputEvent.SHIFT_MASK
	 ...
	 * effect on all Windows that have no such array of their own explicitly set.
	 * Each array will also be inherited, recursively, by any child Component of
	 * those Windows that has no such array of its own explicitly set.
	 */
	 ...
	 * root, then it may be ambiguous as to which Components represent the next and
	 * previous Components to focus during normal focus traversal. In that case, the
	 * current focus cycle root is used to differentiate among the possibilities.
	 */
	 ...
	 * include this KeyboardFocusManager unless it was explicitly re-registered via
	 * a call to <code>addKeyEventDispatcher</code>. If no other KeyEventDispatchers
	 * are registered, this field may be null or refer to a List of length 0.
	 */
	private java.util.LinkedList<KeyEventDispatcher> keyEventDispatchers;

	/**
	 * This KeyboardFocusManager's KeyEventPostProcessor chain. The List does not
	 * include this KeyboardFocusManager unless it was explicitly re-registered via
	 * a call to <code>addKeyEventPostProcessor</code>. If no other
	 * KeyEventPostProcessors are registered, this field may be null or refer to a
	 * List of length 0.
	 */
...
//	private static AWTPermission replaceKeyboardFocusManagerPermission;
	 ...
	 * calling thread. The focus owner is defined as the Component in an application
	 * that will typically receive all KeyEvents generated by the user. KeyEvents
	 * which map to the focus owner's focus traversal keys will not be delivered if
	 * focus traversal keys are enabled for the focus owner. In addition,
	 * KeyEventDispatchers may retarget or consume KeyEvents before they reach the
	 * focus owner.
	 *
	 * @return the focus owner, or null if the focus owner is not a member of the
	...
	/**
	 * Returns the focus owner, even if the calling thread is in a different context
	 * than the focus owner. The focus owner is defined as the Component in an
	 * application that will typically receive all KeyEvents generated by the user.
	 * KeyEvents which map to the focus owner's focus traversal keys will not be
	 * delivered if focus traversal keys are enabled for the focus owner. In
	 * addition, KeyEventDispatchers may retarget or consume KeyEvents before they
	 * reach the focus owner.
	 * <p>
	 * This method will throw a SecurityException if this KeyboardFocusManager is
	 * not the current KeyboardFocusManager for the calling thread's context.
	 *
	 * @return the focus owner
	 * @see #getFocusOwner
	 * @see #setGlobalFocusOwner
	 * @throws SecurityException if this KeyboardFocusManager is not the current
	 *                           KeyboardFocusManager for the calling thread's
	 *                           context and if the calling thread does not have
...
//		checkKFMSecurity();
	...
	/**
	 * Sets the focus owner. The operation will be cancelled if the Component is not
	 * focusable. The focus owner is defined as the Component in an application that
	 * will typically receive all KeyEvents generated by the user. KeyEvents which
	 * map to the focus owner's focus traversal keys will not be delivered if focus
	 * traversal keys are enabled for the focus owner. In addition,
	 * KeyEventDispatchers may retarget or consume KeyEvents before they reach the
	 * focus owner.
	 * <p>
	 * This method does not actually set the focus to the specified Component. It
	 * merely stores the value to be subsequently returned by
	 * <code>getFocusOwner()</code>. Use <code>Component.requestFocus()</code> or
...
//		}
	...
	/**
	 * Clears the focus owner at both the Java and native levels if the focus owner
	 * exists and resides in the same context as the calling thread, otherwise the
	 * method returns silently.
	 * <p>
	 * The focus owner component will receive a permanent FOCUS_LOST event. After
	 * this operation completes, the native windowing system will discard all
	 * user-generated KeyEvents until the user selects a new Component to receive
	 * focus, or a Component is given focus explicitly via a call to
	 * {@code requestFocus()}. This operation does not change the focused or active
	 * Windows.
	 *
	 * @see Component#requestFocus()
	...
	/**
	 * Clears the global focus owner at both the Java and native levels. If there
	 * exists a focus owner, that Component will receive a permanent FOCUS_LOST
	 * event. After this operation completes, the native windowing system will
	 * discard all user-generated KeyEvents until the user selects a new Component
	 * to receive focus, or a Component is given focus explicitly via a call to
	 ...
	 * granted, this method will throw a SecurityException, and the current focus
	 * owner will not be cleared.
	 * <p>
	 * This method is intended to be used only by KeyboardFocusManager set as
	 * current KeyboardFocusManager for the calling thread's context. It is not for
	 * general client use.
	 *
...
//        }
	 ...
	 * same context as the calling thread. The permanent focus owner is defined as
	 * the last Component in an application to receive a permanent FOCUS_GAINED
	 ...
	 * permanent focus owner will again be the focus owner when the temporary focus
	 * change ends.
	 *
	 * @return the permanent focus owner, or null if the permanent focus owner is
	 *         not a member of the calling thread's context
	 ...
	 * different context than the permanent focus owner. The permanent focus owner
	 * is defined as the last Component in an application to receive a permanent
	 * FOCUS_GAINED event. The focus owner and permanent focus owner are equivalent
	 * unless a temporary focus change is currently in effect. In such a situation,
	 * the permanent focus owner will again be the focus owner when the temporary
	 * focus change ends.
	 *
	 ...
	 * Component is not focusable. The permanent focus owner is defined as the last
	 * Component in an application to receive a permanent FOCUS_GAINED event. The
	 * focus owner and permanent focus owner are equivalent unless a temporary focus
	 * change is currently in effect. In such a situation, the permanent focus owner
	 * will again be the focus owner when the temporary focus change ends.
	 * <p>
	 * This method does not actually set the focus to the specified Component. It
	 * merely stores the value to be subsequently returned by
	...
	/**
	 * Returns the focused Window, if the focused Window is in the same context as
	 * the calling thread. The focused Window is the Window that is or contains the
	 * focus owner.
	 *
	 * @return the focused Window, or null if the focused Window is not a member of
	 ...
	 * context than the focused Window. The focused Window is the Window that is or
	...
	/**
	 * Sets the focused Window. The focused Window is the Window that is or contains
	 * the focus owner. The operation will be cancelled if the specified Window to
	 * focus is not a focusable Window.
	 * <p>
	 * This method does not actually change the focused Window as far as the native
	 * windowing system is concerned. It merely stores the value to be subsequently
	 * returned by <code>getFocusedWindow()</code>. Use
	 * <code>Component.requestFocus()</code> or
	 * <code>Component.requestFocusInWindow()</code> to change the focused Window,
	 * subject to platform limitations.
	 *
	...
	/**
	 * Returns the active Window, if the active Window is in the same context as the
	 * calling thread. Only a Frame or a Dialog can be the active Window. The native
	 * windowing system may denote the active Window or its children with special
	 * decorations, such as a highlighted title bar. The active Window is always
	 * either the focused Window, or the first Frame or Dialog that is an owner of
	 * the focused Window.
	 *
	 * @return the active Window, or null if the active Window is not a member of
	 *         the calling thread's context
	 ...
	 * context than the active Window. Only a Frame or a Dialog can be the active
	...
	/**
	 * Sets the active Window. Only a Frame or a Dialog can be the active Window.
	 * The native windowing system may denote the active Window or its children with
	 * special decorations, such as a highlighted title bar. The active Window is
	 * always either the focused Window, or the first Frame or Dialog that is an
	 * owner of the focused Window.
	 * <p>
	 * This method does not actually change the active Window as far as the native
	 * windowing system is concerned. It merely stores the value to be subsequently
	 * returned by <code>getActiveWindow()</code>. Use
	 * <code>Component.requestFocus()</code> or
...
//            }
	...
	/**
	 * Returns the default FocusTraversalPolicy. Top-level components use this value
	 * on their creation to initialize their own focus traversal policy by explicit
	 * call to Container.setFocusTraversalPolicy.
	 *
	...
	/**
	 * Sets the default FocusTraversalPolicy. Top-level components use this value on
	 * their creation to initialize their own focus traversal policy by explicit
	 * call to Container.setFocusTraversalPolicy. Note: this call doesn't affect
	 * already created components as they have their policy initialized. Only new
	 * components will use this policy as their default policy.
	 *
	...
	/**
	 * Sets the default focus traversal keys for a given traversal operation. This
	 * traversal key {@code Set} will be in effect on all {@code Window}s that have
	 * no such {@code Set} of their own explicitly defined. This {@code Set} will
	 * also be inherited, recursively, by any child {@code Component} of those
	 * {@code Windows} that has no such {@code Set} of its own explicitly defined.
	 * <p>
	 ...
	 * <p>
	 * Using the {@code AWTKeyStroke} API, client code can specify on which of two
	 * specific {@code KeyEvent}s, {@code KEY_PRESSED} or {@code KEY_RELEASED}, the
	 * focus traversal operation will occur. Regardless of which {@code KeyEvent} is
	 * specified, however, all {@code KeyEvent}s related to the focus traversal key,
	 * including the associated {@code KEY_TYPED} event, will be consumed, and will
	 * not be dispatched to any {@code Component}. It is a runtime error to specify
	 * a {@code KEY_TYPED} event as mapping to a focus traversal operation, or to
	 * map the same event to multiple default focus traversal operations.
	 * <p>
	 * This method may throw a {@code ClassCastException} if any {@code Object} in
	 * {@code keystrokes} is not an {@code AWTKeyStroke}.
	 *
	 * @param id         one of {@code KeyboardFocusManager.FORWARD_TRAVERSAL_KEYS},
	 ...
	 * @param keystrokes the Set of {@code AWTKeyStroke}s for the specified
	 *                   operation
	 * @see #getDefaultFocusTraversalKeys
	 * @see Component#setFocusTraversalKeys
	 * @see Component#getFocusTraversalKeys
	 ...
	 *                                  or
	 *                                  {@code KeyboardFocusManager.DOWN_CYCLE_TRAVERSAL_KEYS},
	 *                                  or if keystrokes is {@code null}, or if
	 *                                  keystrokes contains {@code null}, or if any
	 *                                  keystroke represents a {@code KEY_TYPED}
	 ...
	 * operation. This traversal key Set will be in effect on all Windows that have
	 * no such Set of their own explicitly defined. This Set will also be inherited,
	 * recursively, by any child Component of those Windows that has no such Set of
	 * its own explicitly defined. (See <code>setDefaultFocusTraversalKeys</code>
	 ...
	 * @return the <code>Set</code> of <code>AWTKeyStroke</code>s for the specified
	 *         operation; the <code>Set</code> will be unmodifiable, and may be
	 *         empty; <code>null</code> will never be returned
	 * @see #setDefaultFocusTraversalKeys
	 * @see Component#setFocusTraversalKeys
	 * @see Component#getFocusTraversalKeys
	 * @throws IllegalArgumentException if id is not one of
	...
	/**
	 * Returns the current focus cycle root, if the current focus cycle root is in
	 * the same context as the calling thread. If the focus owner is itself a focus
	 * cycle root, then it may be ambiguous as to which Components represent the
	 * next and previous Components to focus during normal focus traversal. In that
	 * case, the current focus cycle root is used to differentiate among the
	 * possibilities.
	 * <p>
	 * This method is intended to be used only by KeyboardFocusManagers and focus
	 * implementations. It is not for general client use.
	 *
	 * @return the current focus cycle root, or null if the current focus cycle root
	 ...
	 * different context than the current focus cycle root. If the focus owner is
	 * itself a focus cycle root, then it may be ambiguous as to which Components
	...
	/**
	 * Sets the current focus cycle root. If the focus owner is itself a focus cycle
	 * root, then it may be ambiguous as to which Components represent the next and
	 * previous Components to focus during normal focus traversal. In that case, the
	 * current focus cycle root is used to differentiate among the possibilities.
	 * <p>
	 ...
	 * "replaceKeyboardFocusManager" AWTPermission. If this permission is not
	 * granted, this method will throw a SecurityException, and the current focus
	 * cycle root will not be changed.
	 * <p>
	 * This method is intended to be used only by KeyboardFocusManagers and focus
	 * implementations. It is not for general client use.
	 *
...
//        });
	 ...
	 * be used to remove the PropertyChangeListeners that were registered for all
	 ...
	 *         <code>PropertyChangeListener</code>s or an empty array if no property
	...
	/**
	 * Returns an array of all the <code>PropertyChangeListener</code>s associated
	 * with the named property.
	 *
	 * @return all of the <code>PropertyChangeListener</code>s associated with the
	 *         named property or an empty array if no such listeners have been
	 *         added.
	 *
	 * @see #addPropertyChangeListener(java.lang.String,java.beans.PropertyChangeListener)
	 * @see #removePropertyChangeListener(java.lang.String,java.beans.PropertyChangeListener)
	...
	/**
	 * Fires a PropertyChangeEvent in response to a change in a bound property. The
	 * event will be delivered to all registered PropertyChangeListeners. No event
	 * will be delivered if oldValue and newValue are the same.
	 *
	...
	/**
	 * Removes a VetoableChangeListener from the listener list. This method should
	 * be used to remove the VetoableChangeListeners that were registered for all
	 * vetoable properties of this class.
	 * <p>
	 * If listener is null, no exception is thrown and no action is performed.
	 *
	 ...
	 *
	 * @return all of this keyboard focus manager's
	 *         <code>VetoableChangeListener</code>s or an empty array if no vetoable
	 *         change listeners are currently registered
	 *
	 * @see #addVetoableChangeListener
	 * @see #removeVetoableChangeListener
	 ...
	 * property. This method should be used to remove VetoableChangeListeners that
	 * were registered for a specific bound property.
	 * <p>
	 * If listener is null, no exception is thrown and no action is performed.
	 *
	...
	/**
	 * Returns an array of all the <code>VetoableChangeListener</code>s associated
	 * with the named property.
	 *
	 * @return all of the <code>VetoableChangeListener</code>s associated with the
	 *         named property or an empty array if no such listeners have been
	 *         added.
	 *
	 * @see #addVetoableChangeListener(java.lang.String,java.beans.VetoableChangeListener)
	 ...
	 * The event will be delivered to all registered VetoableChangeListeners. If a
	 * VetoableChangeListener throws a PropertyVetoException, a new event is fired
	 * reverting all VetoableChangeListeners to the old value and the exception is
	 * then rethrown. No event will be delivered if oldValue and newValue are the
	 * same.
	 *
	 * @param propertyName the name of the property that has changed
	 * @param oldValue     the property's previous value
	 ...
	 * This KeyboardFocusManager will request that each KeyEventDispatcher dispatch
	 * KeyEvents generated by the user before finally dispatching the KeyEvent
	 * itself. KeyEventDispatchers will be notified in the order in which they were
	 * added. Notifications will halt as soon as one KeyEventDispatcher returns
	 * <code>true</code> from its <code>dispatchKeyEvent</code> method. There is no
	 * limit to the total number of KeyEventDispatchers which can be added, nor to
	 * the number of times which a particular KeyEventDispatcher instance can be
	 * added.
	 * <p>
	 * If a null dispatcher is specified, no action is taken and no exception is
	 * thrown.
	 * <p>
	 * In a multithreaded application, {@link KeyEventDispatcher} behaves the same
	 * as other AWT listeners. See
	 ...
	 * <p>
	 * If a null dispatcher is specified, if the specified dispatcher is not in the
	 * dispatcher chain, or if this KeyboardFocusManager is specified without having
	 * been explicitly re-registered, no action is taken and no exception is thrown.
	 * <p>
	 * In a multithreaded application, {@link KeyEventDispatcher} behaves the same
	 * as other AWT listeners. See
	 * <a href="doc-files/AWTThreadIssues.html#ListenersThreads" >AWT Threading
	 * Issues</a> for more details.
	 *
	 * @param dispatcher the KeyEventDispatcher to remove from the dispatcher chain
	...
	/**
	 * Returns this KeyboardFocusManager's KeyEventDispatcher chain as a List. The
	 * List will not include this KeyboardFocusManager unless it was explicitly
	 * re-registered via a call to <code>addKeyEventDispatcher</code>. If no other
	 * KeyEventDispatchers are registered, implementations are free to return null
	 * or a List of length 0. Client code should not assume one behavior over
	 * another, nor should it assume that the behavior, once established, will not
	 * change.
	 *
	...
	/**
	 * Adds a KeyEventPostProcessor to this KeyboardFocusManager's post- processor
	 * chain. After a KeyEvent has been dispatched to and handled by its target,
	 * KeyboardFocusManager will request that each KeyEventPostProcessor perform any
	 * necessary post-processing as part of the KeyEvent's final resolution.
	 * KeyEventPostProcessors will be notified in the order in which they were
	 * added; the current KeyboardFocusManager will be notified last. Notifications
	 * will halt as soon as one KeyEventPostProcessor returns <code>true</code> from
	 * its <code>postProcessKeyEvent</code> method. There is no limit to the the
	 * total number of KeyEventPostProcessors that can be added, nor to the number
	 * of times that a particular KeyEventPostProcessor instance can be added.
	 * <p>
	 * If a null post-processor is specified, no action is taken and no exception is
	 * thrown.
	 * <p>
	 ...
	 * Issues</a> for more details.
	 *
	 * @param processor the KeyEventPostProcessor to add to the post-processor chain
	 ...
	 * KeyboardFocusManager's post-processor chain. This KeyboardFocusManager cannot
	 * itself be entirely removed from the chain. Only additional references added
	 * via <code>addKeyEventPostProcessor</code> can be removed.
	 * <p>
	 * If a null post-processor is specified, if the specified post-processor is not
	 * in the post-processor chain, or if this KeyboardFocusManager is specified
	 * without having been explicitly added, no action is taken and no exception is
	 * thrown.
	 * <p>
	 * In a multithreaded application, {@link KeyEventPostProcessor} behaves the
	 * same as other AWT listeners. See
	 ...
	 * Issues</a> for more details.
	 *
	 * @param processor the KeyEventPostProcessor to remove from the post- processor
	 *                  chain
	 ...
	 * The List will not include this KeyboardFocusManager unless it was explicitly
	 * added via a call to <code>addKeyEventPostProcessor</code>. If no
	 * KeyEventPostProcessors are registered, implementations are free to return
	 * null or a List of length 0. Client code should not assume one behavior over
	 * another, nor should it assume that the behavior, once established, will not
	 * change.
	 *
...
//        return weakValue == null ? null : (Component)weakValue.get();
	 ...
	 * KeyboardFocusManager dispatch the specified event on its behalf. It is
	 * expected that all KeyboardFocusManagers will dispatch all FocusEvents, all
	 * WindowEvents related to focus, and all KeyEvents. These events should be
	 * dispatched based on the KeyboardFocusManager's notion of the focus owner and
	 * the focused and active Windows, sometimes overriding the source of the
	 * specified AWTEvent. Dispatching must be done using
	...
	/**
	 * Redispatches an AWTEvent in such a way that the AWT event dispatcher will not
	 * recursively request that the KeyboardFocusManager, or any installed
	 * KeyEventDispatchers, dispatch the event again. Client implementations of
	 * <code>dispatchEvent</code> and client-defined KeyEventDispatchers must call
	 * <code>redispatchEvent(target, e)</code> instead of
	 ...
	 * other KeyEventDispatcher in the dispatcher chain dispatched the KeyEvent, or
	 * if no other KeyEventDispatchers are registered. If an implementation of this
	 * method returns <code>false</code>, <code>dispatchEvent</code> may try to
	 * dispatch the KeyEvent itself, or may simply return <code>false</code>. If
	 * <code>true</code> is returned, <code>dispatchEvent</code> should return
	 * <code>true</code> as well.
	 *
	 * @param e the KeyEvent which the current KeyboardFocusManager has requested
	 *          that this KeyEventDispatcher dispatch
	 * @return <code>true</code> if the KeyEvent was dispatched; <code>false</code>
	 *         otherwise
	 * @see #dispatchEvent
	 */
	@Override
	...
	/**
	 * This method will be called by <code>dispatchKeyEvent</code>. By default, this
	 * method will handle any unconsumed KeyEvents that map to an AWT
	 * <code>MenuShortcut</code> by consuming the event and activating the shortcut.
	 *
	 ...
	 * if focusedComponent were the current focus owner.
	 *
	 * @param focusedComponent the Component that will be the basis for a focus
	 *                         traversal operation if the specified event represents
	 *                         a focus traversal key for the Component
	 ...
	 * owner. If client code requests a focus change, and the AWT determines that
	 * this request might be granted by the native windowing system, then the AWT
	 * will call this method. It is the responsibility of the KeyboardFocusManager
	 * to delay dispatching of KeyEvents with timestamps later than the specified
	 * time stamp until the specified Component receives a FOCUS_GAINED event, or
	 * the AWT cancels the delay request by invoking <code>dequeueKeyEvents</code>
	 * or <code>discardKeyEvents</code>.
	 *
	...
	/**
	 * Called by the AWT to notify the KeyboardFocusManager that it should cancel
	 * delayed dispatching of KeyEvents. All KeyEvents which were enqueued because
	 * of a call to <code>enqueueKeyEvents</code> with the same timestamp and
	 * Component should be released for normal dispatching to the current focus
	 * owner. If the given timestamp is less than zero, the outstanding enqueue
	 * request for the given Component with the <b> oldest</b> timestamp (if any)
	 * should be cancelled.
	 *
	 ...
	 * Called by the AWT to notify the KeyboardFocusManager that it should cancel
	 * delayed dispatching of KeyEvents. All KeyEvents which were enqueued because
	 * of one or more calls to <code>enqueueKeyEvents</code> with the same Component
	 * should be discarded.
	 *
	 ...
	 * cycle root is a Window, then typically the focus owner is set to the Window's
	...
	/**
	 * Moves the focus down one focus traversal cycle. Typically, if aContainer is a
	 * focus cycle root, then the focus owner is set to aContainer's default
	 * Component to focus, and the current focus cycle root is set to aContainer. If
	 ...
	 * Typically, the new focus owner is set to the current focus owner's focus
	 * cycle root, and the current focus cycle root is set to the new focus owner's
	 * focus cycle root. If, however, the current focus owner's focus cycle root is
	 * a Window, then typically the focus owner is set to the focus cycle root's
	 * default Component to focus, and the current focus cycle root is unchanged.
	 */
	...
	/**
	 * Moves the focus down one focus traversal cycle from the current focus owner,
	 * if and only if the current focus owner is a Container that is a focus cycle
	 * root. Typically, the focus owner is set to the current focus owner's default
	 * Component to focus, and the current focus cycle root is set to the current
	 * focus owner. If the current focus owner is not a Container that is a focus
	 * cycle root, then no focus traversal operation occurs.
	 */
...
//            }
	...
	/**
	 * Indicates whether the native implementation should proceed with a pending,
	 * native focus request. Before changing the focus at the native level, the AWT
	 * implementation should always call this function for permission. This function
	 * will reject the request if a duplicate request preceded it, or if the
	 * specified heavyweight Component already owns the focus and no native focus
	 * changes are pending. Otherwise, the request will be approved and the focus
	 * request list will be updated so that, if necessary, the proper descendant
	 * will be focused when the corresponding FOCUS_GAINED event on the heavyweight
	 * is received.
	 *
	 * An implementation must ensure that calls to this method and native focus
	 * changes are atomic. If this is not guaranteed, then the ordering of the focus
	 * request list may be incorrect, leading to errors in the type-ahead mechanism.
	 * Typically this is accomplished by only calling this function from the native
	 * event pumping thread, or by holding a global, native lock during invocation.
	 */
...
//                }
	 ...
	 * null if this is a duplicate request. The active Window is useful because some
	 * native platforms do not support setting the native focus owner to null. On
	 * these platforms, the obvious choice is to set the focus owner to the focus
	 * proxy of the active Window.
	 */
	 ...
	 * 
	 * @param ex previously caught exception that may be processed right here, or
	 * null
	 * 
		...
		try {
			if (localLightweightRequests != null) {
				...
				for (Iterator<KeyboardFocusManager.LightweightFocusRequest> iter = localLightweightRequests
					 ...
					 *
					 * We allow to trigger restoreFocus() in the dispatching process only if we have
					 * the last request to dispatch. If the last request fails, focus will be
					 * restored to either the component of the last previously succedded request, or
					 * to to the focus owner that was before this clearing process.
					 */
					if (!iter.hasNext()) {
					...
					FocusEvent currentFocusOwnerEvent = null;
					/*
					 * We're not dispatching FOCUS_LOST while the current focus owner is null. But
					 * regardless of whether it's null or not, we're clearing ALL the local lw
					 * requests.
					 */
					if (currentFocusOwner != null) {
	...
	static FocusEvent retargetFocusGained(FocusEvent fe) {
		...
		synchronized (heavyweightRequests) {
			...
			if (hwFocusRequest != null && nativeSource == hwFocusRequest.heavyweight) {
				...
				if (currentFocusOwner != null) {
					/*
					 * Since we receive FOCUS_GAINED when current focus owner is not null,
					 * correcponding FOCUS_LOST is supposed to be lost. And so, we keep new focus
					 * owner to determine synthetic FOCUS_LOST event which will be generated by
					 * KeyboardFocusManager for this FOCUS_GAINED.
					 *
					 * This code based on knowledge of DefaultKeyboardFocusManager's implementation
					 * and might be not applicable for another KeyboardFocusManager.
					 */
...
//	static proxyActive;
	...
	private static boolean isProxyActiveImpl(KeyEvent e) {
		return /** @j2sNative e.isProxyActive ||*/false;
//		if (proxyActive == null) {
//			try {
//				proxyActive = java.awt.event.KeyEvent.KeyEvent.class.getDeclaredField("isProxyActive");
