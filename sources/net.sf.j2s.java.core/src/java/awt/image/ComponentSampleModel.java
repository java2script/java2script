...
public class ComponentSampleModel extends SampleModel {
	...
	/**
	 * Line stride (in data array elements) of the region of image data described by
	 * this ComponentSampleModel.
	 */
	protected int scanlineStride;

	/**
	 * Pixel stride (in data array elements) of the region of image data described
	 * by this ComponentSampleModel.
	 */
...
//    }
	...
	/**
	 * Constructs a ComponentSampleModel with the specified parameters. The number
	 * of bands will be given by the length of the bandOffsets array. All bands will
	 * be stored in the first bank of the DataBuffer.
	 * 
	 ...
	 * @throws IllegalArgumentException if <code>w</code> or <code>h</code> is not
	 *                                  greater than 0
	 * @throws IllegalArgumentException if <code>pixelStride</code> is less than 0
	 * @throws IllegalArgumentException if <code>scanlineStride</code> is less than
	 *                                  0
	 * @throws IllegalArgumentException if <code>numBands</code> is less than 1
	 * @throws IllegalArgumentException if the product of <code>w</code> and
	 *                                  <code>h</code> is greater than
	 *                                  <code>Integer.MAX_VALUE</code>
	 * @throws IllegalArgumentException if <code>dataType</code> is not one of the
...
//            (dataType > DataBuffer.TYPE_DOUBLE)
	...
	/**
	 * Constructs a ComponentSampleModel with the specified parameters. The number
	 * of bands will be given by the length of the bandOffsets array. Different
	 * bands may be stored in different banks of the DataBuffer.
	 *
	 * @param dataType       the data type for storing samples
	 * @param w              the width (in pixels) of the region of image data
	 *                       described
	 ...
	 * @throws IllegalArgumentException if <code>scanlineStride</code> is less than
	 *                                  0
	 * @throws IllegalArgumentException if the length of <code>bankIndices</code>
	 *                                  does not equal the length of
	 *                                  <code>bankOffsets</code>
	 * @throws IllegalArgumentException if any of the bank indices of
	 *                                  <code>bandIndices</code> is less than 0
	 * @throws IllegalArgumentException if <code>dataType</code> is not one of the
	 *                                  supported data types
	 */
...
//            
	...
	/**
	 * Creates a new <code>ComponentSampleModel</code> with the specified width and
	 * height. The new <code>SampleModel</code> will have the same number of bands,
	 * storage data type, interleaving scheme, and pixel stride as this
	 ...
	 * DataBuffer that the existing ComponentSampleModel can be used with. The new
	 * ComponentSampleModel/DataBuffer combination will represent an image with a
	 * subset of the bands of the original ComponentSampleModel/DataBuffer
	 * combination.
	 * 
	 * @param bands a subset of bands from this <code>ComponentSampleModel</code>
	 * @return a <code>ComponentSampleModel</code> created with a subset of bands
	 *         from this <code>ComponentSampleModel</code>.
	 */
	@Override
	 ...
	 * <code>ComponentSampleModel</code>. The <code>DataBuffer</code> object's data
	 * type, number of banks, and size are be consistent with this
	 * <code>ComponentSampleModel</code>.
	 * 
	 * @return a <code>DataBuffer</code> whose data type, number of banks and size
	 *         are consistent with this <code>ComponentSampleModel</code>.
	 */
	@Override
...
//            break;
	...
	/**
	 * Gets the offset for the first band of pixel (x,y). A sample of the first band
	 * can be retrieved from a <code>DataBuffer</code> <code>data</code> with a
	 * <code>ComponentSampleModel</code> <code>csm</code> as
	 * 
	 * <pre>
	...
	/**
	 * Gets the offset for band b of pixel (x,y). A sample of band <code>b</code>
	 * can be retrieved from a <code>DataBuffer</code> <code>data</code> with a
	 * <code>ComponentSampleModel</code> <code>csm</code> as
	 * 
	 * <pre>
	 ...
	 * <code>ComponentSampleModel</code>, this is identical to the number of bands.
	 * 
	 ...
	 * <code>TransferType</code>. For a <code>ComponentSampleModel</code>, this is
	 * the same as the data type, and samples are returned one per array element.
	 * Generally, <code>obj</code> should be passed in as <code>null</code>, so that
	 * the <code>Object</code> is created automatically and is the right primitive
	 * data type.
	 * <p>
	 * The following code illustrates transferring data for one pixel from
	 * <code>DataBuffer</code> <code>db1</code>, whose storage layout is described
	 * by <code>ComponentSampleModel</code> <code>csm1</code>, to
	 * <code>DataBuffer</code> <code>db2</code>, whose storage layout is described
	 * by <code>ComponentSampleModel</code> <code>csm2</code>. The transfer is
	 * usually more efficient than using <code>getPixel</code> and
	 * <code>setPixel</code>.
	 * 
	 * <pre>
	 ...
	 * transfer between two <code>DataBuffer/SampleModel</code> pairs is legitimate
	 * if the <code>SampleModel</code> objects have the same number of bands,
	 * corresponding bands have the same number of bits per sample, and the
	 * <code>TransferType</code>s are the same.
	 * <p>
	 * If <code>obj</code> is not <code>null</code>, it should be a primitive array
	 ...
	 * @param obj  if non-<code>null</code>, a primitive array in which to return
	 *             the pixel data
	 ...
	 *                                        or if obj is too small to hold the
	 *                                        ouput.
	 */
	@Override
	...
	/**
	 * Returns all samples for the specified pixel in an int array, one sample per
	 * array element. An <code>ArrayIndexOutOfBoundsException</code> might be thrown
	 ...
	 * @throws NullPointerException           if data is null.
	 * @throws ArrayIndexOutOfBoundsException if the coordinates are not in bounds,
	 *                                        or if iArray is too small to hold the
	 *                                        output.
	 */
	@Override
	...
	/**
	 * Returns all samples for the specified rectangle of pixels in an int array,
	 * one sample per array element. An <code>ArrayIndexOutOfBoundsException</code>
	 ...
	 * float. An <code>ArrayIndexOutOfBoundsException</code> might be thrown if the
	 * coordinates are not in bounds.
	 * 
	 * @param x    The X coordinate of the pixel location
	 * @param y    The Y coordinate of the pixel location
	 * @param b    The band to return
	 * @param data The DataBuffer containing the image data
	 * @return a float value representing the sample in the specified band for the
	 *         specified pixel.
	 */
	@Override
	 ...
	 * double. An <code>ArrayIndexOutOfBoundsException</code> might be thrown if the
	 * coordinates are not in bounds.
	 * 
	 * @param x    The X coordinate of the pixel location
	 * @param y    The Y coordinate of the pixel location
	 * @param b    The band to return
	 * @param data The DataBuffer containing the image data
	 * @return a double value representing the sample in the specified band for the
	 *         specified pixel.
	 */
	@Override
	...
	/**
	 * Returns the samples in a specified band for the specified rectangle of pixels
	 * in an int array, one sample per data array element. An
	 * <code>ArrayIndexOutOfBoundsException</code> might be thrown if the
	 * coordinates are not in bounds.
	 * 
	 * @param x      The X coordinate of the upper left pixel location
	 ...
	 * @param h      the height of the pixel rectangle
	 * @param b      the band to return
	 * @param iArray if non-<code>null</code>, returns the samples in this array
	 * @param data   the <code>DataBuffer</code> containing the image data
	 ...
	 * <code>ArrayIndexOutOfBoundsException</code> might be thrown if the
	 * coordinates are not in bounds, or if <code>obj</code> is not large enough to
	 * hold the pixel data.
	 * 
	 * @param x    the X coordinate of the pixel location
	 * @param y    the Y coordinate of the pixel location
	...
	/**
	 * Sets a pixel in the <code>DataBuffer</code> using an int array of samples for
	 * input. An <code>ArrayIndexOutOfBoundsException</code> might be thrown if the
	 * coordinates are not in bounds.
	 * 
	 * @param x      The X coordinate of the pixel location
	 * @param y      The Y coordinate of the pixel location
	 ...
	 * sample per array element. An <code>ArrayIndexOutOfBoundsException</code>
	 * might be thrown if the coordinates are not in bounds.
	 * 
	 * @param x      The X coordinate of the upper left pixel location
	 * @param y      The Y coordinate of the upper left pixel location
	 ...
	 * from an int array containing one sample per data array element. An
	 * <code>ArrayIndexOutOfBoundsException</code> might be thrown if the
	 * coordinates are not in bounds.
	 * 
	 * @param x      The X coordinate of the upper left pixel location
