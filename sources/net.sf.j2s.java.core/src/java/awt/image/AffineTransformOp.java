...
// SwingJS not implemented
              throw new ImagingOpException ("SwingJS: Unable to transform src image");
...
//        }
	...
	public BufferedImage createCompatibleDestImage(BufferedImage src,
		...
		if (destCM == null) {
			...
			if (interpolationType != TYPE_NEAREST_NEIGHBOR && (
			// SwingJS not implemented cm instanceof IndexColorModel ||
					cm.getTransparency() == Transparency.OPAQUE)) {
				image = new BufferedImage(w, h, BufferedImage.TYPE_INT_ARGB);
			} else {
				image = new BufferedImage(cm, src.getRaster()
						.createCompatibleWritableRaster(w, h), cm.isAlphaPremultiplied(),
						null);
			}
		} else {
