 ...
 * {@link #dispatchEvent(AWTEvent) dispatchEvent(AWTEvent)} method on this
 * <code>EventQueue</code> with the event to be dispatched as an argument. The
 * particular behavior of this machinery is implementation-dependent. The only
 * requirements are that events which were actually enqueued to this queue (note
 * that events being posted to the <code>EventQueue</code> can be coalesced) are
 * dispatched:
 * <dl>
 * <dt>Sequentially.
...
public class EventQueue {
	...
	/*
	 * We maintain one Queue for each priority that the EventQueue supports. That
	 * is, the EventQueue object is actually implemented as NUM_PRIORITIES queues
	 * and all Events on a particular internal Queue have identical priority.
	 * Events are pulled off the EventQueue starting with the Queue of highest
	 * priority. We progress in decreasing order across all Queues.
	 */
	private Queue[] queues = new Queue[NUM_PRIORITIES];

	/*
	 * The next EventQueue on the stack, or null if this EventQueue is on the top
	 * of the stack. If nextQueue is non-null, requests to post an event are
	 ...
	 */
	 private AWTEvent currentEvent; // SwingJS was WeakReference
...
//    }
	...
	public EventQueue() {
		...
		/*
		 * NOTE: if you ever have to start the associated event dispatch thread at
		 * this point, be aware of the following problem: If this EventQueue
	...
	/**
	 * Posts the event to the internal Queue of specified priority, coalescing as
	 * appropriate.
	 * 
	 * @param theEvent
	 *          an instance of <code>java.awt.AWTEvent</code>, or a subclass of it
...
//		}
	 ...
	 * the entire application 10 times by stalling the Event processing. Only here
	 * by backward compatibility reasons.
	 */
	 ...
	 * @param src
	 */
	public void dispatchEventAndWait(AWTEvent event, Object src) {
		dispatchEventImpl(event, src, true);
	}
	 ...
	 */
	private void dispatchEventImpl(final AWTEvent event, final Object src, boolean andWait) { //SwingJS adds andWait
		event.isPosted = true;
		if (event instanceof ActiveEvent) {
			// This could become the sole method of dispatching in time.
			setCurrentEventAndMostRecentTimeImpl(event);
			JSToolkit.dispatchEvent(event, null, andWait); 
		} else if (src instanceof Component) {
			JSToolkit.dispatchEvent(event, src, andWait);
			event.dispatched();
	 ...
	 * calling thread. If an event with a timestamp is currently being dispatched,
	 * its timestamp will be returned. If no events have yet been dispatched, the
	 * EventQueue's initialization time will be returned instead.In the current
	 * version of the JDK, only <code>InputEvent</code>s, <code>ActionEvent</code>
	 * s, and <code>InvocationEvent</code>s have timestamps; however, future
	 * versions of the JDK may add timestamps to additional event types. Note that
	 * this method should only be invoked from an application's event dispatching
	 * thread. If this method is invoked from another thread, the current system
	 * time (as reported by <code>System.currentTimeMillis()</code>) will be
	 * returned instead.
	 * 
	 ...
	 * <code>EventQueue</code> associated with the calling thread. This is useful
	 * if a method needs access to the event, but was not designed to receive a
	 * reference to it as an argument. Note that this method should only be
	 * invoked from an application's event dispatching thread. If this method is
	 * invoked from another thread, null will be returned.
	 * 
	 ...
	 * 
	 * @param newEventQueue
	 *          an <code>EventQueue</code> (or subclass thereof) instance to be
	 *          use
	...
	public synchronized void push(EventQueue newEventQueue) {
		...
		/*
		 * Stop the event dispatch thread associated with the currently active event
		 * queue, so that after the new queue is pushed on the top this event
		 * dispatch thread won't prevent AWT from being automatically shut down. Use
		 * stopDispatchingLater() to avoid deadlock: stopDispatching() waits for the
		 * dispatch thread to exit, so if the dispatch thread attempts to
	 ...
	 *              if no previous push was made on this <code>EventQueue</code>
	 ...
	 * 
	 * @return true if running on the current AWT <code>EventQueue</code>'s
	 *         dispatch thread
	 * @since 1.2
	 */
	public static boolean isDispatchThread() {
		// SwingJS just wraps a dispatch with a variable that can be tested
		return JSToolkit.isDispatchThread();
...
//							public Object run() {
								EventDispatchThread t = new EventDispatchThread(threadGroup,
										name, EventQueue.this);
...
//						});
	...
	/*
	 * Gets the <code>EventDispatchThread</code> for this <code>EventQueue</code>.
	 * 
	...
	private synchronized void setCurrentEventAndMostRecentTimeImpl(AWTEvent e) {
		...
		currentEvent = e;//new WeakReference(e);
	...
	/**
	 * Causes <code>runnable</code> to have its <code>run</code> method called in
	 * the dispatch thread of {@link Toolkit#getSystemEventQueue the system
	 * EventQueue}. This will happen after all pending events are processed.
	 * 
	 * @param runnable
	 *          the <code>Runnable</code> whose <code>run</code> method should be
	 *          executed synchronously on the <code>EventQueue</code>
	 * @see #invokeAndWait
	 * @since 1.2
	 */
	 ...
	 *              if an throwable is thrown when running <code>runnable</code>
	 * @see #invokeLater
	 * @since 1.2
	 */
	public static void invokeAndWait(Runnable runnable)
			throws InterruptedException, InvocationTargetException {
		invokeAndWaitStatic(Toolkit.getDefaultToolkit(), runnable);
	}

	private static void invokeAndWaitStatic(Object source, Runnable runnable)
			throws InterruptedException, InvocationTargetException {
...
//
	 ...
	 * First it proceeds to the EventQueue on the top of the stack, then notifies
	 * the associated dispatch thread if it exists or starts a new one otherwise.
	 */
	public void wakeup(boolean isShutdown) {
		synchronized (this) {
			if (nextQueue != null) {
