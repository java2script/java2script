...
public class DefaultKeyboardFocusManager extends KeyboardFocusManager {
	...
	// PlatformLogger.getLogger("java.awt.focus.DefaultKeyboardFocusManager");
...
//    private WeakReference<Component> realOppositeComponentWR = NULL_COMPONENT_WR;
	private Window realOppositeWindowWR = null;// NULL_WINDOW_WR;
	private Component realOppositeComponentWR = null;// NULL_COMPONENT_WR;
	...
	//protected Component jcMouseDown;
...
//
	...
	/*
	 * This series of restoreFocus methods is used for recovering from a rejected
	 * focus or activation change. Rejections typically occur when the user attempts
	 * to focus a non-focusable Component or Window.
	 */
	...
	/**
	 * Sends a synthetic AWTEvent to a Component. If the Component is in the current
	 * AppContext, then the event is immediately dispatched. If the Component is in
	 * a different AppContext, then the event is posted to the other AppContext's
	 * EventQueue, and this method blocks until the event is handled or target
	 * AppContext is disposed. Returns true if successfuly dispatched event, false
	 * if failed to dispatch.
	 */
...
//			}
	 ...
	 * queue (if any). This may happen when a user types ahead in the window, the
	 * client listeners hang EDT for a while, and the user switches b/w toplevels.
	 * In that case the focus window events may be dispatched before the type-ahead
	 * events get handled. This may lead to wrong focus behavior and in order to
	 * avoid it, the focus window events are reposted to the end of the event queue.
	 * See 6981400.
	 */
	 ...
	 * related to focus, and all KeyEvents. These events are dispatched based on the
	 ...
	 * method returns <code>false</code>, then the AWT event dispatcher will attempt
	 * to dispatch the event itself.
	 *
...
//                        focusLog.fine("Skipping {0} because current focus owner is equal to opposite", e);
			...
			realOppositeComponentWR = (fe.getOppositeComponent() != null) ? currentFocusOwner : null;
			
			
//			System.out.println("DefaultKeyboard OK");

			
//			if (jcMouseDown != null && jcMouseDown != e.getSource() && jcMouseDown.isFocusable()) {
//				jcMouseDown.requestFocus();
//				//System.out.println("DKBM requesting focus " + /** @j2sNative this.jcMouseDown.ui.id + "  " + document.activeElement || */null);
//				JSComponentUI ui = (JSComponentUI) ((JComponent) jcMouseDown).getUI();
//				jcMouseDown = null;
//				ui.requestFocus(jcMouseDown, false, true, System.currentTimeMillis(), CausedFocusEvent.Cause.MOUSE_EVENT);
////				
////				/**
////				 * @j2sNative setTimeout(function(){o.ui.requestFocus()},1);
////				 */
////				Toolkit.getEventQueue().postEvent(new FocusEvent(jcMouseDown, FocusEvent.FOCUS_GAINED, true, (Component) e.getSource()));
//			}
...
//                                  losingFocusWindow, oppositeWindow);
			...
			if (currentFocusOwner != null) {
				...
				sendMessage(currentFocusOwner, new CausedFocusEvent(currentFocusOwner, FocusEvent.FOCUS_LOST, true,
			...
			realOppositeWindowWR = (oppositeWindow != null) ? currentFocusedWindow : null;
	...
	/**
	 * Called by <code>dispatchEvent</code> if no other KeyEventDispatcher in the
	 * dispatcher chain dispatched the KeyEvent, or if no other KeyEventDispatchers
	 * are registered. If the event has not been consumed, its target is enabled,
	 * and the focus owner is not null, this method dispatches the event to its
	 * target. This method will also subsequently dispatch the event to all
	 * registered KeyEventPostProcessors. After all this operations are finished,
	 * the event is passed to peers for processing.
	 * <p>
	 * In all cases, this method returns <code>true</code>, since
	 * DefaultKeyboardFocusManager is designed so that neither
	 * <code>dispatchEvent</code>, nor the AWT event dispatcher, should take further
	 * action on the event in any situation.
	 *
	...
	/**
	 * This method will be called by <code>dispatchKeyEvent</code>. It will handle
	 * any unconsumed KeyEvents that map to an AWT <code>MenuShortcut</code> by
	 * consuming the event and activating the shortcut.
	 *
...
//                dumpMarkers();
	...
	private boolean preDispatchKeyEvent(KeyEvent ke) {
		...
		// dispatching.
		// - If a key event is put to the type ahead queue, its time stamp should not be
		// registered
		// until its dispatching actually starts (by this method).
		EventQueue.setCurrentEventAndMostRecentTime(ke);

		/**
		 * Fix for 4495473. This fix allows to correctly dispatch events when native
		 * event proxying mechanism is active. If it is active we should redispatch key
		 * events after we detected its correct target.
		 */
...
//        }
	...
	/*
	 * @param e is a KEY_PRESSED event that can be used to track the next KEY_TYPED
	 * related.
	 */
	public void consumeNextKeyTyped(KeyEvent e) {
	 ...
	 * not be the case. If it is not, focus traversal will nevertheless proceed as
	 * if focusedComponent were the focus owner.
	 *
	 * @param focusedComponent the Component that is the basis for a focus traversal
	 *                         operation if the specified event represents a focus
	 ...
	 * will be enqueued until the specified Component receives a FOCUS_GAINED event,
	 * or the AWT cancels the delay request by invoking
	 ...
	 *                     if the current event has no timestamp, or the AWT cannot
	 *                     determine which event is currently being handled
	 * @param untilFocused Component which will receive a FOCUS_GAINED event before
	 *                     any pending KeyEvents
...
//        }
	 ...
	 * which were enqueued because of a call to <code>enqueueKeyEvents</code> with
	 * the same timestamp and Component. If the given timestamp is less than zero,
...
//        }
	 ...
	 * <code>enqueueKeyEvents</code> with the specified Component, or one of its
	 * descendants.
	 *
	...
	/**
	 * Moves the focus up one focus traversal cycle. Typically, the focus owner is
	 * set to aComponent's focus cycle root, and the current focus cycle root is set
	 * to the new focus owner's focus cycle root. If, however, aComponent's focus
	 * cycle root is a Window, then the focus owner is set to the focus cycle root's
	 * default Component to focus, and the current focus cycle root is unchanged.
	 *
	 * @param aComponent the Component that is the basis for the focus traversal
	 *                   operation
	 ...
	 * cycle root, then the focus owner is set to aContainer's default Component to
	 * focus, and the current focus cycle root is set to aContainer. If aContainer
	 * is not a focus cycle root, then no focus traversal operation occurs.
	 *
	 * @param aContainer the Container that is the basis for the focus traversal
	 *                   operation
