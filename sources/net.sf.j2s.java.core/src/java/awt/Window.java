...
/**
 * 
 * SWINGJS NOTE! As of 6/23/2018 java.awt.Window has been moved to subclass
 * JComponent. It is fair to say that there will be issues with this, but the
 * reason was to allow JInternalFrame to subclass JFrame instead of duplicating
 * in code all of its functionality.
 * 
...
//    } // static
	...
	/**
	 * This represents the warning message that is to be displayed in a non secure
	 * window. ie : a window that has a security manager installed for which calling
	 * SecurityManager.checkTopLevelWindow() is false. This message can be displayed
	...
	/**
	 * {@code icons} is the graphical way we can represent the frames and dialogs.
	 * {@code Window} can't display icon but it's being inherited by owned
	 * {@code Dialog}s.
	 *
	 * @serial
	 * @see #getIconImages
	 ...
	 * @see #getOwnedWindows
	 */
	transient Vector<Window> ownedWindowList = new Vector<Window>();

	transient boolean showWithParent;

	/**
	 * Contains the modal dialog that blocks this window, or null if the window is
	 * unblocked.
	 *
	 * @since 1.6
	 */
...
//    private transient volatile boolean autoRequestFocus = true;
	...
	/*
	 * Indicates that this window is being shown. This flag is set to true at the
	 * beginning of show() and to false at the end of show().
	 *
	 * @see #show()
	 * 
	...
	/*
	 * The shape assigned to this window. This field is set to null if no shape is
...
//    private static final boolean locationByPlatformProp;
	 ...
	 *
	 * <p>
	 * First, if there is a security manager, its <code>checkTopLevelWindow</code>
	 * method is called with <code>this</code> as its argument to see if it's ok to
	 * display the window without a warning banner. If the default implementation of
	 * <code>checkTopLevelWindow</code> is used (that is, that method is not
	 * overriden), then this results in a call to the security manager's
	 * <code>checkPermission</code> method with an
	 * <code>AWTPermission("showWindowWithoutWarningBanner")</code> permission. It
	 * that method raises a SecurityException, <code>checkTopLevelWindow</code>
	 * returns false, otherwise it returns true. If it returns false, a warning
	 * banner is created.
	 *
	 ...
	 * its argument to determine whether or not the window must be displayed with a
	 * warning banner.
	 *
	 * @param gc the <code>GraphicsConfiguration</code> of the target screen device.
	 *           If <code>gc</code> is <code>null</code>, the system default
	 ...
	 * <code>Frame</code> as its owner. The window will not be focusable unless its
	 ...
	 * warning banner.
	 * 
	 * @param owner the <code>Frame</code> to act as owner or <code>null</code> if
	 ...
	 * warning banner.
	 * 
	 * @param owner the <code>Window</code> to act as owner or <code>null</code> if
	 *              this window has no owner
	 * @exception IllegalArgumentException if the <code>owner</code>'s
	 *                                     <code>GraphicsConfiguration</code> is not
	 *                                     from a screen device
	 ...
	 * <code>Window</code> and a <code>GraphicsConfiguration</code> of a screen
	 * device. The Window will not be focusable unless its nearest owning
	 * <code>Frame</code> or <code>Dialog</code> is showing on the screen.
	 * <p>
	 * If there is a security manager, this method first calls the security
	 * manager's <code>checkTopLevelWindow</code> method with <code>this</code> as
	 * its argument to determine whether or not the window must be displayed with a
	 * warning banner.
	 *
	 * @param owner the window to act as owner or <code>null</code> if this window
	 *              has no owner
	 ...
	 * Subsequent calls to {@code getIconImages} will always return a copy of the
	 * {@code icons} list.
	 * <p>
	 ...
	 * <p>
	 * Ownerless windows with no icon specified use platfrom-default icon. The icon
	 * of an owned window may be inherited from the owner unless explicitly
...
//        }
	...
	/**
	 * Sets the minimum size of this window to a constant value. Subsequent calls to
	 * {@code getMinimumSize} will always return this value. If current window's
	 * size is less than {@code minimumSize} the size of the window is automatically
	 * enlarged to honor the minimum size.
	 * <p>
	 * If the {@code setSize} or {@code setBounds} methods are called afterwards
	 * with a width or height less than that specified by {@code setMinimumSize} the
	 * window is automatically enlarged to honor the {@code minimumSize} value.
	 ...
	 * the {@code minimumSize} value. This behaviour is platform-dependent.
	 *
...
//            }
	 ...
	 * {@inheritDoc}
	 * <p>
	 * The {@code d.width} and {@code d.height} values are automatically enlarged if
...
//        }
	 ...
	 * {@code b}.
	 * 
	 * @param b if {@code true}, makes the {@code Window} visible, otherwise hides
	 *          the {@code Window}. If the {@code Window} and/or its owner are not
	 *          yet displayable, both are made displayable. The {@code Window} will
	 *          be validated prior to being made visible. If the {@code Window} is
	 *          already visible, this will bring the {@code Window} to the front.
	 *          <p>
	 *          If {@code false}, hides this {@code Window}, its subcomponents, and
	 *          all of its owned children. The {@code Window} and its subcomponents
	 *          can be made visible again with a call to {@code #setVisible(true)}.
	...
	public void setVisible(boolean b) {
		...
		if (b) {
			show();
			// BH removed because I added in Component needNotify so that a frame setBounds
			// resize
			// will adjust its size for the root pane.
			// 秘repaint(); // BH SwingJS needs this, because there is no system event set to
			// do this.
		} else {
			hide();
		}
...
//        }
	...
	/**
	 * Releases all of the native screen resources used by this <code>Window</code>,
	 * its subcomponents, and all of its owned children. That is, the resources for
	 * these <code>Component</code>s will be destroyed, any memory they consume will
	 * be returned to the OS, and they will be marked as undisplayable.
	 * <p>
	 * The <code>Window</code> and its subcomponents can be made displayable again
	 * by rebuilding the native resources with a subsequent call to
	 * <code>pack</code> or <code>show</code>. The states of the recreated
	 * <code>Window</code> and its subcomponents will be identical to the states of
	 * these objects at the point where the <code>Window</code> was disposed (not
	 * accounting for additional modifications between those actions).
	 * <p>
	 * <b>Note</b>: When the last displayable window within the Java virtual machine
	 * (VM) is disposed of, the VM may terminate. See
	...
	/*
	 * Fix for 4872170. If dispose() is called on parent then its children have to
	 * be disposed as well as reported in javadoc. So we need to implement this
	 * functionality even if a child overrides dispose() in a wrong way without
...
//		DisposeAction action = new DisposeAction();
	 ...
	 * because parent == owner in Window, and we shouldn't adjust counter on owner
	 */
	@Override
	 ...
	 * visible. Some platforms do not allow Windows which own other Windows to
	 ...
	 * this method will move this Window above all other windows in every situation.
	 * <p>
	 * Because of variations in native windowing systems, no guarantees about
	 * changes to the focused and active Windows can be made. Developers must never
	 * assume that this Window is the focused or active Window until this Window
	 * receives a WINDOW_GAINED_FOCUS or WINDOW_ACTIVATED event. On platforms where
	 * the top-most window is the focused window, this method will <b>probably</b>
	 * focus this Window, if it is not already focused. On platforms where the
	 * stacking order does not typically affect the focused window, this method will
	 * <b>probably</b> leave the focused and active Windows unchanged.
	 * <p>
	 * If this method causes this Window to be focused, and this Window is a Frame
	 * or a Dialog, it will also become activated. If this Window is focused, but it
	 * is not a Frame or a Dialog, then the first Frame or Dialog that is an owner
	 * of this Window will be activated.
	 * <p>
	 ...
	 * any other Windows in this VM. No action will take place is this Window is not
	 * visible. Some platforms do not allow Windows which are owned by other Windows
	 * to appear below their owners. Every attempt will be made to move this Window
	 * as low as possible in the stacking order; however, developers should not
	 * assume that this method will move this Window below all other windows in
	 * every situation.
	 * <p>
	 * Because of variations in native windowing systems, no guarantees about
	 * changes to the focused and active Windows can be made. Developers must never
	 * assume that this Window is no longer the focused or active Window until this
	 * Window receives a WINDOW_LOST_FOCUS or WINDOW_DEACTIVATED event. On platforms
	 * where the top-most window is the focused window, this method will
	 * <b>probably</b> cause this Window to lose focus. In that case, the next
	 * highest, focusable Window in this VM will receive focus. On platforms where
	 * the stacking order does not typically affect the focused window, this method
	 * will <b>probably</b> leave the focused and active Windows unchanged.
	 *
	 * @see #toFront
	 */
	public void toBack() {
...
//       }
	 ...
	 * insecure, the warning string is displayed somewhere in the visible area of
	 * the window. A window is insecure if there is a security manager, and the
	 * security manager's <code>checkTopLevelWindow</code> method returns
	 * <code>false</code> when this window is passed to it as an argument.
	 * <p>
	 * If the window is secure, then <code>getWarningString</code> returns
	 * <code>null</code>. If the window is insecure, this method checks for the
	 * system property <code>awt.appletWarning</code> and returns the string value
	 * of that property.
	 * 
...
//    }
	...
	/**
	 * Gets the <code>Locale</code> object that is associated with this window, if
	 * the locale has been set. If no locale has been set, then the default locale
	 * is returned.
	 * 
...
//    }
	 ...
	 *               If this parameter is null then the cursor for this window will
	 *               be set to the type Cursor.DEFAULT_CURSOR.
	...
	final Window[] getOwnedWindows_NoClientCode() {
		...
		synchronized (ownedWindowList) {
			...
			for (int i = 0; i < fullSize; i++) {
				fullCopy[realSize] = ownedWindowList.elementAt(i);
	...
	/*
	 * Returns a list of all displayable Windows, i. e. all the Windows which peer
	 * is not null.
	 *
	 * @see #addNotify
	 * 
...
//        synchronized (allWindows) {
	...
	private static Window[] getWindows(AppContext appContext) {
		synchronized (Window.class) {
			Window realCopy[];
			Vector<Window> windowList = (Vector<Window>) appContext.get(Window.class);
			if (windowList != null) {
	...
	/**
	 * Returns an array of all {@code Window}s, both owned and ownerless, created by
	 * this application. If called from an applet, the array includes only the
	 * {@code Window}s accessible by that applet.
	 * <p>
	 * <b>Warning:</b> this method may return system created windows, such as a
	 * print dialog. Applications should not assume the existence of these dialogs,
	 * nor should an application assume anything about these dialogs such as
	 * component positions, <code>LayoutManager</code>s or serialization.
	 *
	...
	/**
	 * Returns an array of all {@code Window}s created by this application that have
	 * no owner. They include {@code Frame}s and ownerless {@code Dialog}s and
	 * {@code Window}s. If called from an applet, the array includes only the
	 * {@code Window}s accessible by that applet.
	 * <p>
	 * <b>Warning:</b> this method may return system created windows, such as a
	 * print dialog. Applications should not assume the existence of these dialogs,
	 ...
	 *                      <code>null</code> value is equivivalent to
	 *                      {@link JSDialog.ModalExclusionType#NO_EXCLUDE
	 *                      NO_EXCLUDE}
	 * @throws SecurityException if the calling thread does not have permission to
	 *                           set the modal exclusion property to the window with
	 *                           the given <code>exclusionType</code>
	 * @see java.awt.Dialog.ModalExclusionType
...
//        }
	...
	boolean isModalExcluded(Dialog.ModalExclusionType exclusionType) {
		if ((modalExclusionType != null) && modalExclusionType.ordinal() >= exclusionType.ordinal()) {
			return true;
		}
...
//    }
	 ...
	 * window. If <code>l</code> is <code>null</code>, no exception is thrown and no
	 * action is performed.
	 * <p>
	 * Refer to <a href="doc-files/AWTThreadIssues.html#ListenersThreads" >AWT
	 * Threading Issues</a> for details on AWT's threading model.
	 ...
	 * window events from this window. If <code>l</code> is <code>null</code>, no
	 ...
	 *
	 * @return all of this window's <code>WindowListener</code>s or an empty array
	 *         if no window listeners are currently registered
	 *
	 * @see #addWindowListener
	 * @see #removeWindowListener
	 * @since 1.4
	 ...
	 *
	 * @return all of this window's <code>WindowFocusListener</code>s or an empty
	 ...
	 *
	 * @return all of this window's <code>WindowStateListener</code>s or an empty
	 ...
	 * <code><em>Foo</em>Listener</code>s upon this <code>Window</code>.
	 ...
	 * <code>Window</code> <code>w</code> for its window listeners with the
	 * following code:
	 *
	 * <pre>
	 ...
	 *         <code><em>Foo</em>Listener</code>s on this window, or an empty array
	...
	/**
	 * Processes events on this window. If the event is an <code>WindowEvent</code>,
	 * it invokes the <code>processWindowEvent</code> method, else it invokes its
	 ...
	 * registered WindowListener objects. NOTE: This method will not be called
	 * unless window events are enabled for this component; this happens when one of
	 * the following occurs:
	 * <ul>
	 * <li>A WindowListener object is registered via <code>addWindowListener</code>
	...
	/**
	 * Processes window focus event occuring on this window by dispatching them to
	 * any registered WindowFocusListener objects. NOTE: this method will not be
	 * called unless window focus events are enabled for this window. This happens
	...
	/**
	 * Processes window state event occuring on this window by dispatching them to
	 * any registered <code>WindowStateListener</code> objects. NOTE: this method
	 * will not be called unless window state events are enabled for this window.
	 ...
	 * these windows is unspecified (depends on platform). No window can be brought
	 * to be over the always-on-top window except maybe another always-on-top
	 * window.
	 * <p>
	 * All windows owned by an always-on-top window inherit this state and
	 * automatically become always-on-top. If a window ceases to be always-on-top,
	 * the windows that it owns will no longer be always-on-top. When an
	 * always-on-top window is sent {@link #toBack toBack}, its always-on-top state
	 * is set to <code>false</code>.
	 *
	 * <p>
	 * When this method is called on a window with a value of <code>true</code>, and
	 ...
	 * top-most position. If the window isn`t currently visible, this method sets
	 * the always-on-top state to <code>true</code> but does not bring the window
	 * forward. When the window is later shown, it will be always-on-top.
	 *
	 * <p>
	 * When this method is called on a window with a value of <code>false</code> the
	 * always-on-top state is set to normal. The window remains in the top-most
	 * position but it`s z-order can be changed as for any other window. Calling
	 * this method with a value of <code>false</code> on a window that has a normal
	 * state has no effect. Setting the always-on-top state to false has no effect
	 * on the relative z-order of the windows if there are no other always-on-top
	 * windows.
	 *
	 * <p>
	 * <b>Note</b>: some platforms might not support always-on-top windows. To
	 * detect if always-on-top windows are supported by the current platform, use
	 * {@link Toolkit#isAlwaysOnTopSupported()} and
	 * {@link Window#isAlwaysOnTopSupported()}. If always-on-top mode isn't
	 * supported by the toolkit or for this window, calling this method has no
	 * effect.
	 * <p>
	 ...
	 * @throws SecurityException if the calling thread does not have permission to
	 *                           set the value of always-on-top property
	 * @see #isAlwaysOnTop
	 * @see #toFront
	 * @see #toBack
	 * @see AWTPermission
...
//                }
	 ...
	 * 
	 * @return <code>true</code>, if the always-on-top mode is supported by the
	 *         toolkit and for this window, <code>false</code>, if always-on-top
	 *         mode is not supported for this window or toolkit doesn't support
	 ...
	 * returns the same Component as <code>getFocusOwner()</code>. If this Window is
	 * not focused, then the child Component that most recently requested focus will
	 * be returned. If no child Component has ever requested focus, and this is a
	 * focusable Window, then this Window's initial focusable Component is returned.
	 * If no child Component has ever requested focus, and this is a non-focusable
...
//    	return JSToolkit.isFocused(this);
	 ...
	 * Window's parent's traversal key is returned. If the traversal key has not
	 * been explicitly set for any of this Window's ancestors, then the current
...
//    	return null;
	...
	/**
	 * Always returns <code>true</code> because all Windows must be roots of a focus
	 * traversal cycle.
	 *
	 ...
	 * this Window or any of its subcomponents can become the focus owner. For a
	 * Frame or Dialog to be focusable, its focusable Window state must be set to
	 * <code>true</code>. For a Window which is not a Frame or Dialog to be
	 * focusable, its focusable Window state must be set to <code>true</code>, its
	 * nearest owning Frame or Dialog must be showing on the screen, and it must
	 * contain at least one Component in its focus traversal cycle. If any of these
	 * conditions is not met, then neither this Window nor any of its subcomponents
...
//         traversal cycle to be focusable.
	 ...
	 * other requirements outlined in <code>isFocusableWindow</code>. If this method
	 * returns <code>false</code>, then <code>isFocusableWindow</code> will return
	 * <code>false</code> as well. If this method returns <code>true</code>, then
	 * <code>isFocusableWindow</code> may return <code>true</code> or
	 * <code>false</code> depending upon the other requirements which must be met in
	 * order for a Window to be focusable.
	 * <p>
	 * By default, all Windows have a focusable Window state of <code>true</code>.
	 *
	 ...
	 * requirements outlined in <code>isFocusableWindow</code>. If this Window's
	 * focusable Window state is set to <code>false</code>, then
	 ...
	 * order for a Window to be focusable.
	 * <p>
	 * Setting a Window's focusability state to <code>false</code> is the standard
	 * mechanism for an application to identify to the AWT a Window which will be
	 * used as a floating palette or toolbar, and thus should be a non-focusable
	 * Window.
	 *
	 * Setting the focusability state on a visible <code>Window</code> can have a
	 * delayed effect on some platforms &#151; the actual change may happen only
	 * when the <code>Window</code> becomes hidden and then visible again. To ensure
	 * consistent behavior across platforms, set the <code>Window</code>'s focusable
	 * state when the <code>Window</code> is invisible and then show it.
	 *
...
//                clearGlobalFocusOwner();
	 ...
	 * </ul>
	 * Note that if this Window is inheriting a bound property, then no event will
...
//    }
	...
	Component setTemporaryLostComponent(Component component) {
		...
		// Check that "component" is an acceptable focus owner and don't store it
		// otherwise
...
//        }
	 ...
	 * {@inheritDoc}
	 * <p>
	 * The {@code r.width} or {@code r.height} values will be automatically enlarged
	 * if either is less than the minimum size as specified by previous call to
	 * {@code setMinimumSize}.
	 *
	 * @see #getBounds
	 * @see #setLocation(int, int)
	 ...
	 *         toplevel window are visible, <code>false</code> otherwise
	 */
	@Override
	boolean isRecursivelyVisible() {
		// 5079694 fix: for a toplevel to be displayed, its parent doesn't have to be
		// visible.
...
//            }
	 ...
	 * @throws IllegalArgumentException if the window is in full screen mode, and
...
//     */
	/*
	 * @Override public void setBackground(Color bgColor) { int alpha =
	 * bgColor.getAlpha(); if (alpha < 255) { // non-opaque window
	 * GraphicsConfiguration gc = getGraphicsConfiguration(); GraphicsDevice gd =
	 * gc.getDevice(); if (gc.getDevice().getFullScreenWindow() == this) { throw new
	 * IllegalArgumentException(
	 * "Making full-screen window non opaque is not supported."); } if
	 * (!gc.isTranslucencyCapable()) { GraphicsConfiguration capableGC =
	 * gd.getTranslucencyCapableGC(); if (capableGC == null) { throw new
	 * IllegalArgumentException(
	 * "PERPIXEL_TRANSLUCENT translucency is not supported"); } // TODO: change GC }
	 * setLayersOpaque(this, false); }
	 * 
	 * super.setBackground(bgColor);
	 * 
	 * WindowPeer peer = (WindowPeer)getPeer(); if (peer != null) {
...
//    }
	 ...
	 * <p>
	 * {@code Window} objects are the validate roots, and, therefore, they override
	...
	public void revalidate() {
		// added to see if JFrame can be revalidated this way after a change in
		// contentPane.
