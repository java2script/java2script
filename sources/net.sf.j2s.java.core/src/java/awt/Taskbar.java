...
/**
 * 
 * Not implemented for SwingJS.
 * 
 * Taskbar.getTaskbar() will throw HeadlessException if headless and
 * UnsupportedOperationException if not; Taskbar.isTaskbarSupported() returns
 * false. The enums Taskbar.Feature and Taskbar.State are available.
 * 
 * 
 * The {@code Taskbar} class allows a Java application to interact with the
 * system task area (taskbar, Dock, etc.).
 *
 * <p>
 * There are a variety of interactions depending on the current platform such as
 * displaying progress of some task, appending user-specified menu to the
 * application icon context menu, etc.
 *
 * @implNote Linux support is currently limited to Unity. However to make these
 *           features work on Unity, the app should be run from a .desktop file
 *           with specified {@code java.desktop.appName} system property set to
 *           this .desktop file name:
