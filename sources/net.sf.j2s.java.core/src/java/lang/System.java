/*
 * Note: This class is not used; it was the basis for j2sClazz.js System
...
//    }
	 ...
	 * stream, the value of the variable <code>out</code>, has been redirected to a
...
//		setErr0(err);
	...
	/**
	 * Returns the unique {@link java.io.Console Console} object associated with the
...
//		}
	...
	public static Channel inheritedChannel() throws IOException {
		return null;//SelectorProvider.provider().inheritedChannel();
	}
...
//
	 ...
	 * it's ok to replace the existing security manager. This may result in throwing
	 * a <code>SecurityException</code>.
	 *
	 * <p>
	 * Otherwise, the argument is established as the current security manager. If
	 * the argument is <code>null</code> and no security manager has been
	 * established, then no action is taken and the method simply returns.
	 *
	 * @param s the security manager.
	 * @exception SecurityException if the security manager has already been set and
	 *                              its <code>checkPermission</code> method doesn't
...
//        return security;
	...
	public static /* native */ long currentTimeMillis() {
		/**
		 * @j2sNative return new Date().getTime();
		 */
		{
			return 0;
		}
	...
	public static /* native */ long nanoTime() {
		/**
		 * @j2sNative return Math.round(window.performance.now() * 1e6);
		 */
		{
			return 0;
		}
	 ...
	 * <code>srcPos</code> through <code>srcPos+length-1</code> cannot be converted
	 * to the component type of the destination array by assignment conversion, an
	 * <code>ArrayStoreException</code> is thrown. In this case, let <b><i>k</i></b>
	 * be the smallest nonnegative integer less than length such that
	 * <code>src[srcPos+</code><i>k</i><code>]</code> cannot be converted to the
	 * component type of the destination array; when the exception is thrown, source
	 * array components from positions <code>srcPos</code> through
	 ...
	 *                                      <code>dest</code> array because of a
	 *                                      type mismatch.
	...
	public static /* native */ void arraycopy(Object src, int srcPos, Object dest, int destPos, int length) {
		/**
		 * @j2sNative if (src !== dest || srcPos > destPos) { for (var i = length; --i
		 *            >= 0;) dest[destPos++] = src[srcPos++]; } else { destPos +=
		 *            length; srcPos += length; for (var i = length; --i >= 0;)
	 ...
	 * <p>
	 * First, if there is a security manager, its <code>checkPropertiesAccess</code>
	 * method is called with no arguments. This may result in a security exception.
	 * <p>
	 * The current set of system properties for use by the
	 * {@link #getProperty(String)} method is returned as a <code>Properties</code>
	 ...
	 * property, and the mechanism which implements it, may be removed in a future
	 * release.</i></td>
	 * </tr>
	 * <tr>
...
//
	 ...
	 * same value - the initial value of the {@linkplain #getProperty(String) system
	 * property} {@code line.separator}.
	 *
	 * <p>
...
//		}
	 ...
	 *                                     doesn't allow access to the specified
	 *                                     system property.
...
//
	 ...
	 * <p>
	 * First, if there is a security manager, its <code>checkPropertyAccess</code>
	 * method is called with the <code>key</code> as its argument.
	 * <p>
	 * If there is no current set of system properties, a set of system properties
	 * is first created and initialized in the same manner as for the
	 * <code>getProperties</code> method.
...
//
	 ...
	 * <code>PropertyPermission(key, "write")</code> permission. This may result in
	 * a SecurityException being thrown. If no exception is thrown, the specified
	 * property is set to the given value.
	 * <p>
	 *
	 * @param key   the name of the system property.
	 * @param value the value of the system property.
	 * @return the previous value of the system property, or <code>null</code> if it
	 *         did not have one.
	 *
	 * @exception SecurityException        if a security manager exists and its
	 *                                     <code>checkPermission</code> method
	 *                                     doesn't allow setting of the specified
	 *                                     property.
	 * @exception NullPointerException     if <code>key</code> or <code>value</code>
	 *                                     is <code>null</code>.
	 * @exception IllegalArgumentException if <code>key</code> is empty.
	 * @see #getProperty
...
//		}
//
		return (String) getProps().remove(key);
	}
	 ...
	 * <code>{@link RuntimePermission}("getenv."+name)</code> permission. This may
	 * result in a {@link SecurityException} being thrown. If no exception is thrown
	 * the value of the variable <code>name</code> is returned.
	 *
	 * <p>
	 * <a name="EnvironmentVSSystemProperties"><i>System properties</i> and
	 * <i>environment variables</i></a> are both conceptually mappings between names
	 * and values. Both mechanisms can be used to pass user-defined information to a
	 * Java process. Environment variables have a more global effect, because they
	 * are visible to all descendants of the process which defines them, not just
	 * the immediate Java subprocess. They can have subtly different semantics, such
	 * as case insensitivity, on different operating systems. For these reasons,
	 * environment variables are more likely to have unintended side effects. It is
	 * best to use system properties where possible. Environment variables should be
	 * used when a global effect is desired, or when an external system interface
	 ...
	 * <code>System.getenv("FOO").equals(System.getenv("foo"))</code> is likely to
	 * be true on Microsoft Windows.
	 *
	 ...
	 *                              checkPermission} method doesn't allow access to
	 *                              the environment variable <code>name</code>
	 * @see #getenv()
...
//		return ProcessEnvironment.getenv(name);
	 ...
	 * {@link NullPointerException}. Attempting to query the presence of a key or
	 * value which is not of type {@link String} will throw a
	 * {@link ClassCastException}.
	 *
	 * <p>
	 * The returned map and its collection views may not obey the general contract
	 * of the {@link Object#equals} and {@link Object#hashCode} methods.
	 *
	 * <p>
	 ...
	 * <a href=#EnvironmentVSSystemProperties>system properties</a> are generally
	 * preferred over environment variables.
	 *
	 * @return the environment as a map of variable names to values
	 * @throws SecurityException if a security manager exists and its
	 *                           {@link SecurityManager#checkPermission
	 *                           checkPermission} method doesn't allow access to the
	 *                           process environment
	 * @see #getenv(String)
	 * @see ProcessBuilder#environment()
	 * @since 1.5
...
//		return ProcessEnvironment.getenv();
	 ...
	 * termination.
	 * <p>
	 * This method calls the <code>exit</code> method in class <code>Runtime</code>.
	 * This method never returns normally.
	 * <p>
	 * The call <code>System.exit(n)</code> is effectively equivalent to the call:
	 * <blockquote>
	 * 
	 * <pre>
	 ...
	 * @throws SecurityException if a security manager exists and its
	 *                           <code>checkExit</code> method doesn't allow exit
	 ...
	 *
	 * <p>
	 * If there is a security manager, its <code>checkExit</code> method is first
	 * called with 0 as its argument to ensure the exit is allowed. This could
	 ...
	 * @throws SecurityException if a security manager exists and its
	 *                           <code>checkExit</code> method doesn't allow the
	 *                           exit.
	 *
	 * @see java.lang.Runtime#exit(int)
	 * @see java.lang.Runtime#gc()
	 ...
	 * example, L, and a native library called L is statically linked with the VM,
	 * then the JNI_OnLoad_L function exported by the library is invoked rather than
	 * attempting to load a dynamic library. A filename matching the argument does
	 * not have to exist in the file system. See the JNI Specification for more
	 * details.
	 *
	 ...
	 * @exception UnsatisfiedLinkError if either the filename is not an absolute
	 *                                 path name, the native library is not
	 *                                 statically linked with the VM, or the library
	 *                                 cannot be mapped to a native library image by
	 *                                 the host system.
...
//		Runtime.getRuntime().load0(Reflection.getCallerClass(), filename);
	...
	/**
	 * Loads the native library specified by the <code>libname</code> argument. The
	 * <code>libname</code> argument must not contain any platform specific prefix,
	 * file extension or path. If a native library called <code>libname</code> is
	 * statically linked with the VM, then the JNI_OnLoad_<code>libname</code>
	 * function exported by the library is invoked. See the JNI Specification for
	 * more details.
	 *
	 ...
	 * <p>
	 * The call <code>System.loadLibrary(name)</code> is effectively equivalent to
	 * the call <blockquote>
	 * 
	 * <pre>
	 ...
	 *                                 <code>checkLink</code> method doesn't allow
	 *                                 loading of the specified dynamic library
	 * @exception UnsatisfiedLinkError if either the libname argument contains a
	 *                                 file path, the native library is not
	 *                                 statically linked with the VM, or the library
	 *                                 cannot be mapped to a native library image by
	 *                                 the host system.
