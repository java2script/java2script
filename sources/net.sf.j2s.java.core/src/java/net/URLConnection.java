 ...
 * <p>
 * <center>
 * <table border=2 summary="Describes the process of creating a connection to a
 * URL: openConnection() and connect() over time.">
 * <tr>
 * <th><code>openConnection()</code></th>
 * <th><code>connect()</code></th>
 * </tr>
 * <tr>
 * <td>Manipulate parameters that affect the connection to the remote
 * resource.</td>
 ...
 * access it. This change is also described on the
 * <a href= "http://java.sun.com/products/jdk/1.2/compatibility.html">
...
public abstract class URLConnection //
		extends ContentHandler // SwingJS
{
	 ...
	 * <code>doInput</code> flag to <code>true</code> indicates that the application
	...
	/**
	 * Returns the value of this <code>URLConnection</code>'s <code>doOutput</code>
	 * flag.
	 * 
	 * @return the value of this <code>URLConnection</code>'s <code>doOutput</code>
	 *         flag.
	 ...
	 * <p>
	 * If the <code>connect</code> method is called when the connection has already
	 * been opened (indicated by the <code>connected</code> field having the value
	 * <code>true</code>), the call is ignored.
	 * <p>
	 * URLConnection objects go through two phases: first they are created, then
	 * they are connected. After being created, and before being connected, various
	 * options can be specified (e.g., doInput and UseCaches). After connecting, it
	 * is an error to try to set them. Operations that depend on being connected,
	 * like getContentLength, will implicitly perform the connection, if necessary.
	 * 
	 * @throws SocketTimeoutException if the timeout expires before the connection
	 *                                can be established
	...
	/**
	 * Set the value of the <code>allowUserInteraction</code> field of this
	...
	/**
	 * Returns the value of the <code>allowUserInteraction</code> field for this
	 * object.
	 *
	 * @return the value of the <code>allowUserInteraction</code> field for this
	 *         object.
	...
	/**
	 * Sets the default value of the <code>allowUserInteraction</code> field for all
	 * future <code>URLConnection</code> objects to the specified value.
	 *
	...
	/**
	 * Returns the value of this <code>URLConnection</code>'s <code>useCaches</code>
	 * field.
	 *
	 * @return the value of this <code>URLConnection</code>'s <code>useCaches</code>
	 *         field.
	...
	/**
	 * Sets the value of the <code>ifModifiedSince</code> field of this
	 * <code>URLConnection</code> to the specified value.
	 *
	...
	/**
	 * Sets the default value of the <code>useCaches</code> field to the specified
	 * value.
	 *
	 ...
	 * <code>URLConnection</code> is created, it is initialized with these
	 * properties.
	 *
	 * @param key   the keyword by which the request is known (e.g.,
	 *              "<code>accept</code>").
	 ...
	 *
	 * @deprecated The instance specific setRequestProperty method should be used
	 *             after an appropriate instance of URLConnection is obtained.
	 ...
	 *
	 * @deprecated The instance specific getRequestProperty method should be used
	 *             after an appropriate instance of URLConnection is obtained.
	 *
	...
	/**
	 * Sets the <code>ContentHandlerFactory</code> of an application. It can be
	 * called at most once by an application.
	 * <p>
	 * The <code>ContentHandlerFactory</code> instance is used to construct a
	 * content handler from a content type
	 * <p>
...
//			}
		if (handlers == null)
			handlers = new Hashtable<String, ContentHandler>();
		handlers.put(contentType, handler);
		return handler;
	}

	/*
	 * Media types are in the format: type/subtype*(; parameter). For looking up the
	 * content handler, we should ignore those parameters.
	 */
...
//	 * looks in sun.net.www.content, but users can define a vertical-bar delimited
//	 * set of class prefixes to search through in addition by defining the
...
//
//	/**
//	 * Utility function to map a MIME content type into an equivalent pair of class
//	 * name components. For example: "text/html" would be returned as "text.html"
//	 */
...
// }
	 ...
	 * "file" component of a URL. This is a convenience method that can be used by
	 * subclasses that override the <code>getContentType</code> method.
	 *
	 * @param fname a filename.
	 * @return a guess as to what the content type of the object is, based upon its
	 *         file name.
	 * @see java.net.URLConnection#getContentType()
	 */
...
// return getFileNameMap().getContentTypeFor(fname);
	 ...
	 * override the <code>getContentType</code> method.
	 * <p>
	 * Ideally, this routine would not be needed. But many <code>http</code> servers
	 * return the incorrect content type; in addition, there are many nonstandard
	 * extensions. Direct inspection of the bytes to determine the content type is
	 * often more accurate than believing the content type claimed by the
	 * <code>http</code> server.
	 *
	 * @param is an input stream that supports marks.
	 * @return a guess at the content type, or <code>null</code> if none can be
	 *         determined.
	 * @exception IOException if an I/O error occurs while reading the input stream.
	...
	static public String guessContentTypeFromStream(InputStream is) throws IOException {
		...
		if (c1 == 0xFF && c2 == 0xD8 && c3 == 0xFF) {
			...
			/**
			 * File format used by digital cameras to store images. Exif Format can be read
			 * by any application supporting JPEG. Exif Spec can be found at:
...
// }
		...
		if (c1 == 'R' && c2 == 'I' && c3 == 'F' && c4 == 'F') {
			/*
			 * I don't know if this is official but evidence suggests that .wav files start
			 * with "RIFF" - brown
			 */
			return "audio/x-wav";
		}
...
//
	...
	/**
	 * If <code>true</code>, this <code>URL</code> is being examined in a context in
	 * which it makes sense to allow user interactions such as popping up an
	 * authentication dialog. If <code>false</code>, then no user interaction is
	 * allowed.
	 * <p>
	 * The value of this field can be set by the
	 * <code>setAllowUserInteraction</code> method. Its value is returned by the
	 * <code>getAllowUserInteraction</code> method. Its default value is the value
	 * of the argument in the last invocation of the
	 ...
	 * If <code>false</code>, the protocol must always try to get a fresh copy of
	 * the object.
	 * <p>
	 * This field is set by the <code>setUseCaches</code> method. Its value is
	 * returned by the <code>getUseCaches</code> method.
	 * <p>
	 ...
	 * than that time.
	 * <p>
	 * This variable is set by the <code>setIfModifiedSince</code> method. Its value
	 * is returned by the <code>getIfModifiedSince</code> method.
	 * <p>
	 * The default value of this field is <code>0</code>, indicating that the
...
// private MessageHeader requests;
	 ...
	 * the user-specific table, defined by &quot;content.types.user.table&quot;
	 ...
	 *         references, or <code>-1</code> if the content length is not known.
	 */
	 ...
	 *         <code>0</code> if not known. The value returned is the number of
	...
	/**
	 * Returns the value of the <code>last-modified</code> header field. The result
	 * is the number of milliseconds since January 1, 1970 GMT.
	 *
	 * @return the date the resource referenced by this <code>URLConnection</code>
	 ...
	 * @return the value of the named header field, or <code>null</code> if there is
	 ...
	 * <p>
	 * This form of <code>getHeaderField</code> exists because some connection types
	 * (e.g., <code>http-ng</code>) have pre-parsed headers. Classes for that
	 * connection type can override this method and short-circuit the parsing.
	 *
	 * @param name    the name of the header field.
	 * @param Default the default value.
	 * @return the value of the named field, parsed as an integer. The
	 *         <code>Default</code> value is returned if the field is missing or
	 *         malformed.
	 */
	 ...
	 *         <code>Default</code> argument is returned if the field is missing or
	 *         malformed.
	 */
	...
	/**
	 * Returns the key for the <code>n</code><sup>th</sup> header field. It returns
	 * <code>null</code> if there are fewer than <code>n+1</code> fields.
	 *
	 * @param n an index, where n>=0
	 * @return the key for the <code>n</code><sup>th</sup> header field, or
	 *         <code>null</code> if there are fewer than <code>n+1</code> fields.
	 */
	...
	/**
	 * Returns the value for the <code>n</code><sup>th</sup> header field. It
	 * returns <code>null</code> if there are fewer than <code>n+1</code>fields.
	 * <p>
	 ...
	 *
	 * @param n an index, where n>=0
	 * @return the value of the <code>n</code><sup>th</sup> header field or
	 *         <code>null</code> if there are fewer than <code>n+1</code> fields
	 ...
	 * <code>getContentType</code> method. If this is the first time that the
	 * application has seen that specific content type, a content handler for that
	 * content type is created:
	 * <ol>
	 * <li>If the application has set up a content handler factory instance using
	 * the <code>setContentHandlerFactory</code> method, the
	 * <code>createContentHandler</code> method of that instance is called with the
	 * content type as an argument; the result is a content handler for that content
	 * type.
	 * <li>If no content handler factory has yet been set up, or if the factory's
	 * <code>createContentHandler</code> method returns <code>null</code>, then the
	 * application loads the class named: <blockquote>
	 * 
	 * <pre>
	 ...
	 * <code>period</code> ('.'), and all other non-alphanumeric characters with the
	 * underscore character '<code>_</code>'. The alphanumeric characters are
	 * specifically the 26 uppercase ASCII letters '<code>A</code>' through
	 * '<code>Z</code>', the 26 lowercase ASCII letters '<code>a</code>' through
	 * '<code>z</code>', and the 10 ASCII digits '<code>0</code>' through
	 * '<code>9</code>'. If the specified class does not exist, or is not a subclass
	 * of <code>ContentHandler</code>, then an <code>UnknownServiceException</code>
	 * is thrown.
	 * </ol>
	 *
	 * @return the object fetched. The <code>instanceof</code> operator should be
	 ...
	 * @return the object fetched that is the first match of the type specified in
	 *         the classes array. null if none of the requested types are supported.
	 *         The <code>instanceof</code> operator should be used to determine the
	 ...
	 * returns <code>java.security.AllPermission</code>. Subclasses should override
	 ...
	 * <code>URLConnection</code> representing a <code>file:</code> URL would return
	 * a <code>java.io.FilePermission</code> object.
	 *
	 * <p>
	 ...
	 * obtained through URLConnections, and to check the right of a recipient to
	 * learn about a particular URL. In the first case, the permission should be
	 * obtained <em>after</em> the object has been obtained. For example, in an HTTP
	 * connection, this will represent the permission to connect to the host from
	 * which the data was ultimately fetched. In the second case, the permission
	 * should be obtained and tested <em>before</em> connecting.
	 *
	 * @return the permission object representing the permission necessary to make
	 *         the connection represented by this URLConnection.
	 *
	 * @exception IOException if the computation of the permission requires network
	 *                        or file I/O and an exception occurs while computing
	 *                        it.
	 */
