...
public class FieldPosition {
	...
	/**
	 * Output: End offset of field in text. If the field does not occur in the text,
	 * 0 is returned.
	 */
	int endIndex = 0;
	...
	/**
	 * Creates a FieldPosition object for the given field. Fields are identified by
	 * constants, whose names typically end with _FIELD, in the various subclasses
	 * of Format.
	 *
	...
	/**
	 * Creates a <code>FieldPosition</code> object for the given field. The field is
	 * identified by an attribute constant from one of the <code>Field</code>
	 * subclasses as well as an integer field ID defined by the <code>Format</code>
	 * subclasses. <code>Format</code> subclasses that are aware of
	 * <code>Field</code> should give precedence to <code>attribute</code> and
	 * ignore <code>fieldID</code> if <code>attribute</code> is not null. However,
	 * older <code>Format</code> subclasses may not be aware of <code>Field</code>
	 * and rely on <code>fieldID</code>. If the field has no corresponding integer
	 ...
	 * <code>Field</code> subclasses. May return null if the field is specified only
	 * by an integer field ID.
	 *
	...
	/**
	 * Returns a <code>Format.FieldDelegate</code> instance that is associated with
	 * the FieldPosition. When the delegate is notified of the same field the
	 * FieldPosition is associated with, the begin/end will be adjusted.
	 */
	...
	/**
	 * Return true if the receiver wants a <code>Format.Field</code> value and
	 * <code>attribute</code> is equal to it, or true if the receiver represents an
	 * inteter constant and <code>field</code> equals it.
	 */
	...
	private class Delegate implements Format.FieldDelegate {
		/**
		 * Indicates whether the field has been encountered before. If this is true, and
		 * <code>formatted</code> is invoked, the begin/end are not updated.
		 */
		...
		public void formatted(int fieldID, Format.Field attr, Object value, int start, int end, String[] buffer) {
			if (!encounteredField && matchesField(attr, fieldID)) {
				setBeginIndex(start);
				setEndIndex(end);
				encounteredField = (start != end);
		...
		public void formatted(Format.Field attr, Object value, int start, int end, String[] buffer) {
			if (!encounteredField && matchesField(attr)) {
				setBeginIndex(start);
				setEndIndex(end);
				encounteredField = (start != end);
