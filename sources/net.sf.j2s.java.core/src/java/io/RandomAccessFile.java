...
/**
 * Note - SwingJS replaces a large number of native calls in this class
 * with calls to JSFileSystem.JS
...
//        }
    ...
    public final FileChannel getChannel() throws IOException {
        synchronized (this) {
            if (channel == null) {
            	channel = JSFileChannel.open(fd, path, true, rw, this);
            }
            return channel;
        }
    }
    ...
    public int read(byte b[], int off, int len) throws IOException {
    	if (len > 0)
        	hasRead = true;
        len = channel.readBytes(b, off, len);
    	if (len > 0)
        	hasRead = true;
        return len;
    }
    ...
    private void accessDenied() throws IOException {
	...
	public void close() throws IOException {
        ...
        if (channel != null) {
            if (rw && hasWritten && channel.size() > 0)
            	channel.doSave();
