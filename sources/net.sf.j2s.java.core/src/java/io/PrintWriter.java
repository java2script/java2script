...
public class PrintWriter extends Writer {
	 ...
	 *          the file exists then it will be truncated to zero size; otherwise,
	 *          a new file will be created. The output will be written to the file
	 *          and is buffered.
	 * 
	 ...
	 * written in exactly the manner of the <code>{@link
	 ...
	 * java.lang.String#valueOf(float)}</code> is translated into bytes according
	 * to the platform's default character encoding, and these bytes are written
	 * in exactly the manner of the <code>{@link #write(int)}</code> method.
	 * 
	 * @param f
	...
	/**
	 * Prints a boolean value and then terminates the line. This method behaves as
	 ...
	 *          maximum dimension of a Java array as defined by the <a
	 *          href="http://java.sun.com/docs/books/vmspec/">Java Virtual Machine
	 *          Specification</a>. The behaviour on a <tt>null</tt> argument
	 *          depends on the <a
	 ...
	 *          formatting. If <tt>l</tt> is <tt>null</tt> then no localization is
	 *          applied.
	 * 
	 * @param format
	 *          A format string as described in <a
	...
	private Formatter newFormatter() {
		return (Formatter) Interface.getInstanceWithParams("java.util.Formatter",
				new Class[] { Appendable.class }, new Object[] { this });
	}
	 ...
	 * @return
	 */
	public PrintWriter append(String s, int start, int end) {
		write(s == null ? "null" : s.substring(start, end));
		return this;
	}
