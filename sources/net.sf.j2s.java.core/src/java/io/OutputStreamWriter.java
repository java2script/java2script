 ...
 * {@link java.nio.charset.Charset <code>charset</code>}. The charset that it
 * uses may be specified by name or may be given explicitly, or the platform's
...
public class OutputStreamWriter extends Writer {
	...
	private void checkName(String charsetName) throws UnsupportedEncodingException {
		if (charsetName == null)
			charsetName = "UTF-8";
	...
	public OutputStreamWriter(OutputStream out, Charset cs) {
		...
		try {
			...
			System.err.println("OutputStreamWriter: " + cs.name() + " not supported (only UTF-8 in SwingJS)");
		}
	}
...
//        se = StreamEncoder.forOutputStreamWriter(out, this, enc);
	 ...
	 * returned name, being unique for the encoding, may differ from the name passed
	 * to the constructor. This method may return <tt>null</tt> if the stream has
	 * been closed.
	 * </p>
	 *
	 * @return The historical name of this encoding, or possibly <code>null</code>
...
//        return se.getEncoding();
	...
	/**
	 * Flushes the output buffer to the underlying byte stream, without flushing the
	 * byte stream itself. This method is non-private only so that it may be invoked
	 * by PrintStream.
	 */
	void flushBuffer() throws IOException {
		/* se. */flush(); // just flush it.
