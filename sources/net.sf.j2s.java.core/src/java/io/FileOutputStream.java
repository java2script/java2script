/*
f * Copyright (c) 1994, 2013, Oracle and/or its affiliates. All rights reserved.
...
//	protected OC out;
	...
	/**
	 * The path of the referenced file (null if the stream is created with a file
	 * descriptor)
	 */
	 ...
	 * the second argument is <code>true</code>, then bytes will be written to the
	 * end of the file rather than the beginning. A new <code>FileDescriptor</code>
	 ...
	 * specified <code>File</code> object. A new <code>FileDescriptor</code> object
	 ...
	 * <code>true</code>, then bytes will be written to the end of the file rather
	 * than the beginning. A new <code>FileDescriptor</code> object is created to
...
//        }
	 ...
	 * {@link java.io.FileDescriptor#valid() invalid}. However, if the methods are
	 * invoked on the resulting stream to attempt I/O on the stream, an
...
// SwingJS old:		this(new File("output"), false);
	 ...
	 *
	 * @return the <code>FileDescriptor</code> object that represents the connection
	 *         to the file in the file system being used by this
	 ...
	 *
	 * <p>
	 * The initial {@link java.nio.channels.FileChannel#position() position} of the
	 * returned channel will be equal to the number of bytes written to the file so
	 * far unless this stream is in append mode, in which case it will be equal to
	 * the size of the file. Writing bytes to this stream will increment the
	 * channel's position accordingly. Changing the channel's position, either
	 * explicitly or by writing, will change this stream's file position.
	 *
	...
	protected void finalize() throws IOException {
		if (fd != null) {
			if (fd == FileDescriptor.out || fd == FileDescriptor.err) {
				...
				/*
				 * if fd is shared, the references in FileDescriptor will ensure that finalizer
				 * is only called when safe to do so. All references using the fd have become
				 * unreachable. We can call close()
				 */
				close();
			}
		}
