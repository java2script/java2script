...
//	private volatile boolean readPrimordialConfiguration;
	...
	private boolean initializedGlobalHandlers = false;
	// True if JVM death is imminent and the exit hook has been called.
	private boolean deathImminent;

	static {
//				AccessController.doPrivileged(new PrivilegedAction<LogManager>() {
//			@Override
...
//		});
	...
	private class Cleaner extends Thread {
		...
		private Cleaner() {
			/*
			 * Set context class loader to null in order to avoid keeping a strong reference
			 * to an application classloader.
			 */
...
//			}
	 ...
	 * as J2EE containers) can subclass the object. It is non-public as it is
	 * intended that there only be one LogManager object, whose value is retrieved
...
//		}
	...
	/**
	 * Lazy initialization: if this instance of manager is the global manager then
	 * this method will read the initial configuration and add the root logger and
	 * global logger by calling addLogger().
	 *
	 * Note that it is subtly different from what we do in LoggerContext. In
	 * LoggerContext we're patching up the logger context tree in order to add the
	 * root and global logger *to the context tree*.
	 *
	 * For this to work, addLogger() must have already have been called once on the
	 * LogManager instance for the default logger being added.
	 *
...
//	}
	 ...
	 *
	 * <p>
	 * <b>WARNING:</b> This method is omitted from this class in all subset Profiles
	 * of Java SE that do not include the {@code java.beans} package.
	 * </p>
	 *
	 ...
	 *             platform. This method will be removed in a future release. The
	 *             global {@code LogManager} can detect changes to the logging
	 *             configuration by overridding the {@link #readConfiguration
	 ...
	 * <CODE>addPropertyChangeListener</CODE>, then an equivalent number of
...
// n/a in SwingJS	private final ReferenceQueue<Logger> loggerRefQueue = new ReferenceQueue<>();
	...
	final class LoggerWeakRef {
		...
		private Logger parentRef; // for kids cleanup
		...
		void setParentRef(Logger parentRef) {
		...
		public Logger get() {
			...
			if (ref == null)
				try {
					ref = (Logger) Class.forName(name).newInstance();
					isValid = true;
...
//    }
	 ...
	 * @return true if the argument logger was registered successfully, false if a
	 *         logger of that name already exists.
...
//		});
	 ...
	 * logging. It is also important to note that the Logger associated with the
	 * String {@code name} may be garbage collected at any time if there is no
	 * strong reference to the Logger. The caller of this method must check the
	 * return value for null in order to properly handle the case where the Logger
	 ...
	 * only reports on the loggers that are currently registered. It is also
	 * important to note that this method only returns the name of a Logger, not a
	 * strong reference to the Logger itself. The returned String does nothing to
	 * prevent the Logger from being garbage collected. In particular, if the
	 * returned name is passed to {@code LogManager.getLogger()}, then the caller
	 * must check the return value from {@code LogManager.getLogger()} for null to
	 * properly handle the case where the Logger has been garbage collected in the
	 ...
	 * @exception SecurityException if a security manager exists and if the caller
	 *                              does not have LoggingPermission("control").
	 * @exception IOException       if there are IO problems reading the
	 *                              configuration.
	 */
...
//		}
	...
	/**
	 * Get the value of a logging property. The method returns null if the property
	 * is not found.
	 * 
...
//			sm.checkPermission(controlPermission);
	...
	/**
	 * Returns <tt>LoggingMXBean</tt> for managing loggers. An alternative way to
	 * manage loggers is through the
	 ...
	 * java.beans. This class can be removed once the addPropertyChangeListener and
	 * removePropertyChangeListener methods are removed.
	 */
	private static class Beans {
		...
		/**
		 * Returns a new PropertyChangeEvent with the given source, property name, old
		 * and new values.
		 */
		...
		/**
		 * Invokes the given PropertyChangeListener's propertyChange method with the
		 * given event.
		 */
