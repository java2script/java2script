 ...
 * SwingJS:
 * 
 * No BigDecimal, BigInteger
 * No precision with toHex for double
 * No locales 
 * Q: What is "General" number conversion?
 */
package java.util;
 ...
 * <tt>byte</tt>, {BigDecimal BigDecimal}, and {@link Calendar}
 ...
 * Integer}, <tt>long</tt>, {@link Long}, and {BigInteger
 * BigInteger}
 *
 * <li><b>Floating Point</b> - may be applied to Java floating-point types:
 * <tt>float</tt>, {@link Float}, <tt>double</tt>, {@link Double}, and {
 ...
 * <tt>'X'</tt> conversions applied to {BigInteger BigInteger}
 ...
 *     using the {BigDecimal#ROUND_HALF_UP round half up
 ...
 *
 * <p> The following conversions may be applied {BigDecimal
 * BigDecimal}.
 *
 * <table cellpadding=5 summary="floatConv">
 *
...
public final class Formatter implements Flushable {
    ...
    private FormatString[] parse(String s) {
        ...
        // in JavaScript we must recompile each time!
...
//          System.out.println(((FormatString) al.get(j)).toString());
    ...
    private interface FormatString {
        int index();
        void printOL(Object arg, Locale l) throws IOException;
        @Override
				String toString();
    }
    ...
    private class FixedString implements FormatString {
        ...
        @Override
				public void printOL(Object arg, Locale l)
    ...
    private class FormatSpecifier implements FormatString {
        ...
        @Override
				public void printOL(Object arg, Locale l) throws IOException {
            if (dt) {
                printDateTime(arg, l);
                return;
            }
        ...
        private void printInteger(Object arg, Locale l) throws IOException {
            if (arg == null)
                ...
                printB(((Byte)arg).byteValue(), l);
            else if (arg instanceof Short)
                printSh(((Short)arg).shortValue(), l);
            else if (arg instanceof Integer)
                printI(((Integer)arg).intValue(), l);
            else if (arg instanceof Long)
                printL(((Long)arg).longValue(), l);
...
//                print(((BigInteger)arg), l);
        ...
        private void printFloat(Object arg, Locale l) throws IOException {
            if (arg == null)
                ...
                printF(((Float)arg).floatValue(), l);
            else if (arg instanceof Double)
                printDL(((Double)arg).doubleValue(), l);
...
//            } else 
        ...
        private void printS(String s) throws IOException {
        ...
        private void printB(byte value, Locale l) throws IOException {
            long v = value;
            if (value < 0
        ...
        private void printSh(short value, Locale l) throws IOException {
            long v = value;
            if (value < 0
                && (c == Conversion.OCTAL_INTEGER
                    || c == Conversion.HEXADECIMAL_INTEGER)) {
        ...
        private void printI(int value, Locale l) throws IOException {
            long v = value;
            if (value < 0
                && (c == Conversion.OCTAL_INTEGER
                    || c == Conversion.HEXADECIMAL_INTEGER)) {
        ...
        private void printL(long value, Locale l) throws IOException {
...
//        }
        ...
        private void printF(float value, Locale l) throws IOException {
            printDL((double) value, l);
        }

        private void printDL(double value, Locale l) throws IOException {
            StringBuilder sb = new StringBuilder();
            ...
            if (!Double.isNaN(value)) {
                ...
                if (!Double.isInfinite(v))
                    printD(sb, v, l, f, c, precision, neg);
                else
                    sb.append(f.contains(Flags.UPPERCASE)
                              ? "INFINITY" : "Infinity");
        ...
        private void printD(StringBuilder sb, double value, Locale l,
            ...
            if (c == Conversion.SCIENTIFIC) {
                ...
                FormattedFloatingDecimal fd
                    = new FormattedFloatingDecimal(value, prec,
                        FormattedFloatingDecimal.SCIENTIFIC);
               ...
               int len = fd.getChars(v);
                ...
                FormattedFloatingDecimal fd
                    = new FormattedFloatingDecimal(value, prec,
                        FormattedFloatingDecimal.DECIMAL_FLOAT);
                ...
                FormattedFloatingDecimal fd
                    = new FormattedFloatingDecimal(value, prec,
                        FormattedFloatingDecimal.GENERAL);
...
//        }
        ...
        private Appendable printDTL(StringBuilder sb, Calendar t, char c,
                                 Locale l)
            throws IOException
        {
            assert(width == -1);
...
//                break;
            ...
            case DateTime.TIME_24_HOUR:    { // 'R' (hh:mm same as %H:%M)
                ...
                if (c == DateTime.TIME) {
                    sb.append(sep);
                    printDTL(sb, t, DateTime.SECOND, l);
                }
                break;
            }
            ...
            case DateTime.DATE_TIME:    { // 'c' (Sat Nov 04 12:02:33 EST 1999)
                ...
                printDTL(sb, t, DateTime.TIME, l).append(sep);
                printDTL(sb, t, DateTime.ZONE, l).append(sep);
                printDTL(sb, t, DateTime.YEAR_4, l);
                break;
            }
    ...
    private static class Flags {
        ...
        private static Flags parseChar(char c) {
            switch (c) {
...
// *        case LOCALE_DATE:
    ...
    /**
     * 
     * A class to emulate sun.misc.FormattedFloatingDecimal
     * 
     * 
     * 
     * @author Bob Hanson
	...
	//	output from sun.misc.FormattedFloatingDecimal:
	//	
	//	""+x                               g             f                  e                (e;prec-1)
	//	
	//	0.15000000000000002 prec=3 exp=-1  0.150         0.150              1.500e-01         1.50e-01
	//	1.5E-10             prec=3 exp=-10 1.5e-10       0.000              1.5e-10           1.5e-10
	//	0.015               prec=3 exp=-2  0.015         0.015              1.5e-02           1.5e-02
	//	1.23456789356789E-4 prec=3 exp=-4  0.000123      0.000              1.235e-04         1.23e-04
	//	1.23456789356789E-5 prec=3 exp=-5  1.23e-05      0.000              1.235e-05         1.23e-05
	//	12.345678935678901  prec=4 exp=1   12.35         12.3457            1.2346e+01        1.235e+01
	//	1234.56789356789    prec=3 exp=3   1.23e+03      1234.568           1.235e+03         1.23e+03
	//	1234.56789356789    prec=4 exp=3   1235          1234.5679          1.2346e+03        1.235e+03
	//	1234567.89356789    prec=3 exp=6   1.23e+06      1234567.894        1.235e+06         1.23e+06
	//	1.0                 prec=3 exp=0   1             1                  1e+00             1e+00
	//	150.0               prec=5 exp=2   150           150                1.5e+02           1.5e+02
	//	1500000.0           prec=10 exp=6  1500000       1500000            1.5e+06           1.5e+06
	//	1.23456789356789E8  prec=8 exp=8   1.2345679e+08 123456789.356789   1.23456789e+08    1.2345679e+08
	//	1.23456789356789E8  prec=9 exp=8   123456789     123456789.356789   1.234567894e+08   1.23456789e+08
	//	1003456.7891234001  prec=11 exp=6  1003456.7891  1003456.7891234001 1.00345678912e+06 1.0034567891e+06
	//	
	//	notes:
	//	
	//	General: is sci(x, prec-1), but if (exp >= -4 and exp < prec), then sci(x, prec-1) written as decimal
	//	
	//	Note, however, that in both Java and JavaScript, 1.5 * Math.pow(10,-1) is 0.15000000000000002, not 0.15,
	//	while 1.5 * Math.pow(10,-10) is 1.5e-10, so the number of "significant" digits is unreliable.
		
		public static final int GENERAL = 0;
		public static final int DECIMAL_FLOAT = 1;
		public static final int SCIENTIFIC = 2;
		private String str;
		private int exp, expr;
		...
		private void getString(int type, double value, int prec) {
			...
			// for "1.2e-5" vs 1.200e-5" we take the first
			// for "1.2" vs 1.200" we take the first
			if (sNoRound.length() < sRound.length())
				sRound = sNoRound;
			int pt = sRound.indexOf("e"); // f
			switch (type) {
			case SCIENTIFIC:
		...
		public int getChars(char[] v) {
			int len = str.length();
			System.arraycopy(str.toCharArray(), 0, v, 0, len);
			return len;
		}
