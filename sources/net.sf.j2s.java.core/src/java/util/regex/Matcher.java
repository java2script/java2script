...
//		String source;
	...
	public boolean find() {
		...
		if (nextSearchIndex < leftBound)
			nextSearchIndex = leftBound;

		// If next search starts beyond region then it fails
		if (nextSearchIndex > rightBound) {
			clearGroups();
			return false;
		}
		return search(nextSearchIndex, UNSPECIFIED);
	}
...
//			locals[i] = -1;
	...
	public boolean find(int start) {
		...
		reset();
		return search(start, UNSPECIFIED);
	}
	...
	public Matcher appendReplacement(StringBuffer sb, String replacement) {
		...
		processedRepl = (String) processRepl(replacement);
		// Append the intervening text
		sb.append(cs, appendPos, first);
	...
	private String processRepl(String replacement) {
		if (this.replacement != null && this.replacement.equals(replacement)) {
			return (replacementParts == null ? processedRepl : /** @j2sNative 1 ? this.replacementParts.join("") : */
					null);
		}
		...
		while (index < len) {
			char nextChar = replacement.charAt(index);
			switch (nextChar) {
			case '\\':
				index++;
				if (index == replacement.length())
					throw new IllegalArgumentException("character to be escaped is missing");
				nextChar = replacement.charAt(index);
				res.append(nextChar);
				index++;
				break;
			case '$':
				...
				if (nextChar == '{') {
					...
					if (pat.namedGroups == null || !pat.namedGroups().containsKey(gname))
						throw new IllegalArgumentException("No group with name {" + gname + "}");
					gr = pat.namedGroups().get(gname);
					index++;
				} else {
					if (replacementParts == null)
						replacementParts = new String[0];
					...
					while (!done) {
						...
						int nextDigit = replacement.charAt(index) - '0';
						if ((nextDigit < 0) || (nextDigit > 9)) { // not a number
							break;
						}
						int newRefNum = (gr * 10) + nextDigit;
				...
				int n = res.length();
				if (replacementPos != n) {
					replacementParts[replacementParts.length] = res.substring(replacementPos, n);
					replacementPos = n;
				}
				String g = (replacementParts[replacementParts.length] = new Value(gr)).toString();
				// Append group
//				int p0 = start(gr);
//				int p1 = end(gr);
//				if (p0 >= 0 && p1 >= p0)
//					res.append(cs, p0, p1);
		...
		if (replacementParts != null && replacementPos != res.length()) {
			replacementParts[replacementParts.length] = res.substring(replacementPos, res.length());
		}
		return res.toString();
	}
	...
	public StringBuffer appendTail(StringBuffer sb) {
		return sb.append(cs, appendPos, getTextLength());
	}
...
//        return getSubSequence(groups[group * 2], groups[group * 2 + 1]).toString();
	 ...
	 *                                  the previous match operation failed
	 *
	 * @throws IllegalArgumentException If there is no capturing group in the
...
//        return getSubSequence(groups[group * 2], groups[group * 2 + 1]).toString();
	...
	public boolean matches() {
		return match(leftBound, ENDANCHOR);
	}
	...
	private String[] execRE(RegExp rg, String s) {
		return /** @j2sNative 1 ? rg.exec(s) : */
		null;
	}
	...
	CharSequence getSubSequence(int beginIndex, int endIndex) {
		return cs.subSequence(beginIndex, endIndex);
	}
