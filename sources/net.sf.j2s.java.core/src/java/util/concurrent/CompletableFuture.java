...
public class CompletableFuture<T> implements Future<T>, CompletionStage<T> {
	 ...
	 * linked stack. It atomically completes by CASing a result field, and then pops
	 * off and runs those actions. This applies across normal vs exceptional
	 * outcomes, sync vs async actions, binary triggers, and various forms of
	 * completions.
	 *
	 * Non-nullness of field result (set via CAS) indicates done. An AltResult is
	 * used to box null as a result, as well as to hold exceptions. Using a single
	 * field makes completion simple to detect and trigger. Encoding and decoding is
	 * straightforward but adds to the sprawl of trapping and associating exceptions
	 * with targets. Minor simplifications rely on (static) NIL (to box null
	 * results) being the only AltResult with a null exception field, so we don't
	 * usually need explicit comparisons. Even though some of the generics casts are
	 * unchecked (see SuppressWarnings annotations), they are placed to be
	 * appropriate even if checked.
	 *
	 * Dependent actions are represented by Completion objects linked as Treiber
	 * stacks headed by field "stack". There are Completion classes for each kind of
	 * action, grouped into single-input (UniCompletion), two-input (BiCompletion),
	 * projected (BiCompletions using either (not both) of two inputs), shared
	 * (CoCompletion, used by the second of two sources), zero-input source actions,
	 * and Signallers that unblock waiters. Class Completion extends ForkJoinTask to
	 * enable async execution (adding no space overhead because we exploit its "tag"
	 * methods to maintain claims). It is also declared as Runnable to allow usage
	 * with arbitrary executors.
	 *
	 * Support for each kind of CompletionStage relies on a separate class, along
	 * with two CompletableFuture methods:
	 *
	 * * A Completion class with name X corresponding to function, prefaced with
	 * "Uni", "Bi", or "Or". Each class contains fields for source(s), actions, and
	 * dependent. They are boringly similar, differing from others only with respect
	 * to underlying functional forms. We do this so that users don't encounter
	 * layers of adaptors in common usages. We also include "Relay" classes/methods
	 * that don't correspond to user methods; they copy results from one stage to
	 * another.
	 *
	 * * Boolean CompletableFuture method x(...) (for example uniApply) takes all of
	 * the arguments needed to check that an action is triggerable, and then either
	 * runs the action or arranges its async execution by executing its Completion
	 * argument, if present. The method returns true if known to be complete.
	 *
	 * * Completion method tryFire(int mode) invokes the associated x method with
	 * its held arguments, and on success cleans up. The mode argument allows
	 * tryFire to be called twice (SYNC, then ASYNC); the first to screen and trap
	 * exceptions while arranging to execute, and the second when called from a
	 * task. (A few classes are not used async so take slightly different forms.)
	 * The claim() callback suppresses function invocation if already claimed by
	 * another thread.
	 *
	 * * CompletableFuture method xStage(...) is called from a public stage method
	 * of CompletableFuture x. It screens user arguments and invokes and/or creates
	 * the stage object. If not async and x is already complete, the action is run
	 * immediately. Otherwise a Completion c is created, pushed to x's stack (unless
	 * done), and started or triggered via c.tryFire. This also covers races
	 * possible if x completes while pushing. Classes with two inputs (for example
	 * BiApply) deal with races across both while pushing actions. The second
	 * completion is a CoCompletion pointing to the first, shared so that at most
	 * one performs the action. The multiple-arity methods allOf and anyOf do this
	 * pairwise to form trees of completions.
	 *
	 * Note that the generic type parameters of methods vary according to whether
	 * "this" is a source, dependent, or completion.
	 *
	 * Method postComplete is called upon completion unless the target is guaranteed
	 * not to be observable (i.e., not yet returned or linked). Multiple threads can
	 * call postComplete, which atomically pops each dependent action, and tries to
	 * trigger it via method tryFire, in NESTED mode. Triggering can propagate
	 * recursively, so NESTED mode returns its completed dependent (if one exists)
	 * for further processing by its caller (see method postFire).
	 *
	 * Blocking methods get() and join() rely on Signaller Completions that wake up
	 * waiting threads. The mechanics are similar to Treiber stack wait-nodes used
	 * in FutureTask, Phaser, and SynchronousQueue. See their internal documentation
	 ...
	 * Completion.detach). The screening checks needed anyway harmlessly ignore null
	 * arguments that may have been obtained during races with threads nulling out
	 * fields. We also try to unlink fired Completions from stacks that might never
	 * be popped (see method postFire). Completion fields need not be declared as
	 * final or volatile because they are only visible to other threads upon safe
	 * publication.
	 */
	...
	/**
	 * Returns the encoding of the given (non-null) exception as a wrapped
	 * CompletionException unless it is one already. May return the given Object r
	 * (which must have been the result of a source future) if it is equivalent,
	 * i.e. if this is a simple relay of an existing CompletionException.
	 */
	 ...
	 * CompletionException unless it is one already, unless already completed. May
	 * complete with the given Object r (which must have been the result of a source
	 * future) if it is equivalent, i.e. if this is a simple propagation of an
	...
	/**
	 * Returns the encoding of the given arguments: if the exception is non-null,
	 * encodes as AltResult. Otherwise uses the given value, boxed as NIL if null.
	 */
	...
	/**
	 * Completes with r or a copy of r, unless already completed. If exceptional, r
	 * is first coerced to a CompletionException.
	 */
	...
	/**
	 * A marker interface identifying asynchronous tasks produced by {@code async}
	 * methods. This may be useful for monitoring, debugging, and tracking
	...
	abstract static class Completion extends ForkJoinTask<Void> implements Runnable, AsynchronousCompletionTask {
		...
		/**
		 * Performs completion action if triggered, returning a dependent that may need
		 * propagation, if one exists.
		 *
	...
	/**
	 * Pops and tries to trigger all reachable dependents. Call only when known to
	 * be done.
	 */
	final void postComplete() {
		/*
		 * On each step, variable f holds current dependents to pop and run. It is
		 * extended along only one path at a time, pushing others to avoid unbounded
		 * recursion.
		 */
	...
	abstract static class UniCompletion<T, V> extends Completion {
		...
		/**
		 * Returns true if action can be run. Call only when known to be triggerable.
		 * Uses FJ tag bit to ensure that only one thread claims ownership. If async,
		 * starts as task -- a later call to tryFire will run action.
		 */
		final boolean claim() {
			Executor e = executor;
	...
	/**
	 * Post-processing by dependent after successful UniCompletion tryFire. Tries to
	 * clean stack of source a, and then either runs postComplete or returns this to
		...
		tryComplete: if (result == null) {
			...
			try {
				...
				if (g.result == null || !uniRelay(g)) {
					UniRelay<T, T> copy = new UniRelay<T, T>(this, g);
					g.push(copy);
					copy.tryFire(SYNC);
					if (result == null)
						return false;
	...
	private <V> CompletableFuture<V> uniComposeStage(Executor e, Function<? super T, ? extends CompletionStage<V>> f) {
		...
		if (e == null && (r = result) != null) {
			...
			try {
				...
				CompletableFuture<V> d = new CompletableFuture<V>();
				UniRelay<V, V> copy = new UniRelay<V, V>(d, g);
				g.push(copy);
				copy.tryFire(SYNC);
				return d;
			} catch (Throwable ex) {
	 ...
	 * implements ManagedBlocker to avoid starvation when blocking actions pile up
	 * in ForkJoinPools.
	 */
	@SuppressWarnings("serial")
	...
	/**
	 * Returns {@code true} if completed in any fashion: normally, exceptionally, or
	 * via cancellation.
	 *
	 ...
	 * {@link CompletionException} with the underlying exception as its cause.
	 *
	 * @return the result value
	...
	/**
	 * If not already completed, causes invocations of {@link #get()} and related
	 ...
	 * flexible versions of this functionality are available using methods
	 ...
	 * any way. Possible causes include cancellation, explicit invocation of {@code
	 ...
	 * {@link #get()} and related methods, whether or not already completed. This
	 * method is designed for use only in error recovery actions, and even in such
	 * situations may result in ongoing dependent completions using established
	 * versus overwritten outcomes.
	 *
	...
	/**
	 * Forcibly causes subsequent invocations of method {@link #get()} and related
	 * methods to throw the given exception, whether or not already completed. This
	 * method is designed for use only in error recovery actions, and even in such
	 * situations may result in ongoing dependent completions using established
	 * versus overwritten outcomes.
	 *
	 ...
	 * awaiting completion of this CompletableFuture. This method is designed for
	 ...
	 * completed"} followed by the number of CompletableFutures dependent upon its
	 * completion, if any.
	 *
	...
	// In SwingJS, all these are just String names, not memory offsets.
    ...
    public Executor defaultExecutor() {
        return getAsync_PoolJS();
    }

    private static Executor getAsync_PoolJS() {
    	return ASYNC_POOL == null ? (ASYNC_POOL = new ThreadPerTaskExecutor()) : ASYNC_POOL;
    }
    ...
    public static Executor delayedExecutor(long delay, TimeUnit unit) {
        if (unit == null)
            throw new NullPointerException();
        return new DelayedExecutor(delay, unit, getAsync_PoolJS());
    }
    ...
    final void unipush(Completion c) {
        if (c != null) {
            while (!tryPushStack(c)) {
                if (result != null) {
                    UNSAFE.putObject(c, NEXT, null);//NEXT.set(c, null);
                    break;
                }
            }
            if (result != null)
