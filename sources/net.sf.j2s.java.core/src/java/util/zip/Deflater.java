...
public class Deflater extends swingjs.jzlib.Deflater {
	...
	public Deflater(int level, boolean nowrap) {
		...
		if (level != Integer.MAX_VALUE)
			init(level, DEFAULT_STRATEGY, 0, nowrap);
	}

	/**
	 * Creates a new compressor using the specified compression level. Compressed
	...
	private void init(int level, int strategy, int maxBits, boolean nowrap) {
		super.init(level, maxBits, nowrap);
	...
	public void setInput(byte[] b, int off, int len) {
        ...
        super.setInput(b, off, len, false);
	}
	/**
	 * Sets input data for compression. This should be called whenever needsInput()
	 * returns true indicating that more input data is required.
	...
	public void setDictionary(byte[] b, int off, int len) {
		...
		super.setDictionaryRet(b, off, len);
	}
	 ...
	 * strategy (and may be flushed); the new strategy will take effect only after
	 * that invocation.
	 *
	 * @param strategy the new compression strategy
	 * @exception IllegalArgumentException if the compression strategy is invalid
	 */
	 ...
	 *
	 * <p>
	 * If the compression level is changed, the next invocation of {@code deflate}
	 * will compress the input available so far with the old level (and may be
	 * flushed); the new level will take effect only after that invocation.
	 *
	 ...
	 * indicates that {@link #needsInput() needsInput} should be called in order to
	 * determine if more input data is required.
	 *
	 * <p>
	 * This method uses {@link #NO_FLUSH} as its compression flush mode. An
	 * invocation of this method of the form {@code deflater.deflate(b, off, len)}
	 ...
	 * @return the actual number of bytes of compressed data written to the output
	 *         buffer
	 */
	public int deflate(byte[] out, int off, int len) {
		return deflate(out, off, len, NO_FLUSH);
	}
	 ...
	 * <p>
	 * This method uses {@link #NO_FLUSH} as its compression flush mode. An
	 * invocation of this method of the form {@code deflater.deflate(b)} yields the
	 * same result as the invocation of
	 ...
	 * (should be used in normal use scenario). A return value of 0 in this flush
	 * mode indicates that {@link #needsInput()} should be called in order to
	 * determine if more input data is required.
	 *
	 * <li>{@link #SYNC_FLUSH}: all pending output in the deflater is flushed, to
	 * the specified output buffer, so that an inflater that works on compressed
	 * data can get all input data available so far (In particular the
	 * {@link #needsInput()} returns {@code true} after this invocation if enough
	 * output space is provided). Flushing with {@link #SYNC_FLUSH} may degrade
	 * compression for some compression algorithms and so it should be used only
	 * when necessary.
	 *
	 * <li>{@link #FULL_FLUSH}: all pending output is flushed out as with
	 * {@link #SYNC_FLUSH}. The compression state is reset so that the inflater that
	 * works on the compressed output data can restart from this point if previous
	 * compressed data has been damaged or if random access is desired. Using
	 * {@link #FULL_FLUSH} too often can seriously degrade compression.
	 * </ul>
	 *
	 * <p>
	 * In the case of {@link #FULL_FLUSH} or {@link #SYNC_FLUSH}, if the return
	 * value is {@code len}, the space available in output buffer {@code b}, this
	 * method should be invoked again with the same {@code flush} parameter and more
	 * output space.
	 *
	 * @param b     the buffer for the compressed data
	 * @param off   the start offset of the data
	...
	public int deflate(byte[] b, int off, int len, int flush) {
		...
		return super.deflate(b, off, len, flush);
//		synchronized (zsRef) {
//			ensureOpen();
...
//		}
	 ...
	 * {@link #getBytesRead()} method is now the preferred means of obtaining this
	 * information.
	 * </p>
	 *
...
//		}
	...
	/**
	 * Closes the compressor and discards any unprocessed input. This method should
	 * be called when the compressor is no longer being used, but will also be
	 * called automatically by the finalize() method. Once this method is called,
