...
public class JSTableHeaderUI extends JSLightweightUI {
	...
	protected DOMNode updateDOMNode() {
		if (domNode == null) {
			...
			DOMNode.setStyles(domNode, "width", w + "px", "height", 
					h + "px", "table-layout","fixed");
			// we need to set the header row here?
		}
	...
	public class MouseInputHandler implements MouseInputListener {
		...
		public void mouseDragged(MouseEvent e) {
			...
			if (resizingColumn != null) {
				...
				mouseXOffset += changeColumnWidth(resizingColumn, header, oldWidth,
						newWidth);
	...
	/**
	 * This method gets called every time the rollover column in the table header
	 * is updated. Every look and feel supporting rollover effect in table header
	 * should override this method and repaint the header.
	 * 
	 * @param oldColumn
	 *          the index of the previous rollover column or -1 if the mouse was
	 *          not over a column
	 * @param newColumn
	...
	private static class Actions extends UIAction {
		...
		private boolean maybeMoveColumn(boolean leftArrow, JTableHeader th,
			...
			if (th.getComponentOrientation().isLeftToRight()) {
				newIndex = leftArrow ? ui.selectPreviousColumn(doIt) : ui
						.selectNextColumn(doIt);
			} else {
				newIndex = leftArrow ? ui.selectNextColumn(doIt) : ui
						.selectPreviousColumn(doIt);
			}
