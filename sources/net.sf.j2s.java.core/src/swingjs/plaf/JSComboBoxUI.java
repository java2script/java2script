...
/**
 * An upgraded JQuery-driven div/ul/li combobox. See swingjs/jquery/j2scb.htm
 * 
 * painting of ComboBox is not implemented.
 * 
 * painting of JList, JButton, and JScrollPane is ignored -- j2scb will take
 * care of that.
 * 
 * 
 */
...
//
	...
	/**
	 * Creates and installs listeners for the combo box and its model. This method
	 * is called when the UI is installed.
	 */
	...
	/**
	 * Removes the installed listeners from the combo box and its model. The number
	 * and types of listeners removed and in this method should be the same that was
	 * added in <code>installListeners</code>
	 */
...
//	protected JButton arrowButton;
	...
	/**
	 * This protected field is implementation specific. Do not access directly or
	 * override. Override the listener construction method instead.
	 *
	...
	/**
	 * This is tricky, this variables is needed for DefaultKeySelectionManager to
	 * take into account time factor.
	 */
	...
	/**
	 * Indicates whether or not the combo box button should be square. If square,
	 * then the width and height are equal, and are both set to the height of the
	 * combo minus appropriate insets.
	 *
	 * @since 1.7
	 */
	 ...
	 * laying out and painting the "selected" item in the combo box. These insets
	 * add to those specified by the cell renderer.
	 *
	 * @since 1.7
	 */
...
//BH??			renderer = new DefaultListCellRenderer();
	...
	/**
	 * Creates a <code>KeyListener</code> which will be added to the combo box. If
	...
	/**
	 * Creates a <code>FocusListener</code> which will be added to the combo box. If
	 * this method returns null then it will not be added to the combo box.
	 *
	 ...
	 * <code>ComboBoxModel</code>. If this method returns null then it will not be
	...
	/**
	 * Creates an <code>ItemListener</code> which will be added to the combo box. If
	 * this method returns null then it will not be added to the combo box.
	 * <p>
	 ...
	 * combo box. If this method returns null then it will not be added to the combo
	 * box.
	 *
	...
	/**
	 * Creates a layout manager for managing the components which make up the combo
	 * box.
	 *
	 ...
	 * A default renderer will used only if a renderer has not been explicitly set
	 * with <code>setRenderer</code>.
	 *
	...
	/**
	 * This listener checks to see if the key event isn't a navigation key. If it
	 * finds a key event that wasn't a navigation key it dispatches it to
	 * JComboBox.selectWithKeyChar() so that it can do type-ahead.
	 *
	 * This public inner class should be treated as protected. Instantiate it only
	 * within subclasses of <code>JSComboBoxUI</code>.
	 */
	...
	/**
	 * This listener hides the popup when the focus is lost. It also repaints when
	 ...
	 * box.
	 * <p>
	 * Subclasses which wish to listen to combo box property changes should call the
	 * superclass methods to ensure that the combo box ui correctly handles property
	 * changes.
	 * <p>
	 * This public inner class should be treated as protected. Instantiate it only
	 * within subclasses of <code>JSComboBoxUI</code>.
	...
	// Syncronizes the ToolTip text for the components within the combo box to be
	// the
	...
	/**
	 * This layout manager handles the 'standard' layout of combo boxes. It puts the
	 * arrow button to the right and the editor to the left. If there is no editor
	 * it still keeps the arrow button to the right.
	 *
	 * This public inner class should be treated as protected. Instantiate it only
	 * within subclasses of <code>JSComboBoxUI</code>.
	 */
...
//
	...
	/**
	 * The aggregate components which comprise the combo box are unregistered and
...
//		arrowButton = null;
	 ...
	 * call or override. To implement a specific editor create a custom
	...
	/**
	 * This protected method is implementation specific and should be private. do
...
//		currentValuePane.paintComponent(g, c, comboBox, x, y, w, h, shouldValidate);
	...
	/**
	 * Return the default size of an empty display area of the combo box using the
	 ...
	 * portion of the combo box in which the selected item is displayed. This method
	 * will use the prototype display value if it has been set.
	 * <p>
	 * For combo boxes with a non trivial number of items, it is recommended to use
	 * a prototype display value to significantly speed up the display size
	 * calculation.
	 *
	...
	protected void installKeyboardActions() {
		...
		LazyActionMap.installLazyActionMap(comboBox, JSComboBoxUI.class, "ComboBox.actionMap");
	}

	InputMap getInputMap(int condition) {
		if (condition == JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT) {
			InputMap keyMap = (InputMap) DefaultLookup.get(comboBox, this, "ComboBox.ancestorInputMap");
			InputMap listMap = (InputMap) DefaultLookup.get(comboBox, this, "List.focusInputMap");
			if (listMap == null)
				return keyMap;
			listMap.setParent(keyMap);
				return listMap;
		}
		return null;
	}
	...
	private static class Actions extends UIAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			JSComboBoxUI ui = (JSComboBoxUI) comboBox.getUI();
			if (key == HIDE) {
				...
				if (ui != null && (key == TOGGLE || !comboBox.isEditable())) {
					if (ui.isTableCellEditor()) {
						...
						comboBox.setSelectedIndex(ui.listBox.getSelectedIndex());
					} else {
				...
				if (comboBox.isPopupVisible()) {
					...
					if (UIManager.getBoolean("ComboBox.noActionOnKeyNavigation")) {
						Object listItem = ui.listBox.getSelectedValue();
						if (listItem != null) {
					...
					if (root != null) {
						...
						if (im != null && am != null) {
							...
							if (obj != null) {
								...
								if (action != null) {
									action.actionPerformed(new ActionEvent(root, e.getID(), e.getActionCommand(),
		...
		private int getNextIndex(JComboBox comboBox, String key) {
			...
			if (UIManager.getBoolean("ComboBox.noActionOnKeyNavigation")
					&& (comboBox.getUI() instanceof JSComboBoxUI)) {
				selectedIndex = ((JSComboBoxUI) comboBox.getUI()).listBox.getSelectedIndex();
			}
...
//						arrowButton.setFocusable(focusable);
		...
		public void keyPressed(KeyEvent e) {
			if (isNavigationKey(e.getKeyCode(), e.getModifiers())) {
				lastTime = 0L;
			} else if (comboBox.isEnabled() && comboBox.getModel().getSize() != 0 && isTypeAheadKey(e)
				...
				if (am != null) {
					...
					if (action != null) {
						action.actionPerformed(
								new ActionEvent(comboBox, evt.getID(), evt.getActionCommand(), evt.getModifiers()));
					}
				}
			}
		}
