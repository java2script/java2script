...
public class JSProgressBarUI extends JSLightweightUI {
	...
	// derived from view-source:https://css-tricks.com/examples/ProgressBars/
static String indeterminateStyle =  
			"<style>.j2s-progress {\n" + 
			"			background: #555;\n" + 
			"		}\n" + 
			"		.j2s-progress > span {\n" + 
...
//    }
    ...
    protected void paintIndeterminate(Graphics g, JComponent c) {
		if (indeterminateStyle != null) {
			$("body").append(indeterminateStyle);
			indeterminateStyle = null;
		}
		addClass(barNode, "j2s-progress");
    	paintDeterminate(g, c);
...
//        }
	...
	protected void paintDeterminate(Graphics g, JComponent c) {
		...
		if (progressBar.isIndeterminate()) {
    		...
    		DOMNode.setTopLeftAbsolute(barNode, b.top, b.left);
	    		...
	    		DOMNode.setSize(barNode, barRectHeight, barRectWidth);
			...
			if (progressBar.getOrientation() == JProgressBar.HORIZONTAL) {
				...
				if (JSGraphicsUtils.isLeftToRight(c)) {
				...
				if (cellSpacing == 0 && amountFull > 0) {
					...
					// draw each individual cell
					g2.setStroke(new BasicStroke((float) barRectWidth, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 0f,
    ...
    protected void paintString(Graphics g, int x, int y,
        ...
        if (progressBar.getOrientation() == JProgressBar.HORIZONTAL) {
            if (JSGraphicsUtils.isLeftToRight(progressBar)) {
//                if (progressBar.isIndeterminate()) {
//                    boxRect = getBox(boxRect);
...
//        if ((progressBar.getOrientation() == JProgressBar.HORIZONTAL) &&
//            (JSGraphicsUtils.isLeftToRight(progressBar))) {
