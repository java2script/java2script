 ...
 * 
 * this implementation is not finished -- just a sketch.
 * 
...
public class JSTreeUI extends JSPanelUI {
	...
	/**
	 * Distance to add to leftChildIndent to determine where cell contents will be
	 * drawn.
	 */
	...
	/**
	 * Set to true if the renderer that is currently in the tree was created by this
	 * instance.
	 */
	...
	/**
	 * The time factor to treate the series of typed alphanumeric key as prefix for
...
//			bindJSKeyEvents(focusNode, false);
	...
	/**
	 * Returns the Rectangle enclosing the label portion that the last item in path
	 * will be drawn into. Will return null if any component in path is currently
	 * valid.
	 */
	...
	/**
	 * Stops the current editing session. This has no effect if the tree isn't being
	 * edited. Returns true if the editor allows the editing session to stop.
	 */
	...
	/**
	 * Selects the last item in path and tries to edit it. Editing will fail if the
...
//        }
	...
	protected void installKeyboardActions() {
		...
		LazyActionMap.installLazyActionMap(tree, JSTreeUI.class, "Tree.actionMap");
	}
	...
	InputMap getInputMap(int condition) {
		if (condition == JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT) {
			...
			if (tree.getComponentOrientation().isLeftToRight() || ((rtlKeyMap = (InputMap) DefaultLookup.get(tree, this,
	...
	/**
	 * Creates an instance of NodeDimensions that is able to determine the size of a
	...
	/**
	 * Creates a listener that is responsible that updates the UI based on how the
	 * tree changes.
	 */
	...
	/**
	 * Creates a listener that is responsible for updating the display when focus is
	 * lost/gained.
	 */
	...
	/**
	 * Creates the listener responsible for getting property change events from the
	 * selection model.
	 */
	...
	/**
	 * Creates and returns a new ComponentHandler. This is used for the large model
	 * to mark the validCachedPreferredSize as invalid when the component moves.
	 */
	...
	/**
	 * Creates the object responsible for managing what is expanded, as well as the
	 * size of nodes.
	 */
	 ...
	 * This is meant for {@code javax.swing.DropMode.INSERT} and
...
//	
	...
	/**
	 * Returns true if the expand (toggle) control should be drawn for the specified
	 * row.
	 */
	protected boolean shouldPaintExpandControl(TreePath path, int row, boolean isExpanded, boolean hasBeenExpanded,
			boolean isLeaf) {
		if (isLeaf)
			return false;
		int depth = path.getPathCount() - 1;
	...
	/**
	 * The vertical element of legs between nodes starts at the bottom of the parent
	 * node by default. This method makes the leg start below that.
	 */
	...
	/**
	 * Returns the location, along the x-axis, to render a particular row at. The
	 * return value does not include any Insets specified on the JTree. This does
	 * not check for the validity of the row or depth, it is assumed to be correct
	 * and will not throw an Exception if the row or depth doesn't match that of the
	 * tree.
	 *
	...
	/**
	 * Makes all the nodes that are expanded in JTree expanded in LayoutCache. This
	 ...
	 * contained in. If the tree is editable but doesn't have a cellEditor, a basic
	 * one will be used.
	 */
	...
	/**
	 * Resets the TreeState instance based on the tree we're providing the look and
	 * feel for.
	 */
	...
	/**
	 * Updates the <code>preferredSize</code> instance variable, which is returned
	 * from <code>getPreferredSize()</code>.
	 * <p>
			...
			if (beginRow == endRow) {
				...
				if (beginRect != null) {
					...
					tree.scrollRectToVisible(
							new Rectangle(visRect.x, beginY, 1, testRect.y + testRect.height - beginY));
				}
			}
		}
	}
	 ...
	 * <I>checkConsistency</I> is true <b>checkConsistency</b> is messaged first.
	 */
	...
	/**
	 * Messages to stop the editing session. If the UI the receiver is providing the
	 * look and feel for returns true from <code>getInvokesStopCellEditing</code>,
	 * stopCellEditing will invoked on the current editor. Then completeEditing will
	 * be messaged with false, true, false to cancel any lingering editing.
	 */
	...
	/**
	 * Stops the editing session. If messageStop is true the editor is messaged with
	 * stopEditing, if messageCancel is true the editor is messaged with
	...
	protected void completeEditing(boolean messageStop, boolean messageCancel, boolean messageTree) {
		if (stopEditingInCompleteEditing && editingComponent != null) {
			...
			boolean requestFocus = (tree != null
					&& (tree.hasFocus() || SwingUtilities.findFocusOwner(editingComponent) != null));
		...
		if (cellEditor != null && tree.isPathEditable(path)) {
			...
			if (cellEditor.isCellEditable(event)) {
				editingComponent = cellEditor.getTreeCellEditorComponent(tree, path.getLastPathComponent(),
						tree.isPathSelected(path), tree.isExpanded(path), treeModel.isLeaf(path.getLastPathComponent()),
						row);
				...
				if (event != null) {
					 ...
					 * mouseReleased.
					 */
					Point componentPoint = SwingUtilities.convertPoint(tree, new Point(event.getX(), event.getY()),
							editingComponent);
	...
	/**
	 * If the <code>mouseX</code> and <code>mouseY</code> are in the expand/collapse
	 * region of the <code>row</code>, this will toggle the row.
	 */
	...
	/**
	 * Returns true if <code>mouseX</code> and <code>mouseY</code> fall in the area
	 * of row that is used to expand/collapse the node and the node at
	 ...
	 * expanding a path and JTree scrolls on expand, ensureRowsAreVisible is invoked
	 * to scroll as many of the children to visible as possible (tries to scroll to
	 * last visible descendant of path).
	 */
	 ...
	 * If the event is a toggle selection event, the row is either selected, or
	 * deselected. If the event identifies a multi selection event, the selection is
	 * updated from the anchor point. Otherwise the row is selected, and if the
	 * event specified a toggle event the row is expanded/collapsed.
	 */
	...
	/**
	 * Extends the selection from the anchor to make <code>newLead</code> the lead
	 * of the selection. This does not scroll.
	 */
	...
	public class ComponentHandler extends ComponentAdapter implements ActionListener {
		...
		/**
		 * Creates, if necessary, and starts a Timer to check if need to resize the
		 * bounds.
		 */
	...
	} // End of JSTreeUI.ComponentHandler
	...
	} // End of JSTreeUI.TreeModelHandler
	...
	}// End of JSTreeUI.TreeSelectionHandler
	...
	public class KeyHandler extends KeyAdapter {
		 ...
		 *
		 * Moves the keyboard focus to the first element whose first letter matches the
	...
	} // End of class JSTreeUI.FocusHandler

	/**
	 * Class responsible for getting size of node, method is forwarded to JSTreeUI
	 * method. X location does not include insets, that is handled in getPathBounds.
	 */
	...
	public class NodeDimensionsHandler extends AbstractLayoutCache.NodeDimensions {
		 ...
		 */
		protected int getRowX(int row, int depth) {
			return JSTreeUI.this.getRowX(row, depth);
		}

	} // End of class JSTreeUI.NodeDimensionsHandler

	/**
	 * TreeMouseListener is responsible for updating the selection based on mouse
	 * events.
	 */
	...
	} // End of JSTreeUI.PropertyChangeHandler
	...
	} // End of JSTreeUI.SelectionModelPropertyChangeHandler

	/**
	 * <code>TreeTraverseAction</code> is the action used for left/right keys. Will
	 * toggle the expandedness of a node, as well as potentially incrementing the
	 * selection.
	 */
	...
	} // JSTreeUI.TreePageAction

	/**
	 * TreeIncrementAction is used to handle up/down actions. Selection is moved up
	 * or down based on direction.
	 */
	public class TreeIncrementAction extends AbstractAction {
		/** Specifies the direction to adjust the selection by. */
		protected int direction;
		/**
		 * If true the new item is added to the selection, if false the selection is
		 * reset.
		 */
		private boolean addToSelection;
		private boolean changeSelection;
	...
	} // End of class JSTreeUI.TreeIncrementAction
	...
	} // End of class JSTreeUI.TreeHomeAction
	...
	} // End of class JSTreeUI.TreeToggleAction
	...
	} // End of class JSTreeUI.TreeCancelEditingAction
	 ...
	 * with. It is assumed all the events are currently target at source.
	 */
	...
	} // End of class JSTreeUI.MouseInputHandler
...
//    }
	...
	private class Handler implements CellEditorListener, FocusListener, KeyListener, MouseListener, MouseMotionListener,
			PropertyChangeListener, TreeExpansionListener, TreeModelListener, TreeSelectionListener
//                  ,
//                  BeforeDrag 
	{
		//
		 ...
		 * sequence of alphanumeric keys pressed by the user with delay less than value
		 * of <code>timeFactor</code> property (or 1000 milliseconds if it is not
		 * defined). Subsequent same key presses move the keyboard focus to the next
		 * object that starts with the same letter until another key is pressed, then it
...
//			}
	...
	private static class Actions extends UIAction {
		...
		private void scrollChangeSelection(JTree tree, JSTreeUI ui, int direction, boolean addToSelection,
		...
		private void scroll(JTree component, JSTreeUI ui, int direction, int amount) {
		...
		private void extendSelection(JTree tree, JSTreeUI ui) {
			if (ui.getRowCount(tree) > 0) {
				int lead = ui.getLeadSelectionRow();
		...
		private void selectAll(JTree tree, JSTreeUI ui, boolean selectAll) {
		...
		private void startEditing(JTree tree, JSTreeUI ui) {
		...
		private void cancelEditing(JTree tree, JSTreeUI ui) {
			tree.cancelEditing();
		}

		private void toggle(JTree tree, JSTreeUI ui) {
		...
		private void expand(JTree tree, JSTreeUI ui) {
			int selRow = ui.getLeadSelectionRow();
			tree.expandRow(selRow);
		}

		private void collapse(JTree tree, JSTreeUI ui) {
			int selRow = ui.getLeadSelectionRow();
			tree.collapseRow(selRow);
		}

		private void increment(JTree tree, JSTreeUI ui, int direction, boolean addToSelection,
				boolean changeSelection) {

			// disable moving of lead unless in discontiguous mode
			if (!addToSelection && !changeSelection
					&& tree.getSelectionModel().getSelectionMode() != TreeSelectionModel.DISCONTIGUOUS_TREE_SELECTION) {
			...
			if (ui.treeSelectionModel != null && (rowCount = tree.getRowCount()) > 0) {
				...
				if (addToSelection
						&& ui.treeSelectionModel.getSelectionMode() != TreeSelectionModel.SINGLE_TREE_SELECTION) {
		...
		private void traverse(JTree tree, JSTreeUI ui, int direction, boolean changeSelection) {
		...
		private void moveSelectionToParent(JTree tree, JSTreeUI ui) {
			int selRow = ui.getLeadSelectionRow();
		...
		private void page(JTree tree, JSTreeUI ui, int direction, boolean addToSelection, boolean changeSelection) {
		...
		private void home(JTree tree, final JSTreeUI ui, int direction, boolean addToSelection,
				boolean changeSelection) {
