...
try{
 ...
 var delayMe = function(element, f, ms) {
	 ...
	 System.out.println([ms, id, f]);
	 return id;
 };	
 
 var clearMe = function(id) {
	 System.out.println("j2sm clear " + id)
...
this._trigger("focus",e,{item:t})},
 ...
 blur:function(e,t){
	 ...
	 t||clearMe(this.timer);if(!this.active)return;
	 
	 this.active
	 //.children("a")
	 .removeClass("ui-state-focus"),this.active=null,this._trigger("blur",e,{item:this.active})
	 },
 _startOpening:function(e){clearMe(this.timer);if(e.attr("aria-hidden")!=="true")return;
 this.timer=delayMe(this, function(){this._close(),this._open(e)},this.delay)},
 _open:function(t){var n=e.extend({of:this.active},this.options.position);clearMe(this.timer),this.element.find(".ui-menu").not(t.parents(".ui-menu")).hide().attr("aria-hidden","true"),t.show().removeAttr("aria-hidden").attr("aria-expanded","true").position(n)},
...
.removeClass("ui-state-active")
 ...
 expand:function(e){var t=this.active&&this.active.children(".ui-menu ").children(".ui-menu-item").first();t&&t.length&&(this._open(t.parent()),
		 delayMe(this,function(){this.focus(e,t)}))},
