...
class ContextFinder {
	 ...
	 * rt.jar cannot have META-INF/services/javax.xml.bind.JAXBContext because if it
	 * has, it will take precedence over any file that applications have in their
	 * jar files.
	 *
	 * <p>
	 * When the user bundles his own JAXB implementation, we'd like to use it, and
	 * we want the platform default to be used only when there's no other JAXB
	 * provider.
	 *
	 * <p>
	...
	// previous value of JAXBContext.JAXB_CONTEXT_FACTORY, using also this to ensure
...
//            }
	 ...
	 * wrapped, throw the wrapped exception. Otherwise returns exception to be
	 * wrapped for further processing.
	 */
	...
	private static JAXBException handleClassCastException(Class originalType, Class targetType) {
		...
		return new JAXBException(Messages.format(Messages.ILLEGAL_CAST,
				...
				// lives in the impl
				// class' ClassLoader
				getClassClassLoader(originalType).getResource("javax/xml/bind/JAXBContext.class"), targetTypeURL));
	}

	/**
	 * Create an instance of a class using the specified ClassLoader
	 */
	static JAXBContext newInstance(String contextPath, Class[] contextPathClasses, String className,
	...
	static JAXBContext newInstance(String contextPath, Class[] contextPathClasses, Class spFactory,
			ClassLoader classLoader, Map properties) throws JAXBException {
		...
		try {
			...
			/*
			 * javax.xml.bind.context.factory points to a class which has a static method
			 * called 'createContext' that returns a javax.xml.JAXBContext.
			 */
	...
	static JAXBContext newInstance(Class[] classes, Map properties, String className) throws JAXBException {
		...
		// Logger.log(Level.FINE, "loaded {0} from {1}", new Object[]{className,
		// which(spi)});
		// }
...
//
		...
		JAXBContextFactory obj = ServiceLoaderUtil.firstByServiceLoader(JAXBContextFactory.class, logger,
				EXCEPTION_HANDLER);
	...
	static JAXBContext find(Class<?>[] classes, Map<String, ?> properties) throws JAXBException {
		...
		for (final Class c : classes) {
			// this classloader is used only to load jaxb.properties, so doing this should
			// be safe.
			...
			// TODO: do we want to optimize away searching the same package? org.Foo,
			// org.Bar, com.Baz
			...
			if (jaxbPropertiesUrl != null) {
				...
				String factoryClassName = classNameFromPackageProperties(jaxbPropertiesUrl,
		...
		// Logger.fine("Trying to create the platform default provider");
		Class ctxFactoryClass = (Class) ServiceLoaderUtil.lookupUsingOSGiServiceLoader("javax.xml.bind.JAXBContext",
				logger);
	...
	/**
	 * first factoryId should be the preferred one, more of those can be provided to
	 * support backwards compatibility
	 */
	private static String classNameFromPackageProperties(URL packagePropertiesUrl, String... factoryIds)
			throws JAXBException {

		// Logger.log(Level.FINE, "Trying to locate {0}",
		// packagePropertiesUrl.toString());
	...
	private static String getDeprecatedSystemProperty(String property) {
		...
		if (value != null) {
			// Logger.log(Level.WARNING, "Using non-standard property: {0}. Property {1}
