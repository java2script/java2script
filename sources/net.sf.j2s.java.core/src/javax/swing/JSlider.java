...
public class JSlider extends JComponent implements SwingConstants {
	...
	/**
	 * If true, the knob (and the data value it represents) resolve to the closest
	 * slider value next to where the user positioned the knob.
	 */
	 ...
	 * model. This listener is initialized to the {@code ChangeListener} returned
	 * from {@code createChangeListener}, which by default just forwards events to
	 ...
	 * generated here is always "this". The event is lazily created the first time
	 * that an event notification is fired.
	 * 
	 ...
	 * <p>
	 * The <code>BoundedRangeModel</code> that holds the slider's data handles any
	 * issues that may arise from improperly setting the minimum and maximum
	 ...
	 * <p>
	 * The <code>BoundedRangeModel</code> that holds the slider's data handles any
	 * issues that may arise from improperly setting the minimum, initial, and
	 * maximum values on the slider. See the {@code BoundedRangeModel}
	 ...
	 * {@code ChangeListener} simply calls the {@code fireStateChanged} method to
	 * forward {@code ChangeEvent}s to the {@code ChangeListener}s that have been
	 ...
	 * 
	 * @return all of the <code>ChangeListener</code>s added or an empty array if
	 ...
	 * {@code ChangeEvent}s. This method is called each time a {@code ChangeEvent}
	 ...
	 * 
	 * @param newModel
	 *          the new, {@code non-null} <code>BoundedRangeModel</code> to use
	 * 
	 * @see #getModel
	 * @see BoundedRangeModel
	...
	/**
	 * Sets the slider's current value to {@code n}. This method forwards the new
	 * value to the model.
	 * <p>
	 ...
	 * begins, and to {@code false} when the drag ends. The slider model will not
	 * generate {@code ChangeEvent}s while {@code valueIsAdjusting} is
	 * {@code true}.
	 * 
	 * @param b
	 ...
	 * respresents the range of values "covered" by the knob.
	 * 
	 ...
	 * either side of the knob. This method just forwards the new extent value to
	 * the model.
	 * <p>
	 * The data model (an instance of {@code BoundedRangeModel}) handles any
	 * mathematical issues arising from assigning faulty values. See the
	 ...
	 *           description: Set the scrollbars orientation to either VERTICAL or
	 *           HORIZONTAL. enum: VERTICAL JSlider.VERTICAL HORIZONTAL
	 *           JSlider.HORIZONTAL
	 * 
	 */
	...
	public Dictionary getLabelTable() {
		/*
		 * if ( labelTable == null && getMajorTickSpacing() > 0 ) { setLabelTable(
		 * createStandardLabels( getMajorTickSpacing() ) ); }
		 */
		return labelTable;
	}
	 ...
	 * 
	 * @param labels
	 *          new {@code Dictionary} of labels, or {@code null} to remove all
	 *          labels
	 ...
	 * slider minimum, and using the increment specified. For example, if you call
	 * <code>createStandardLabels( 10 )</code> and the slider minimum is zero,
	 ...
	 * must be passed into {@code setLabelTable}, and {@code setPaintLabels} must
	 * be set to {@code true}.
	 * <p>
	 ...
	 * if you call <code>createStandardLabels( 10, 2 )</code>, then labels will be
	 * created for the values 2, 12, 22, 32, and so on.
	 * <p>
	 * For the labels to be drawn on the slider, the returned {@code Hashtable}
	 * must be passed into {@code setLabelTable}, and {@code setPaintLabels} must
	 * be set to {@code true}.
	 ...
	 *              if {@code start} is out of range, or if {@code increment} is
	...
	public Hashtable createStandardLabels(int increment, int start) {
		...
		JSliderSmartHashtable table = new JSliderSmartHashtable(increment, start);
	 ...
	 * put the value range in the normal order. The order depends on the slider's
	 * <code>ComponentOrientation</code> property. Normal (non-inverted)
	 * horizontal sliders with a <code>ComponentOrientation</code> value of
	 * <code>LEFT_TO_RIGHT</code> have their maximum on the right. Normal
	 * horizontal sliders with a <code>ComponentOrientation</code> value of
	 ...
	 * is set to 10, you will get major ticks next to the following values: 0, 10,
	 * 20, 30, 40, 50.
	 * 
	 ...
	 * to {@code true}.
	 * <p>
	 * This method will also set up a label table for you. If there is not already
	 * a label table, and the major tick spacing is {@code > 0}, and
	 * {@code getPaintLabels} returns {@code true}, a standard label table will be
	 * generated (by calling {@code createStandardLabels}) with labels at the
	 * major tick marks. For the example above, you would get text labels: "0",
	 * "10", "20", "30", "40", "50". The label table is then set on the slider by
	 * calling {@code setLabelTable}.
	 * 
	 * @param n
	 ...
	 * is set to 10, you will get minor ticks next to the following values: 0, 10,
	 * 20, 30, 40, 50.
	 * 
	...
	/**
	 * Specifying true makes the knob (and the data value it represents) resolve
	 * to the closest slider value next to where the user positioned the knob. If
	 * the {@code snapToTicks} property has also been set to {@code true}, the
	 * snap-to-ticks behavior will prevail. By default, the snapToValue property
	 * is {@code true}.
	 * 
	 * @param b
	 ...
	 * <p>
	 * This method will also set up a label table for you. If there is not already
	 * a label table, and the major tick spacing is {@code > 0}, a standard label
	 * table will be generated (by calling {@code createStandardLabels}) with
	 * labels at the major tick marks. The label table is then set on the slider
	...
	class JSliderSmartHashtable extends Hashtable implements
		...
		public JSliderSmartHashtable(int increment, int start) {
			super();
	...
	class JSliderLabelUIResource extends JLabel implements UIResource {
		public JSliderLabelUIResource(String text, int alignment) {
