...
class Parser implements DTDConstants {
    ...
    /**
     * 
     * SwingJS - no DTD; just take all attributes as they come
...
//                  attvalue = parseAttributeValue((att != null) && (att.type != CDATA) && (att.type != NOTATION));
            ...
            if (att != null) {
            	...
            	att = elem.秘addAttribute(attname.toLowerCase());
    ...
    void parseTag() throws IOException {
          ...
          case '/':
            ...
            switch (ch = readCh()) {
              ...
              default:
                ...
                if (false && // SwingJS ignore DTD 
                		!dtd.elementExists(elemStr)) {
        ...
        if (!parseIdentifier(true)) {
            ...
            if (false && !dtd.elementExists(elemStr)) {
                //              parseInvalidTag();
    ...
    public synchronized void parse(Reader in) throws IOException {
        ...
        try {
            ...
            errorContext();
            error("exception", e.toString()/* SwingJS TODO .getClass().getName()*/, e.getMessage());
            e.printStackTrace();
        } catch (ThreadDeath e) {
            errorContext();
            error("terminated");
