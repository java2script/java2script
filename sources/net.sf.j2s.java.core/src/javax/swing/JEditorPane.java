 ...
 * This will be established by a call to the
 * <a href="#setContentType">setContentType</a> method. If the content is loaded
 * by the <a href="#setPage">setPage</a> method the content type will have been
 * set according to the specification of the URL. It the file is loaded
 * directly, the content type would be expected to have been set prior to
 * loading.
...
//                                    getManagingFocusBackwardTraversalKeys());
	 ...
	 *
	 * @return all of the <code>HyperLinkListener</code>s added or an empty array if
	 ...
	 * event type. This is normally called by the currently installed
	 ...
	 * and if the editor kit for the pane is non-<code>null</code>, then a new
	 * default document is created and the URL is read into it. If the URL contains
	 * and reference location, the location will be scrolled to by calling the
	 * <code>scrollToReference</code> method. If the desired URL is the one
	 * currently being displayed, the document will not be reloaded. To force a
	 * document reload it is necessary to clear the stream description property of
	 * the document. The following code shows how this can be done:
	 *
	 * <pre>
	 ...
	 * <code>getStream</code> method is called to give subclasses control over the
	 * stream provided.
	 * <p>
	 * This may load either synchronously or asynchronously depending upon the
	 * document returned by the <code>EditorKit</code>. If the <code>Document</code>
	 * is of type <code>AbstractDocument</code> and has a value returned by
	 ...
	 * prior to being installed into the editor with a call to
	 * <code>setDocument</code>, which is bound and will fire a property change
	 * event. If an <code>IOException</code> is thrown the partially loaded document
	 * will be discarded and neither the document or page property change events
	 * will be fired. If the document is successfully loaded and installed, a view
	 * will be built for it by the UI which will then be scrolled if necessary, and
	 * then the page property change event will be fired.
	 * <p>
	 * If the document is loaded asynchronously, the document will be installed into
	 * the editor immediately using a call to <code>setDocument</code> which will
	 * fire a document property change event, then a thread will be created which
	 * will begin doing the actual loading. In this case, the page property change
	 * event will not be fired by the call to this method directly, but rather will
	 * be fired when the thread doing the loading has finished. It will also be
	 * fired on the event-dispatch thread. Since the calling thread can not throw an
	 * <code>IOException</code> in the event of failure on the other thread, the
	 * page property change event will be fired when the other thread is done
	 * whether the load was successful or not.
	 *
	 * @param page the URL of the page
	 * @exception IOException for a <code>null</code> or invalid page specification,
	...
	public void setPage(URL page) throws IOException {
		...
		if ((loaded == null) || !loaded.sameFile(page) || (postData != null)) {
			...
			if ((postData == null) || (p < 0)) {
				...
				if (getEditorKit() != null) { // BH SwingJS was just kit != null, but then initial page in TreeDemo does
												// not load
	 ...
	 * <code>HTMLDocument</code>, then it invokes the <code>HTMLEditorKit</code> to
...
//        }
	 ...
	 * case where a <code>ChangedCharSetException</code> is thrown this exception
	 * will contain the new CharSet. Therefore the <code>read</code> operation is
	 * then restarted after building a new Reader with the new charset.
	 *
	...
	class PageLoader extends Thread {
		 ...
		 */
		PageLoader(Document doc, InputStream in, int priority, URL old, URL page) {
		...
		/**
		 * The Document instance to load into. This is cached in case a new Document is
		 * created between the time the thread this is created and run.
		 */
		Document doc;
	}
	...
	static class PageStream extends FilterInputStream {
		...
		/**
		 * Cancel the loading of the stream by throwing an IOException on the next
		 * request.
		 */
	 ...
	 * returns the stream. This can be reimplemented to do useful things like fetch
	 * the stream from a cache, monitor the progress of the stream, etc.
	 * <p>
	 * This method is expected to have the the side effect of establishing the
	 * content type, and therefore setting the appropriate <code>EditorKit</code> to
	 * use for loading the stream.
	 * <p>
	 * If this the stream was an http connection, redirects will be followed and the
	 * resulting URL will be set as the
	 * <code>Document.StreamDescriptionProperty</code> so that relative URL's can be
	 * properly resolved.
	 *
	 * @param page the URL of the page
	 */
...
// TODO SwingJS -- we do not have access to ContentType or ContentEncoding
...
//    }
	 ...
	 * by the <code>UL.getRef</code> method for the URL being displayed). By
	 * default, this method only knows how to locate a reference in an HTMLDocument.
	 * The implementation calls the <code>scrollRectToVisible</code> method to
	 * accomplish the actual scrolling. If scrolling to a reference location is
	 * needed for document types other than HTML, this method should be
	 * reimplemented. This method will have no effect if the component is not
	 * visible.
	 *
...
//            }
	 ...
	 * creation of the document, this will return <code>null</code>, and relative
	 * URL's will not be resolved.
	 *
	 ...
	 * @exception IOException for a <code>null</code> or invalid URL specification
	 */
	 ...
	 * is defined to be the type associated with the currently installed
	 * <code>EditorKit</code>.
	 *
	 ...
	 * <code>getEditorKitForContentType</code>, and then <code>setEditorKit</code>
	 * if an editor kit can be successfully located. This is mostly convenience
	 * method that can be used as an alternative to calling
	 * <code>setEditorKit</code> directly.
	 * <p>
	 * If there is a charset definition specified as a parameter of the content type
	 * specification, it will be used when loading input streams using the
	 * associated <code>EditorKit</code>. For example if the type is specified as
	 * <code>text/html; charset=EUC-JP</code> the content will be loaded using the
	 * <code>EditorKit</code> registered for <code>text/html</code> and the Reader
	 * provided to the <code>EditorKit</code> to load unicode into the document will
	 * use the <code>EUC-JP</code> charset for translating to unicode. If the type
	 * is not recognized, the content will be loaded using the
	 * <code>EditorKit</code> registered for plain text, <code>text/plain</code>.
	 *
	 * @param type the non-<code>null</code> mime type for the content editing
	 *             support
	 * @see #getContentType
	 * @beaninfo description: the type of content
	 * @throws NullPointerException if the <code>type</code> parameter is
	 *                              <code>null</code>
	 */
	 ...
	 * <p>
	 * <em>NOTE: This has the side effect of changing the model, because the
	 * <code>EditorKit</code> is the source of how a particular type of content is
	 * modeled. This method will cause <code>setDocument</code> to be called on
	 * behalf of the caller to ensure integrity of the internal state.</em>
	 *
	...
	public void setEditorKit(EditorKit kit) {
		...
		if (this.kit != null) {
			...
			if (kit instanceof HTMLEditorKit) {
				秘jsHTMLHelper = new JSHTMLHelper(this, (HTMLEditorKit) kit);
			}
	...
	public void setDocument(Document doc) {
		...
		if (秘jsHTMLHelper != null)
			秘jsHTMLHelper.setDocument((HTMLDocument) doc);
	}
	 ...
	 * the component doesn't have an <code>EditorKit</code> registered for the given
	 * type, it will try to create an <code>EditorKit</code> from the default
	 * <code>EditorKit</code> registry. If that fails, a <code>PlainEditorKit</code>
	 * is used on the assumption that all text documents can be represented as plain
	 * text.
	 * <p>
	 * This method can be reimplemented to use some other kind of type registry.
	 * This can be reimplemented to use the Java Activation Framework, for example.
	 *
	 ...
	 * <code>createEditorKitForContentType</code> to install handlers for content
	 * types with a look-and-feel bias.
	 *
	 * @param type the non-<code>null</code> content type
	 ...
	 * <code>null</code>) this amounts to a removal of the current selection. The
	 * replacement text will have the attributes currently defined for input. If the
	 * component is not editable, beep and return.
	 * <p>
	 ...
	 * Use Threads</A> for more information.
	 *
	 * @param content the content to replace the selection with. This value can be
	 *                <code>null</code>
	 */
	@Override
	 ...
	 * kits. The registry is created if necessary. If the registered class has not
	 * yet been loaded, an attempt is made to dynamically load the prototype of the
	 * kit for the given type. If the type was registered with a
	 * <code>ClassLoader</code>, that <code>ClassLoader</code> will be used to load
	 * the prototype. If there was no registered <code>ClassLoader</code>,
	 * <code>Class.forName</code> will be used to load the prototype.
	 * <p>
	 * Once a prototype <code>EditorKit</code> instance is successfully located, it
	 * is cloned and the clone is returned.
	 *
	 * @param type the content type
	 * @return the editor kit, or <code>null</code> if there is nothing registered
	 *         for the given type
	 */
...
//                k = (EditorKit) c.newInstance();
	 ...
	 * loading unwanted classes. The prototype <code>EditorKit</code> will be loaded
	 * with <code>Class.forName</code> when registered with this method.
	 *
	 * @param type      the non-<code>null</code> content type
	 ...
	 * Establishes the default bindings of <code>type</code> to
	 * <code>classname</code>. The class will be dynamically loaded later when
	 * actually needed using the given <code>ClassLoader</code>, and can be safely
	 * changed before attempted uses to avoid loading unwanted classes.
	 *
	 * @param type      the non-<code>null</code> content type
	 * @param classname the class to load later
	...
	/**
	 * Returns the currently registered <code>EditorKit</code> class name for the
	 * type <code>type</code>.
	 *
	 * @param type the non-<code>null</code> content type
	 *
...
//
	...
	/**
	 * This is invoked every time the registries are accessed. Loading is done this
	 * way instead of via a static as the static is only called once when running in
	 * plugin resulting in the entries only appearing in the first applet.
	 *
	 */
...
//			defaultEditorKitMap.put("application/rtf", "javax.swing.text.rtf.RTFEditorKit");
	...
	/**
	 * Returns the preferred size for the <code>JEditorPane</code>. The preferred
	 * size for <code>JEditorPane</code> is slightly altered from the preferred size
	 * of the superclass. If the size of the viewport has become smaller than the
	 * minimum size of the component, the scrollable definition for tracking width
	 * or height will turn to false. The default viewport layout will give the
	 * preferred size, and that is not desired in the case where the scrollable is
	 * tracking. In that case the <em>normal</em> preferred size is adjusted to the
	 * minimum size. This allows things like HTML tables to shrink down to their
	 * minimum size and then be laid out at their minimum size, refusing to shrink
	 * any further.
	 *
	...
	/**
	 * Sets the text of this <code>TextComponent</code> to the specified content,
	 * which is expected to be in the format of the content type of this editor. For
	 * example, if the type is set to <code>text/html</code> the string should be
	 * specified in terms of HTML.
	 * <p>
	 ...
	 * changing out the model, while supporting the content type currently set on
	 * this component. The assumption is that the previous content is relatively
	 * small, and that the previous content doesn't have side effects. Both of those
	 * assumptions can be violated and cause undesirable results. To avoid this,
	 * create a new document, <code>getEditorKit().createDefaultDocument()</code>,
	 * and replace the existing <code>Document</code> with the new one. You are then
	 * assured the previous <code>Document</code> won't have any lingering state.
	 * <ol>
	 * <li>Leaving the existing model in place means that the old view will be torn
	 * down, and a new view created, where replacing the document would avoid the
	 * tear down of the old view.
	 * <li>Some formats (such as HTML) can install things into the document that can
	 * influence future contents. HTML can have style information embedded that
	 * would influence the next content installed unexpectedly.
	 * </ol>
	 * <p>
	 * An alternative way to load this component with a string would be to create a
	 * StringReader and call the read method. In this case the model would be
	 * replaced after it was initialized with the contents of the string.
	 * <p>
	 * This method is thread safe, although most Swing methods are not. Please see
	 * <A HREF=
	 * "http://java.sun.com/docs/books/tutorial/uiswing/misc/threads.html">How to
	 * Use Threads</A> for more information.
	 *
	 * @param t the new text to be set; if <code>null</code> the old text will be
	 *          deleted
	 * @see #getText
	 ...
	 * retrieve the text, <code>null</code> will be returned. This is implemented to
	 * call <code>JTextComponent.write</code> with a <code>StringWriter</code>.
	 *
	 * @return the text
	 * @see #setText
	 */
...
//        return txt;
	 ...
	 *
	 * @return true if a viewport should force the <code>Scrollable</code>'s height
	 *         to match its own, false otherwise
	 */
	@Override
...
//
	...
	/**
	 * Stream currently loading asynchronously (potentially cancelable). Access to
	 * this variable should be synchronized.
	 */
	PageStream loading;
	...
	/**
	 * Returns a string representation of this <code>JEditorPane</code>. This method
...
//            }
		...
		static class PlainParagraph extends javax.swing.text.ParagraphView {
			...
			/**
			 * This class can be used to represent a logical view for a flow. It keeps the
			 * children updated to reflect the state of the model, gives the logical child
			 * views access to the view hierarchy, and calculates a preferred span. It
			 * doesn't do any rendering, layout, or model/view translation.
			 */
...
//                    }
				...
				/**
				 * Forward the DocumentEvent to the given child view. This is implemented to
				 * reparent the child to the logical view (the children may have been parented
				 * by a row in the flow if they fit without breaking) and then execute the
				 * superclass behavior.
				 *
	 ...
	 * sensibly: From a String like: 'timeout=15, max=5' create an array of Strings:
	 * { {"timeout", "15"}, {"max", "5"} } From one like: 'Basic Realm="FuzzFace"
	 * Foo="Biz Bar Baz"' create one like (no quotes in literal): { {"basic", null},
	 * {"realm", "FuzzFace"} {"foo", "Biz Bar Baz"} } keys are converted to lower
