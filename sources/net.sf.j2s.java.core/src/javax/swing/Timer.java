 ...
 * visiting <a
 * href="http://java.sun.com/docs/books/tutorial/uiswing/misc/timer.html" target
 * = "_top">How to Use Timers</a>, a section in <em>The Java Tutorial.</em> For
 * more examples and help in choosing between this <code>Timer</code> class and
 * <code>java.util.Timer</code>, see <a
 * href="http://java.sun.com/products/jfc/tsc/articles/timer/"
 * target="_top">Using Timers in Swing Applications</a>, an article in
...
public class Timer implements Serializable {
	...
	private int html5Id = Integer.MIN_VALUE;

	private long dispatchTime;

	/**
	 * Creates a {@code Timer} and initializes both the initial delay and
	 * between-event delay to {@code delay} milliseconds. If {@code delay} is less
	 * than or equal to zero, the timer fires as soon as it is started. If
	...
	/**
	 * DoPostEvent is a runnable class that fires actionEvents to the listeners on
	 * the EventDispatchThread, via invokeLater.
	 * 
	 * @see Timer#post
	 */
	 ...
	 * can query a <code>Timer</code> instance <code>t</code> for its action
	 ...
	 *         <code><em>Foo</em>Listener</code>s on this timer, or an empty array
	 ...
	 * 
	 * @param flag
	 *          specify <code>false</code> to make the timer stop after sending
	...
	/**
	 * Returns <code>true</code> (the default) if the <code>Timer</code> will send
	 ...
	 * receive a sequence of events with no delay between them. Coalescing avoids
	 * this situation by reducing multiple pending events to a single event.
	 * <code>Timer</code>s coalesce events by default.
	 * 
	 * @param flag
	...
	/**
	 * Starts the <code>Timer</code>, causing it to start sending action events to
	 * its listeners.
	 * 
	 * @see #stop
	 */
	...
	private void dispatch(int msDelay) {
		...
		Runnable r = new Runnable() {
			...
			public void run() {
				...
				if (logTimers)
					System.out.println("timer " + html5Id + " posted at " + postTime);
				post();
			}
		};
		html5Id = JSToolkit.dispatch(r, msDelay, 0);
		dispatchTime = System.currentTimeMillis();
		if (logTimers)
			System.out.println("timer " + html5Id + " dispatched at " + dispatchTime);
	}
	...
	public boolean isRunning() {
		return timerQueue().contains(this);
	}
	...
	private ArrayList<Object> timerQueue() {
		return JSToolkit.getTimerQueue();
	}

	/**
	 * Restarts the <code>Timer</code>, canceling any pending firings and causing
	 ...
	 * its listeners. This does not stop a repeatable Timer from firing again, use
...
// timer ringing before it is posted??
// Boolean.toString does not work
