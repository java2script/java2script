 ...
 * "http://java.sun.com/docs/books/tutorial/uiswing/overview/hierarchy.html"
 ...
 * from {@link javax.swing.plaf.ComponentUI}. See <a
 * href="http://java.sun.com/docs/books/tutorial/uiswing/misc/plaf.html">How to
 * Set the Look and Feel</a> in <em>The Java Tutorial</em> for more information.
 ...
 * >Keyboard Bindings in Swing</a>, an article in <em>The Swing Connection</em>,
 ...
 * for borders. For more information see <a
 * href="http://java.sun.com/docs/books/tutorial/uiswing/overview/draw.html"
 * >Painting</a> and <a
 * href="http://java.sun.com/docs/books/tutorial/uiswing/misc/border.html">How
 * to Use Borders</a>, both of which are sections in <em>The Java Tutorial</em>.
 * </ul>
 * For more information on these subjects, see the <a
 * href="package-summary.html#package_description">Swing package description</a>
 * and <em>The Java Tutorial</em> section <a href=
 ...
 * see <a
 * href="http://java.sun.com/docs/books/tutorial/uiswing/misc/focus.html"> How
 * to Use the Focus Subsystem</a>, a section in <em>The Java Tutorial</em>.
 * <p>
...
public abstract class JComponent extends Container {
	...
	/**
	 * Key used to look up a value from the AppContext to determine the JComponent
	 * the InputVerifier is running for. That is, if
	 ...
	 * initiated the painting operation. If <code>paintingChild</code> is opaque,
	 * no need to paint any child components after <code>paintingChild</code>.
	...
	/**
	 * The comment to display when the cursor is over the component, also known as
	 * a "value tip", "flyover help", or "flyover label".
	 */
...
//		}
	 ...
	 * subclasses that are implemented as a number of <code>JComponent</code>s may
	 * set this to true.
	 * <p>
	 ...
	 * is responsible for registering bindings and adding the necessary listeners
	 * such that the <code>JPopupMenu</code> will be shown at the appropriate
	 * time. When the <code>JPopupMenu</code> is shown depends upon the look and
	 * feel: some may show it on a mouse event, some may enable a key binding.
	 * <p>
	 * If <code>popup</code> is null, and <code>getInheritsPopupMenu</code>
	 * returns true, then <code>getComponentPopupMenu</code> will be delegated to
	 * the parent. This provides for a way to make all child components inherit
...
//			LookAndFeel.installProperty(this, "focusTraversalKeysBackward", getManagingFocusBackwardTraversalKeys());
	 ...
	 * <code>DebugGraphics</code> object if necessary. Otherwise we just configure
	 * the specified graphics object's foreground and font.
	 * 
	 * @param g
	 ...
	 * <code>null</code>. We pass the delegate a copy of the <code>Graphics</code>
	 * object to protect the rest of the paint code from irrevocable changes (for
	 * example, <code>Graphics.translate</code>).
	 * <p>
...
//			((JSComponentUI)ui).setPainted(g);
	...
	public void paint(Graphics g) {
		...
		try {
			...
			// SwingJS -- we are not going to worry about components having special
			// clipping
			...
			// for instance, will paint into the rectangle associated with that
			// button.
			...
			// if (clipRect == null) {
			// SwingJS not clipping for performance co.setClip(clipX, clipY, clipW,
			// clipH);
			// }
	 ...
	 * <code>printChildren</code>. It is recommended that you override one of the
	 * previously mentioned methods rather than this one if your intention is to
	 * customize the way printing looks. However, it can be useful to override
	 * this method should you want to prepare state before invoking the superclass
	 * behavior. As an example, if you wanted to change the component's background
	 * color before printing, you could do the following:
	 * 
	 * <pre>
	 ...
	 * you can query during painting whether or not the component is in the midst
	 * of a print operation. The <code>isPaintingForPrint</code> method provides
	 ...
	 * <p>
	 * This method sets the component's state such that the double buffer will not
	 * be used: painting will be done directly on the passed in
	 * <code>Graphics</code>.
	 * 
	 * @param g
	 *          the <code>Graphics</code> context in which to paint
	 ...
	 * <code>paintComponent</code> on the component. Override this if you wish to
	 * add special painting behavior when printing.
	 * 
	 * @param g
	 *          the <code>Graphics</code> context in which to paint
	 * @see #print
	...
	/**
	 * Returns true if the component is currently painting a tile. If this method
	 * returns true, paint will be called again for another tile. This method
	 * returns false if you are not painting a tile or if the last tile is
	 * painted. Use this method to keep some state you might need between tiles.
	 * 
	 ...
	 * component is part of a <code>print</code> operation. This method is useful
	 * when you want to customize what you print versus what you show on the
	 * screen.
	 * <p>
	 ...
	 * <p>
	 * Note: This method provides complimentary functionality to that provided by
	 * other high level Swing printing APIs. However, it deals strictly with
	 ...
	 * operation doesn't necessarily result in a continuous rendering of the full
	 * component, and the return value of this method can change multiple times
	 * during that operation. It is even possible for the component to be painted
	 * to the screen while the printing process is ongoing. In such a case, the
	 * return value of this method is <code>true</code> when, and only when, the
	 * table is being painted as part of the printing process.
	 * 
	 * @return true if the current painting operation on this component is part of
	 *         a print operation
	 * @see #print
	 * @since 1.6
	 */
	 ...
	 * to Use the Focus Subsystem</a>, a section in <em>The Java Tutorial</em>.
	 * <p>
	 * Changes this <code>JComponent</code>'s focus traversal keys to CTRL+TAB and
	 * CTRL+SHIFT+TAB. Also prevents <code>SortingFocusTraversalPolicy</code> from
	 * considering descendants of this JComponent when computing a focus traversal
	 * cycle.
	 * 
	 ...
	 * <code>JComponent</code>'s focus traversal cycle by unconditionally setting
	 * the specified <code>Component</code> as the next <code>Component</code> in
	 * the cycle, and this <code>JComponent</code> as the specified
	 * <code>Component</code>'s previous <code>Component</code> in the cycle.
	 * 
	 * @param aComponent
	 ...
	 * mouse operations, but not keyboard operations. For example, look and feels
	 * could verify this property is true before requesting focus during a mouse
	 * operation. This would often times be used if you did not want a mouse press
	 * on a <code>JComponent</code> to steal focus, but did want the
	...
	/**
	 * Returns <code>true</code> if this <code>JComponent</code> should get focus;
	 ...
	 * more information on focus, see <a
	 * href="http://java.sun.com/docs/books/tutorial/uiswing/misc/focus.html"> How
	 * to Use the Focus Subsystem</a>, a section in <em>The Java Tutorial</em>.
	 * 
	 ...
	 * Component.requestFocus(boolean)} for a complete description of this method.
	 * <p>
	 * Note that the use of this method is discouraged because its behavior is
	 * platform dependent. Instead we recommend the use of
	 * {@link #requestFocusInWindow(boolean) requestFocusInWindow(boolean)}. If
	 * you would like more information on focus, see <a
	 * href="http://java.sun.com/docs/books/tutorial/uiswing/misc/focus.html"> How
	 * to Use the Focus Subsystem</a>, a section in <em>The Java Tutorial</em>.
	 * 
	 * @param temporary
	 ...
	 * method.
	 * <p>
	 * If you would like more information on focus, see <a
	 * href="http://java.sun.com/docs/books/tutorial/uiswing/misc/focus.html"> How
	 * to Use the Focus Subsystem</a>, a section in <em>The Java Tutorial</em>.
	 * 
	 * @return <code>false</code> if the focus change request is guaranteed to
	 ...
	 * Component.requestFocusInWindow(boolean)} for a complete description of this
	 * method.
	 * <p>
	 * If you would like more information on focus, see <a
	 * href="http://java.sun.com/docs/books/tutorial/uiswing/misc/focus.html"> How
	 ...
	 * displayable, visible, and focusable for the request to be granted.
	 * <p>
	...
	/**
	 * Returns the value that indicates whether the input verifier for the current
	 * focus owner will be called before this component requests focus.
	 * 
	 ...
	 * general, when you want to set a border on a standard Swing component other
	 * than <code>JPanel</code> or <code>JLabel</code>, we recommend that you put
	 * the component in a <code>JPanel</code> and set the border on the
	 * <code>JPanel</code>.
	 * <p>
	 * This is a bound property.
	 * 
	...
	public void setBorder(Border border) {
		...
		if (border != oldBorder) {
			if (border == null
					|| oldBorder == null
					|| !(border.getBorderInsets(this).equals(oldBorder
	 ...
	 * <code>getActionMap()</code> and <code>getInputMap()</code> for similiar
	 ...
	 * the <code>getInputMap</code> method that takes an integer.
	 * <p>
	 * Register a new keyboard action. <code>anAction</code> will be invoked if a
	 * key event matching <code>aKeyStroke</code> occurs and
	 ...
	 * <p>
	 * The <code>aCommand</code> will be set in the delivered event if specified.
	 * <p>
	 ...
	 * <DD>The action will be invoked when the keystroke occurs while the
	 * component has the focus or if the component is an ancestor of the component
	 * that has the focus.
	 * </DL>
	 * </blockquote>
	 * <p>
	 ...
	 * (using the KeyStroke class) and direct to a parent or child of a component
	 * that has the focus, or to the component itself. In other words, in any
	 * hierarchical structure of components, an arbitrary key-combination can be
	 * immediately directed to the appropriate component in the hierarchy, and
	 * cause a specific method to be invoked (usually by way of adapter objects).
	 * <p>
	 * If an action has already been registered for the receiving container, with
	 * the same charCode and the same modifiers, <code>anAction</code> will
	 * replace the action.
	 * 
	 * @param anAction
	 ...
	 * place a dummy binding the <code>InputMap</code>. Removing the binding from
	 * the <code>InputMap</code> allows bindings in parent <code>InputMap</code>s
	 * to be active, whereas putting a dummy binding in the <code>InputMap</code>
	 * effectively disables the binding from ever happening.
	 * <p>
	 * Unregisters a keyboard action. This will remove the binding from the
	 * <code>ActionMap</code> (if it exists) as well as the <code>InputMap</code>
	 * s.
	 */
	...
	public KeyStroke[] getRegisteredKeyStrokes() {
		...
		for (int counter = 0; counter < 3; counter++) {
			...
			counts[counter] = (strokes[counter] != null) ? strokes[counter].length
					: 0;
		}
	...
	/**
	 * Returns the condition that determines whether a registered action occurs in
	 * response to the specified keystroke.
	 * <p>
	 ...
	 * <code>ActionMap</code>. This has the effect of removing any local bindings,
	 * and allowing the bindings defined in parent
	 ...
	 *              <code>WHEN_IN_FOCUSED_WINDOW</code> and <code>map</code> is
	 *              not an instance of <code>ComponentInputMap</code>; or if
	 *              <code>condition</code> is not one of the legal values
	 *              specified above
	 * @since 1.3
	 */
	 ...
	 * parent of the <code>am</code> to be the <code>ActionMap</code> from the UI
	 ...
	 * <code>Action</code> to fire for particular <code>KeyStroke</code> binding.
	 * The returned <code>ActionMap</code>, unless otherwise set, will have the
	 * <code>ActionMap</code> from the UI set as the parent.
	 * 
	 ...
	 *          created
	 * @return the <code>InputMap</code> for the given <code>condition</code>; if
	 *         <code>create</code> is false and the <code>InputMap</code> hasn't
	 *         been created, returns <code>null</code>
	 * @exception IllegalArgumentException
	 ...
	 * 
	 * @param create
	 *          if true, create the <code>ActionMap</code> if it is not already
	 *          created
	 ...
	 * name. If this component does not have a <code>ComponentUI</code> -1 will be
	 * returned. If a value &gt;= 0 is returned, then the component has a valid
	 ...
	 * calculated return <code>BaselineResizeBehavior.OTHER</code>. Callers should
	 * first ask for the baseline using <code>getBaseline</code> and if a value
	 * &gt;= 0 is returned use this method. It is acceptable for this method to
	 * return a value other than <code>BaselineResizeBehavior.OTHER</code> even if
	 ...
	 * <code>FocusTraversalPolicy</code>'s default <code>Component</code>. If this
	 * <code>JComponent</code> is a focus cycle root, then its
	...
	/**
	 * Sets whether or not this component is enabled. A component that is enabled
	 * may respond to user input, while a component that is not enabled cannot
	 * respond to user input. Some components may alter their visual
	...
	/**
	 * Sets the foreground color of this component. It is up to the look and feel
	 * to honor this property, some may choose to ignore it.
	 * 
	 * @param fg
	 ...
	 * <code>JComponent</code> or <code>ComponentUI</code> implementations. Direct
	 * subclasses of <code>JComponent</code> must override
	 ...
	 * 
	 * The default locale has "AppContext" scope so that applets (and potentially
	 * multiple lightweight applications running in a single VM) can have their
	 * own setting. An applet can safely alter its default locale because it will
	 * have no affect on other applets (or the browser).
	 * 
	 ...
	 * called after the focus manager and any interested listeners have been given
	 * a chance to steal away the event. This method is called only if the event
	 * has not yet been consumed. This method is called prior to the keyboard UI
	 * logic.
	 * <p>
	 ...
	 * <code>ActionMap</code>, and then (if the action is found and the component
	...
	boolean processKeyBindings(KeyEvent e, boolean pressed) {
		...
		/*
		 * We have no key binding. Let's try the path from our parent to the window
		 * excluded. We store the path components so we can avoid asking the same
		 * component twice.
		 */
		...
		/*
		 * No components between the focused component and the window is actually
		 * interested by the key event. Let's try the other JComponent in this
		 * window.
		 */
		if (parent != null) {
	...
	static boolean processKeyBindingsForAllComponents(KeyEvent e,
		...
		while (true) {
			if (KeyboardManager.getCurrentManager().fireKeyboardAction(e, pressed,
					container)) {
				return true;
			}
	 ...
	 * component's coordinate system. It is up to the look and feel to honor this
	 * property, some may choose to ignore it. If {@code null}, the look and feel
	 * will choose a suitable location.
	 * 
	 * @param event
	 *          the {@code MouseEvent} that triggered the popup to be shown, or
	 *          {@code null} if the popup is not being shown as the result of a
	 *          mouse event
	 ...
	 * but it can be used to cause different tooltips to be displayed differently.
	 * 
	 ...
	 * <code>JComponent</code>'s parent. Components that can service the request,
	 * such as <code>JViewport</code>, override this method and perform the
	 * scrolling.
	 * 
	 * @param aRect
	 ...
	 * button continues to be held down). The synthetic events make it appear that
	 * the drag gesture has resumed in the direction established when the
	 ...
	 * <code>JTextArea</code>, and <code>JEditorPane</code>) already handle mouse
	 * dragged events in this way. To enable autoscrolling in any other component,
	 * add a mouse motion listener that calls <code>scrollRectToVisible</code>.
	...
	static class KeyboardState {
		...
		/**
		 * Updates internal state of the KeyboardState and returns true if the event
	 ...
	 * <p>
	 * The <code>clientProperty</code> dictionary is not intended to support large
	 * scale extensions to JComponent nor should be it considered an alternative
	 * to subclassing when designing a new component.
	 * 
	 * @param key
	 *          the new client property key
	 * @param value
	 *          the new client property value; if <code>null</code> this method
	...
	/**
	 * Sets the look and feel delegate for this component. <code>JComponent</code>
	 * subclasses generally override this method to narrow the argument type. For
	 * example, in <code>JSlider</code>:
	 * 
	 * <pre>
...
//		}
	...
	/**
	 * Returns true if this component is lightweight, that is, if it doesn't have
	 * a native window system peer.
	 * 
	 ...
	 * considered, more distant descendants are ignored. A <code>JComponent</code>
	 * is opaque if <code>JComponent.isOpaque()</code> returns true, other
	 * lightweight components are always considered transparent, and heavyweight
	 * components are always considered opaque.
	 * 
	 * @param x
	 ...
	 * intersection of the visible rectangles for the component <code>c</code> and
	 * all of its ancestors. The return value is stored in
	 ...
	 *          visible rectangles for the component <code>c</code> and all of its
	 *          ancestors -- this is the return value for this method
	 * @see #getVisibleRect
	 */
	static final void computeVisibleRect(Component c, Rectangle visibleRect) {
		...
		if (p == null || p.isWindowOrJSApplet()) {
			...
			SwingUtilities.computeIntersection(0, 0, bounds.width, bounds.height,
					visibleRect);
		}
	}
	...
	/**
	 * Returns the <code>Component</code>'s "visible rectangle" - the intersection
	 * of this component's visible rectangle,
	 ...
	 * 
	 * @return the top-level <code>Container</code> that this component is in, or
	 ...
	 * can query a <code>JComponent</code> <code>c</code> for its mouse listeners
...
//		System.out.println(">>>addnotify for " + getUIClassID() + " " +  this);
	...
	/**
	 * Notifies this component that it no longer has a parent component. When this
	 * method is invoked, any <code>KeyboardAction</code>s set up in the the chain
	 * of parent components are removed.
	 * 
	 * @see #registerKeyboardAction
	 */
	 ...
	 * showing. The component will be repainted after all of the currently pending
	 ...
	 * value changes such that size, location, or internal layout of this
	 * component has been affected. This automatic updating differs from the AWT
	 * because programs generally no longer need to invoke <code>validate</code>
...
//					}
	 ...
	 * validated. Returns false by default. <code>JScrollPane</code> overrides
	 ...
	 * 
	 * @return true if painting should originate from this Component or one of its
	 *         ancestors.
	 */
	 ...
	 * collapse redundant requests into a single paint call. This method is useful
	 * if one needs to update the display while the current event is being
	 * dispatched.
	 * 
	 * @param x
...
//			if (!ontop) {
		...
		if (c == null || // c.getPeer() == null ||
				paintImmediatelyClip.width <= 0 || paintImmediatelyClip.height <= 0) {
			recycleRectangle(paintImmediatelyClip);
			return;
		}
...
//				}
	 ...
	 * not adjust the Graphics in anyway, callers must do that first. This method
	 * is package-private for RepaintManager.PaintManager and its subclasses to
	 * call, it is NOT intended for general use outside of that.
	 */
	...
	/**
	 * Returns whether or not the region of the specified component is obscured by
	 * a sibling.
	 * 
	 * @return NOT_OBSCURED if non of the siblings above the Component obscure it,
	 *         COMPLETELY_OBSCURED if one of the siblings completely obscures the
	 *         Component or PARTIALLY_OBSCURED if the Comonent is only partially
	 *         obscured.
	 */
	private int getObscuredState(int compIndex, int x, int y, int width,
		...
		for (int i = compIndex - 1; i >= 0; i--) {
			...
			if (opaque && x >= siblingRect.x
					&& (x + width) <= (siblingRect.x + siblingRect.width)
					&& y >= siblingRect.y
					&& (y + height) <= (siblingRect.y + siblingRect.height)) {
				recycleRectangle(tmpRect);
				return COMPLETELY_OBSCURED;
			} else if (retValue == NOT_OBSCURED
					&& !((x + width <= siblingRect.x) || (y + height <= siblingRect.y)
	 ...
	 * method is intended to be used only for debugging purposes, and the content
