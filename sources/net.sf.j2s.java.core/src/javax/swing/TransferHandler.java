...
public class TransferHandler {
	...
	public static class DropLocation {
		 ...
		 *
		 * @param dropPoint the drop point, representing the mouse's current location
	 ...
	 * <p>
	 * Developers typically need not create instances of this class. Instead, they
	 * are something provided by the DnD implementation to certain methods in
	...
	public final static class TransferSupport {
		...
		/**
		 * The source is a {@code DropTargetDragEvent} or {@code DropTargetDropEvent}
		 * for drops, and a {@code Transferable} otherwise
		 */
		...
		/**
		 * Create a <code>TransferSupport</code> with <code>isDrop()</code>
		 * <code>false</code> for the given component and <code>Transferable</code>.
		 *
		 * @param component    the target component
...
//            }
			...
			/*
			 * The drop location may be null at this point if the component doesn't return
			 * custom drop locations. In this case, a point-only drop location will be
			 * created lazily when requested.
			 */
		}

		/**
		 * Returns whether or not this <code>TransferSupport</code> represents a drop
		 * operation.
		 *
		 * @return <code>true</code> if this is a drop operation, <code>false</code>
		 *         otherwise.
		 */
		...
		/**
		 * Checks that this is a drop and throws an {@code IllegalStateException} if it
		 * isn't.
		 *
		...
		/**
		 * Returns the current (non-{@code null}) drop location for the component, when
		 * this {@code TransferSupport} represents a drop.
		 * <p>
		 * Note: For components with built-in drop support, this location will be a
		 * subclass of {@code DropLocation} of the same type returned by that
		 * component's {@code getDropLocation} method.
		 * <p>
		 * This method is only for use with drag and drop transfers. Calling it when
		 * {@code isDrop()} is {@code false} results in an
		...
		public DropLocation getDropLocation() {
			...
			if (dropLocation == null) {
				/*
				 * component didn't give us a custom drop location, so lazily create a
				 * point-only location
				 */
				Point p = source instanceof DropTargetDragEvent ? ((DropTargetDragEvent) source).getLocation()
						: ((DropTargetDropEvent) source).getLocation();
		 ...
		 * transfer - which must represent a drop. This is applicable to those
		 * components that automatically show the drop location when appropriate during
		 * a drag and drop operation). By default, the drop location is shown only when
		 * the {@code TransferHandler} has said it can accept the import represented by
		 * this {@code TransferSupport}. With this method you can force the drop
		 * location to always be shown, or always not be shown.
		 * <p>
		 * This method is only for use with drag and drop transfers. Calling it when
		 * {@code isDrop()} is {@code false} results in an
		 * {@code IllegalStateException}.
		 ...
		 * supported by the source's drop actions, and must be one of {@code COPY},
		 * {@code MOVE} or {@code LINK}.
		 * <p>
		 * This method is only for use with drag and drop transfers. Calling it when
		 * {@code isDrop()} is {@code false} results in an
		 * {@code IllegalStateException}.
		...
		/**
		 * Returns the action chosen for the drop, when this {@code TransferSupport}
		 * represents a drop.
		 * <p>
		 * Unless explicitly chosen by way of {@code setDropAction}, this returns the
		 * user drop action provided by {@code getUserDropAction}.
		 * <p>
		 * You may wish to query this in {@code TransferHandler}'s {@code importData}
		...
		/**
		 * Returns the user drop action for the drop, when this {@code TransferSupport}
		 * represents a drop.
		 * <p>
		 * The user drop action is chosen for a drop as described in the documentation
		 * for {@link java.awt.dnd.DropTargetDragEvent} and
		 * {@link java.awt.dnd.DropTargetDropEvent}. A different action may be chosen as
		 * the drop action by way of the {@code setDropAction} method.
		 * <p>
		 * You may wish to query this in {@code TransferHandler}'s {@code canImport}
		 * method when determining the suitability of a drop or when deciding on a drop
		 * action to explicitly choose.
		 * <p>
		 * This method is only for use with drag and drop transfers. Calling it when
		 * {@code isDrop()} is {@code false} results in an
		 * {@code IllegalStateException}.
		 ...
		 * <p>
		 * The source actions represent the set of actions supported by the source of
		 * this transfer, and are represented as some bitwise-OR combination of
		 * {@code COPY}, {@code MOVE} and {@code LINK}. You may wish to query this in
		 * {@code TransferHandler}'s {@code canImport} method when determining the
		 * suitability of a drop or when deciding on a drop action to explicitly choose.
		 * To determine if a particular action is supported by the source, bitwise-AND
		 * the action with the source drop actions, and then compare the result against
		 * the original action. For example:
		 * 
		 * <pre>
		 ...
		 * <p>
		 * Note: Unless it is necessary to fetch the <code>Transferable</code> directly,
		 * use one of the other methods on this class to inquire about the transfer.
		 * This may perform better than fetching the <code>Transferable</code> and
		 * asking it directly.
		 *
	...
	/**
	 * Returns an {@code Action} that performs cut operations to the clipboard. When
	 * performed, this action operates on the {@code JComponent} source of the
	 * {@code ActionEvent} by invoking {@code exportToClipboard}, with a
	 ...
	 * When performed, this action operates on the {@code JComponent} source of the
	 * {@code ActionEvent} by invoking {@code exportToClipboard}, with a
	 ...
	 * When performed, this action operates on the {@code JComponent} source of the
	 * {@code ActionEvent} by invoking {@code importData}, with the clipboard
	 * contents, on the component's {@code TransferHandler}.
	 *
	 ...
	 * UI implementations in the <code>javax.swing.plaf.basic</code> package if the
	 * dragEnabled property is set on the component. This can be called by custom UI
	 * implementations to use the Swing drag support. This method can also be called
	 * by a Swing extension written as a subclass of <code>JComponent</code> to take
	 * advantage of the Swing drag support.
	 * <p>
	 * The transfer <em>will not necessarily</em> have been completed at the return
	 * of this call (i.e. the call does not block waiting for the drop). The
	 * transfer will take place through the Swing implementation of the
	 ...
	 * @param action the transfer action initially requested; either {@code COPY},
	 ...
	 * mechanism, requiring no further effort from the developer. Any data transfer
	 * <em>will</em> be complete and the <code>exportDone</code> method will be
	 * called with the action that occurred, before this method returns. Should the
	 * clipboard be unavailable when attempting to place data on it, the
	 * <code>IllegalStateException</code> thrown by
	 * {@link Clipboard#setContents(Transferable, ClipboardOwner)} will be
	 * propogated through this method. However, <code>exportDone</code> will first
	 * be called with an action of <code>NONE</code> for consistency.
	 *
	 * @param comp   the component holding the data to be transferred; provided to
	 *               enable sharing of <code>TransferHandler</code>s
	 * @param clip   the clipboard to transfer the data into
	 * @param action the transfer action requested; this should be a value of either
	 *               <code>COPY</code> or <code>MOVE</code>; the operation performed
	 *               is the intersection of the transfer capabilities given by
	 *               getSourceActions and the requested action; the intersection may
	 *               result in an action of <code>NONE</code> if the requested
	 ...
	 * <code>Transferable</code> to be imported and the component to transfer to are
	 * contained within the <code>TransferSupport</code>.
	 * <p>
	 * While the drag and drop implementation calls {@code canImport} to determine
	 * the suitability of a transfer before calling this method, the implementation
	 * of paste does not. As such, it cannot be assumed that the transfer is
	 * acceptable upon a call to this method for paste. It is recommended that
	 * {@code canImport} be explicitly called to cover this case.
	 * <p>
	 * Note: The <code>TransferSupport</code> object passed to this method is only
	 * valid for the duration of the method call. It is undefined what values it may
	 * contain after this method returns.
	 *
	 ...
	 * The <code>Transferable</code> represents the data to be imported into the
	 * component.
	 * <p>
	 * Note: Swing now calls the newer version of <code>importData</code> that takes
	 * a <code>TransferSupport</code>, which in turn calls this method (if the
	 * component in the {@code TransferSupport} is a {@code JComponent}). Developers
	 * are encouraged to call and override the newer version as it provides more
	 * information (and is the only version that supports use with a
	 * {@code TransferHandler} set directly on a {@code JFrame} or other
	 * non-{@code JComponent}).
	 *
	 * @param comp the component to receive the transfer; provided to enable sharing
	 *             of <code>TransferHandler</code>s
...
//                    MethodUtil.invoke(writer, comp, args);
	...
	/**
	 * This method is called repeatedly during a drag and drop operation to allow
	 * the developer to configure properties of, and to return the acceptability of
	 * transfers; with a return value of {@code true} indicating that the transfer
	 * represented by the given {@code TransferSupport} (which contains all of the
	 * details of the transfer) is acceptable at the current time, and a value of
	 * {@code false} rejecting the transfer.
	 * <p>
	 * For those components that automatically display a drop location during drag
	 * and drop, accepting the transfer, by default, tells them to show the drop
	 * location. This can be changed by calling {@code setShowDropLocation} on the
	 * {@code TransferSupport}.
	 * <p>
	 * By default, when the transfer is accepted, the chosen drop action is that
	 * picked by the user via their drag gesture. The developer can override this
	 * and choose a different action, from the supported source actions, by calling
	 * {@code setDropAction} on the {@code TransferSupport}.
	 * <p>
	 ...
	 * Upon a drop, {@code canImport} is called one final time before calling into
	 * {@code importData}. Any state set on the {@code TransferSupport} during that
	 * last call will be available in {@code importData}.
	 * <p>
	 * This method is not called internally in response to paste operations. As
	 * such, it is recommended that implementations of {@code importData} explicitly
	 * call this method for such cases and that this method be prepared to return
	 * the suitability of paste operations as well.
	 * <p>
	 * Note: The <code>TransferSupport</code> object passed to this method is only
	 * valid for the duration of the method call. It is undefined what values it may
	 * contain after this method returns.
	...
	/**
	 * Indicates whether a component will accept an import of the given set of data
	 * flavors prior to actually attempting to import it.
	 * <p>
	 * Note: Swing now calls the newer version of <code>canImport</code> that takes
	 * a <code>TransferSupport</code>, which in turn calls this method (only if the
	 * component in the {@code TransferSupport} is a {@code JComponent}). Developers
	 * are encouraged to call and override the newer version as it provides more
	 * information (and is the only version that supports use with a
	 * {@code TransferHandler} set directly on a {@code JFrame} or other
	 * non-{@code JComponent}).
	 *
	 * @param comp            the component to receive the transfer; provided to
	 *                        enable sharing of <code>TransferHandler</code>s
	 ...
	 * <p>
	 * Some models are not mutable, so a transfer operation of {@code MOVE} should
	 * not be advertised in that case. Returning {@code NONE} disables transfers
	 * from the component.
	 *
	 * @param c the component holding the data to be transferred; provided to enable
	 *          sharing of <code>TransferHandler</code>s
	 * @return {@code COPY} if the transfer property can be found, otherwise returns
	 *         <code>NONE</code>
	 */
	 ...
	 * both providing feedback while performing a drag operation and for
	 * representing the transfer in a clipboard implementation that has a visual
	 * appearance. The implementation of the <code>Icon</code> interface should not
	 * alter the graphics clip or alpha level. The icon implementation need not be
	 * rectangular or paint all of the bounding rectangle and logic that calls the
	 * icons paint method should not assume the all bits are painted.
	 * <code>null</code> is a valid return value for this method and indicates there
	 * is no visual representation provided. In that case, the calling logic is free
	 * to represent the transferable however it wants.
	 * <p>
	 * The default Swing logic will not do an alpha blended drag animation if the
	 * return is <code>null</code>.
	 *
	 ...
	 *         <code>null</code> if the property associated with <code>c</code> is
	 *         <code>null</code>
	 *
	 */
	 ...
	 * was transferred if the action was <code>MOVE</code>.
	 * <p>
	 * This method is implemented to do nothing since <code>MOVE</code> is not a
	 * supported action of this implementation (<code>getSourceActions</code> does
	 * not include <code>MOVE</code>).
	 *
	 ...
	 * handler on the given component (transfer handler may be shared). This returns
	 * <code>null</code> if the property descriptor can't be found or there is an
	 * error attempting to fetch the property descriptor.
	 */
...
//        return null;
	...
	/**
	 * Fetches the data flavor from the array of possible flavors that has data of
	 * the type represented by property type. Null is returned if there is no match.
	 */
	...
	static class PropertyTransferable implements Transferable {
		...
		/**
		 * Returns an array of <code>DataFlavor</code> objects indicating the flavors
		 * the data can be provided in. The array should be ordered according to
		 * preference for providing the data (from most richly descriptive to least
		 * descriptive).
		 * 
		 ...
		 * @return true if this <code>DataFlavor</code> is supported, otherwise false
		 */
		@Override
...
//                value = MethodUtil.invoke(reader, component, (Object[])null);
	 ...
	 * set. This class sets the one listener as the linkage of drop handling to the
	 * <code>TransferHandler</code>, and adds support for additional listeners which
	 * some of the <code>ComponentUI</code> implementations install to manipulate a
	 * drop insertion location.
	 */
	...
	private static class DropHandler implements DropTargetListener, Serializable, ActionListener {
		...
		/**
		 * Update the geometry of the autoscroll region. The geometry is maintained as a
		 * pair of rectangles. The region can cause a scroll if the pointer sits inside
		 * it for the duration of the timer. The region that causes the timer countdown
		 * is the area between the two rectangles.
		 * <p>
		 * This is implemented to use the visible area of the component as the outer
		 * rectangle, and the insets are fixed at 10. Should the component be smaller
		 * than a total of 20 in any direction, autoscroll will not occur in that
		 * direction.
		 */
		...
		/**
		 * Perform an autoscroll operation. This is implemented to scroll by the unit
		 * increment of the Scrollable using scrollRectToVisible. If the cursor is in a
		 * corner of the autoscroll region, more than one axis will scroll.
		 */
		...
		/**
		 * Initializes the internal properties if they haven't been already inited. This
		 * is done lazily to avoid loading of desktop properties.
		 */
		...
		/**
		 * The timer fired, perform autoscroll if the pointer is within the autoscroll
		 * region.
		 * <P>
		 * 
...
//                }
		...
		@Override
		public void drop(DropTargetDropEvent e) {
			// added in case dragOver is not triggered
			component = e.getDropTargetContext().getComponent();
			TransferHandler importer = ((HasGetTransferHandler) component).getTransferHandler();
		...
		public void dropActionChanged(DropTargetDragEvent e) {
			/*
			 * Work-around for Linux bug where dropActionChanged is called before dragEnter.
			 */
			if (component == null) {
				return;
			}
...
//            return clipboard;
		...
		/**
		 * Key used in app context to lookup Clipboard to use if access to System
		 * clipboard is denied.
		 */
