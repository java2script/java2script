...
public class AudioInputStream extends InputStream {
	 ...
	 * 
	 * @param stream
	 *          the stream on which this <code>AudioInputStream</code> object is
	 *          based
	 * @param format
	...
	/**
	 * 
	 *            Reads some number of bytes from the audio input stream and
	 *            stores them into the buffer array <code>b</code>. The number of
	 *            bytes actually read is returned as an integer. This method
	 ...
	 *            the length of the array is not an integral number of frames, a
	 *            maximum of <code>b.length - (b.length % frameSize)
	...
	public int read(byte[] b, int off, int len) throws IOException {
		...
		if (bytesRead > 0) {
			...
			if (pushBackLen > 0) {
				...
				System.arraycopy(b, off + bytesRead - pushBackLen, pushBackBuffer, 0,
						pushBackLen);
				bytesRead -= pushBackLen;
			}
	 ...
	 * next invocation of a <code>read</code> or <code>skip</code> method for this
	 * audio input stream; the limit can vary each time these methods are invoked.
	 ...
	 * 
	 * @return <code>true</code> if this stream supports the <code>mark</code> and
	 *         <code>reset</code> methods; <code>false</code> otherwise
	 * @see #mark
	 * @see #reset
	 */
	@Override
