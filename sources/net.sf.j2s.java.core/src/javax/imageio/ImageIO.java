...
public final class ImageIO {
	...
	static class CacheInfo {
		boolean useCache = false; // SwingJS
		...
		public File getCacheDirectory() {
			return null;// return cacheDirectory;
		}
	 ...
	 * creating <code>ImageInputStream</code>s and <code>ImageOutputStream</code>
	 * s.
	 * 
	 * <p>
	 * When reading from a standard <code>InputStream</code>>, it may be necessary
	 * to save previously read information in a cache since the underlying stream
	 * does not allow data to be re-read. Similarly, when writing to a standard
	 * <code>OutputStream</code>, a cache may be used to allow a previously
	 * written value to be changed before flushing it to the final destination.
	 * 
	 * <p>
	 * The cache may reside in main memory or on disk. Setting this flag to
	 * <code>false</code> disallows the use of disk for future streams, which may
	 * be advantageous when working with small images, as the overhead of creating
	 * and destroying files is removed.
	 * 
	 * <p>
	 * On startup, the value is set to <code>true</code>.
	 * 
	 * @param useCache
	 *          a <code>boolean</code> indicating whether a cache file should be
	 *          used, in cases where it is optional.
	 * 
	 * @see #getUseCache
	 */
	 ...
	 * temporary-file directory is to be used. If <code>getUseCache</code> returns
	 ...
	 *              if <code>cacheDir</code> is non-<code>null</code> but is not a
	 *              directory.
	 * 
	 ...
	 * 
	 * @return a <code>File</code> indicating the directory where cache files will
	 *         be created, or <code>null</code> to indicate the system-dependent
	...
	// SwingJS TODO // ImageOutputStreams
	//
	// /**
	 ...
	 * <code>InputStream</code> is wrapped in an <code>ImageInputStream</code>. If
	 * no registered <code>ImageReader</code> claims to be able to read the
	 * resulting stream, <code>null</code> is returned.
	 * 
	 * <p>
...
//	}
	...
	/**
	 * Returns a <code>BufferedImage</code> as the result of decoding a supplied
	 * <code>URL</code> with an <code>ImageReader</code> chosen automatically from
	 * among those currently registered. An <code>InputStream</code> is obtained
	 * from the <code>URL</code>, which is wrapped in an
	 ...
	 * <code>ImageOutputStream</code> after the write operation has completed; it
	 * is the responsibility of the caller to close the stream, if desired.
	 * 
	 * @param im
