...
import swingjs.image.JSImageWriter;
...
public final class ImageIO {
	...
	// getRegistry().registerApplicationClasspathSpis();
	// }
	...
	private static int JPG = 1;
	private static int GIF = 2;
	private static int BMP = 3;
	...
	private static String[] readerSuffixes = { "jpg", "bmp", "jpeg", "wbmp",
			"png", "gif" };
	
	private static boolean useSPI;

	/**
	 * Returns an array of <code>String</code>s listing all of the informal format
	 * names understood by the current set of registered readers.
	 * 
	 * @return an array of <code>String</code>s.
	 */
	public static String[] getReaderFormatNames() {
		return (useSPI ? getReaderWriterInfo(ImageReaderSpi.class, SpiInfo.FORMAT_NAMES) : readerFormatNames);
	}
	...
	public static String[] getReaderMIMETypes() {
		return useSPI ? getReaderWriterInfo(ImageReaderSpi.class, SpiInfo.MIME_TYPES) : readerTypes;
	}
	...
	public static String[] getReaderFileSuffixes() {
		return useSPI ? getReaderWriterInfo(ImageReaderSpi.class, SpiInfo.FILE_SUFFIXES) : readerSuffixes;
	}


//	private static ImageInputStream createImageInputStream(InputStream input) {
//		return null;
//
//	}

	public static BufferedImage readJS(URL input) throws IOException {
	 ...
	 * completed, unless <code>null</code> is returned, in which case this method
	 * <em>does not</em> close the stream.
	 * 
	 * @param stream
	 ...
	 */
	public static BufferedImage readJS(ImageInputStream stream) throws IOException {
		JSUtil.notImplemented("ImageIO.read(ImageInputStream");
		return null;
	}

	/**
	 * Writes an image using the an arbitrary <code>ImageWriter</code> that supports
	 * the given format to an <code>ImageOutputStream</code>. The image is written
	 * to the <code>ImageOutputStream</code> starting at the current stream pointer,
	 * overwriting existing stream data from that point forward, if present.
	 * 
	 * <p>
	 * This method <em>does not</em> close the provided
	 * <code>ImageOutputStream</code> after the write operation has completed; it is
	 ...
	 */
	public static boolean writeJS(RenderedImage im, String formatName, ImageOutputStream output) throws IOException {
		ImageWriter writer = getWriter(formatName);
		return (writer != null && ((JSImageWriter) writer).write(im, null, (OutputStream) (Object) output));
	}
	 ...
	 *              if an error occurs during writing.
	 */
	public static boolean writeJS(RenderedImage im, String formatName,
			OutputStream output) throws IOException {
		ImageWriter writer = getWriter(formatName);
		return (writer != null && ((JSImageWriter)writer).write(im, null, output));
	}
	 ...
	 */
	 private static ImageWriter getWriter(String formatName) {
		 try {
			 return (useSPI ? getImageWritersByFormatName(formatName) : getImageWritersByFormatNameJS(formatName)).next();
		 } catch (Exception e) {
			 return null;
		 }
	 }

	public static Iterator<ImageWriter> getImageWritersByFormatNameJS(String formatName) {
		if (formatName == null) {
    ...
    private static IIORegistry theRegistry;
    
    static IIORegistry getRegistry() {

        return (theRegistry == null ? theRegistry = IIORegistry.getDefaultInstance() : theRegistry);
    }
        ...
        static class CacheInfo {
            boolean useCache = useSPI;// reconsider?? true;
            File cacheDirectory = null;
            Boolean hasPermission = Boolean.TRUE;// null;
        ...
        public static ImageInputStream createImageInputStream(Object input)
            ...
            try {
                iter = getRegistry().getServiceProviders(ImageInputStreamSpi.class,
                                                       true);
        ...
        public static ImageOutputStream createImageOutputStream(Object output)
            ...
            // Ensure category is present
            try {
                iter = getRegistry().getServiceProviders(ImageOutputStreamSpi.class,
                                                       true);
            } catch (IllegalArgumentException e) {
                return null;
            }
        ...
        static class ImageReaderIterator implements Iterator<ImageReader> {
            ...
            public ImageReader next() {
                ...
                try {
                    ...
                    // an ImageReaderSpi
                    getRegistry().deregisterServiceProvider(spi, ImageReaderSpi.class);
                }
                return null;
            }
        ...
        public static Iterator<ImageReader> getImageReaders(Object input) {
            ...
            // Ensure category is present
            try {
                iter = getRegistry().getServiceProviders(ImageReaderSpi.class,
	...
	static {
        ...
        static class ImageWriterIterator implements Iterator<ImageWriter> {
            ...
            public ImageWriter next() {
                ...
                try {
                    ...
                    getRegistry().deregisterServiceProvider(spi, ImageWriterSpi.class);
                }
                return null;
            }
            ...
            getImageWritersByFormatName(String formatName)
        {
            ...
            // Ensure category is present
            try {
                iter = getRegistry().getServiceProviders(ImageWriterSpi.class,
        ...
        public static ImageWriter getImageWriter(ImageReader reader) {
            ...
            ImageWriterSpi writerSpi = (ImageWriterSpi)
                getRegistry().getServiceProviderByClass(writerSpiClass);
            if (writerSpi == null) {
                return null;
            }
            ...
            try {
                ...
                // Deregister the spi in this case, but only as a writerSpi
                getRegistry().deregisterServiceProvider(writerSpi,
        ...
        public static ImageReader getImageReader(ImageWriter writer) {
            ...
            ImageReaderSpi readerSpi = (ImageReaderSpi)
                getRegistry().getServiceProviderByClass(readerSpiClass);
            if (readerSpi == null) {
                return null;
            }
            ...
            try {
                ...
                getRegistry().deregisterServiceProvider(readerSpi,
            ...
            getImageTranscoders(ImageReader reader, ImageWriter writer)
        {
            ...
            // Ensure category is present
            try {
                iter = getRegistry().getServiceProviders(ImageTranscoderSpi.class,
                                                filter, true);
            } catch (IllegalArgumentException e) {
                return Collections.emptyIterator();
            }
    	...
    	public static BufferedImage readJS(InputStream input) throws IOException {
        ...
        public static boolean write(RenderedImage im,
            ...
            return useSPI ? doWrite(im, getWriter(im, formatName), output) : writeJS(im, formatName, output);
        }
        ...
        public static boolean write(RenderedImage im,
            ...
            if (!useSPI)
            	return writeJS(im, formatName, output);
