...
public class Test_Ints extends Test_ {
	...
	private static int builtInExtFunction(int i, int j, int k) {
	...
	public static void main(String[] args) {
		...
		stack[stack_pointer - 1] = builtInExtFunction(5, stack[stack_pointer - 1], stack[stack_pointer]);
		stack_pointer--;
		
		System.out.println(stack_pointer + " " + Arrays.toString(stack));
		
		stack = new int[] { 0, 1, 2, 3, 4, 5 };
		stack_pointer = 3;

		stack[--stack_pointer] = builtInExtFunction(5, stack[stack_pointer], stack[stack_pointer + 1]);
		...
		System.out.println((0xFFFFFFFFL & ((-1 << 24) | ((0xFF & -1) << 16) | ((0xFF & -1) << 8) | (0xFF & -1))));

		long[] uint = /** @j2sNative new Uint32Array(1) || */
				new long[0];

		System.out.println(/** @j2sNative 1?(uint[0] = ix, uint[0]) : */
				ix & 0xFFFFFFFFL);
