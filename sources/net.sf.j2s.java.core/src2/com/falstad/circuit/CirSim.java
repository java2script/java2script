...
public class CirSim extends Frame implements ComponentListener, ActionListener,
	...
	public void init() {
		...
		inputMenu.add(getClassCheckItem("Add Voltage Source (1-terminal)",
				"RailElm"));
		inputMenu
				.add(getClassCheckItem("Add A/C Source (1-terminal)", "ACRailElm"));
		...
		activeMenu.add(getClassCheckItem("Add Schmitt Trigger (Inverting)",
				"InvertingSchmittElm"));
		...
		chipMenu.add(getClassCheckItem("Add 7 Segment Decoder",
				"SevenSegDecoderElm"));
...
//	void DebugRegister(Class c, CircuitElm elm) {
...
//			shortcuts[s] = sclass;
//		}
//		System.out.println("{\"" + sclass.getName().replace(BASE_PACKAGE,"") 
//				+ "\",\t" + (sclass.getName().equals(dclass.getName()) ? null : "\"" + dclass.getName().replace(BASE_PACKAGE,"") + "\"") + ",\"" + (int) t + "\",\t\"" + s + "\"},");
//	}
...
//	}
	...
	public void updateCircuit(Graphics realg) {
		...
		if (nodeList != null)
			for (i = 0; i != nodeList.size(); i++) {
				...
				if (!cn.internal && cn.links.size() == 1) {
					...
					for (j = 0; j != elmList.size(); j++) { // TODO: (hausen) see if this
																									// change does not break stuff
		...
		if (stopMessage != null) {
			...
			if (mouseElm != null) {
				 ...
				 * " " + mouseElm.nodes[i]; if (mouseElm.getVoltageSourceCount() > 0)
			...
			if (badnodes > 0)
				info[i++] = badnodes
						+ ((badnodes == 1) ? " bad connection" : " bad connections");
		...
		if (timer == null) {
			timer = new Timer((int) (delay < 0 ? 0 : delay), this);
	...
	String getHint() {
		...
		if (hintType == HINT_LC) {
			...
			return "res.f = "
					+ CircuitElm.getUnitText(
							1 / (2 * pi * Math.sqrt(ie.inductance * ce.capacitance)), "Hz");
		}
		if (hintType == HINT_RC) {
			...
			CapacitorElm ce = (CapacitorElm) c2;
			return "RC = "
					+ CircuitElm.getUnitText(re.resistance * ce.capacitance, "s");
		}
		...
		if (hintType == HINT_3DB_L) {
			...
			return "f.3db = "
					+ CircuitElm.getUnitText(re.resistance / (2 * pi * ie.inductance),
							"Hz");
		}
	...
	void analyzeCircuit() {
			...
			if (j == matrixSize) {
				...
				if (qm == -1) {
					...
					for (k = 0; elt.type == RowInfo.ROW_EQUAL && k < 100; k++) {
						// follow the chain
						/*
						 * System.out.println("following equal chain from " + i + " " + qp +
						 * " to " + elt.nodeEq);
						 */
						qp = elt.nodeEq;
		...
		/*
		 * System.out.println("matrixSize = " + matrixSize + " " +
		 * circuitNonLinear); for (j = 0; j != circuitMatrixSize; j++) { for (i = 0;
		 * i != circuitMatrixSize; i++) System.out.print(circuitMatrix[j][i] + " ");
...
//		}
	...
	private void repaintCV(int pause) {
		cv.repaint(pause);
	}
	...
	public void keyTyped(KeyEvent e) {
		...
		if (e.getKeyChar() > ' ' && e.getKeyChar() < 127) {
			Class<?> c = getClassFromShortCut(e.getKeyChar());
			if (c == null)
				return;
			CircuitElm elm = null;
	...
	final static String[][] elmData = {
			//  class   dumpClass dumpid shortcut
			{"ACRailElm",	"RailElm","82",	null},
			{"ACVoltageElm",	"VoltageElm","118",	null},
			{"ADCElm",	null,"167",	null},
			{"AMElm",	null,"200",	null},
			{"AnalogSwitch2Elm",	null,"160",	null},
			{"AnalogSwitchElm",	null,"159",	null},
			{"AndGateElm",	null,"150",	"50"},
			{"AntennaElm",	null,"65",	null},
			{"BoxElm",	null,"98",	null},
			{"CapacitorElm",	null,"99",	"99"},
			{"CC2Elm",	null,"179",	null},
			{"CC2NegElm",	"CC2Elm","179",	null},
			{"ClockElm",	"RailElm","82",	null},
			{"CounterElm",	null,"164",	null},
			{"CurrentElm",	null,"105",	null},
			{"DACElm",	null,"166",	null},
			{"DCVoltageElm",	"VoltageElm","118",	"118"},
			{"DecadeElm",	null,"163",	null},
			{"DeMultiplexerElm",	null,"185",	null},
			{"DFlipFlopElm",	null,"155",	null},
			{"DiodeElm",	null,"100",	"100"},
			{"FMElm",	null,"201",	null},
			{"FullAdderElm",	null,"196",	null},
			{"GroundElm",	null,"103",	"103"},
			{"HalfAdderElm",	null,"195",	null},
			{"InductorElm",	null,"108",	null},
			{"InverterElm",	null,"73",	"49"},
			{"InvertingSchmittElm",	null,"183",	null},
			{"JKFlipFlopElm",	null,"156",	null},
			{"LampElm",	null,"181",	null},
			{"LatchElm",	null,"168",	null},
			{"LEDElm",	null,"162",	"108"},
			{"LEDMatrixElm",	null,"207",	null},
			{"LogicInputElm",	null,"76",	"105"},
			{"LogicOutputElm",	null,"77",	"111"},
			{"MemristorElm",	null,"109",	null},
			{"MonostableElm",	null,"194",	null},
			{"MultiplexerElm",	null,"184",	null},
			{"NandGateElm",	null,"151",	"64"},
			{"NJfetElm",	"JfetElm","106",	null},
			{"NMosfetElm",	"MosfetElm","102",	null},
			{"NorGateElm",	null,"153",	"35"},
			{"NTransistorElm",	"TransistorElm","116",	null},
			{"OpAmpElm",	null,"97",	null},
			{"OpAmpSwapElm",	"OpAmpElm","97",	null},
			{"OrGateElm",	null,"152",	"51"},
			{"OutputElm",	null,"79",	null},
			{"PhaseCompElm",	null,"161",	null},
			{"PisoShiftElm",	null,"186",	null},
			{"PJfetElm",	"JfetElm","106",	null},
			{"PMosfetElm",	"MosfetElm","102",	null},
			{"PotElm",	null,"174",	null},
			{"ProbeElm",	null,"112",	null},
			{"PTransistorElm",	"TransistorElm","116",	null},
			{"PushSwitchElm",	"SwitchElm","115",	null},
			{"RailElm",	null,"82",	"86"},
			{"RelayElm",	null,"178",	"82"},
			{"ResistorElm",	null,"114",	"114"},
			{"SchmittElm",	null,"182",	null},
			{"SCRElm",	null,"177",	null},
			{"SeqGenElm",	null,"188",	null},
			{"SevenSegDecoderElm",	null,"197",	null},
			{"SevenSegElm",	null,"157",	null},
			{"SipoShiftElm",	null,"189",	null},
			{"SparkGapElm",	null,"187",	null},
			{"SquareRailElm",	"RailElm","82",	null},
			{"SweepElm",	null,"170",	null},
			{"Switch2Elm",	null,"83",	"83"},
			{"SwitchElm",	null,"115",	"115"},
			{"TappedTransformerElm",	null,"169",	null},
			{"TextElm",	null,"120",	"116"},
			{"TFlipFlopElm",	null,"193",	null},
			{"TimerElm",	null,"165",	null},
			{"TransformerElm",	null,"84",	null},
			{"TransLineElm",	null,"171",	null},
			{"TriodeElm",	null,"173",	null},
			{"TriStateElm",	null,"180",	null},
			{"TunnelDiodeElm",	null,"175",	null},
			{"VarRailElm",	null,"172",	null},
			{"VCOElm",	null,"158",	null},
			{"WireElm",	null,"119",	"119"},
			{"XorGateElm",	null,"154",	"52"},
			{"ZenerElm",	null,"122",	null},
	};
	
	private final static String[] elmKey = { "", "", "d", "s" };
	
	final static Map<String, String[]> elmMap = new Hashtable<String, String[]>();
	static {
		for (int i = elmData.length; --i >= 0;) {
			String[] data = elmData[i];
			if (data[1] == null)
				data[1] = data[0];
			for (int j = 0; j < 4; j++)
				if (data[j] != null)
					elmMap.put(elmKey[j] + data[j], data);
		}
	};
	
	Class<?> getDumpClass(int t) {
		switch (t) {
		case 'o':
		case 'h':
		case '$':
		...
		try {
			String[] data = elmMap.get("d" + t);
			if (data != null)
				return Class.forName(BASE_PACKAGE + data[1]);
		} catch (Exception e) {
			//
		}
		return null;
	...
	private Class<?> getClassFromShortCut(int keyChar) {
		try {
			String[] data = elmMap.get("s" + keyChar);
			if (data != null)
				return Class.forName(BASE_PACKAGE + data[0]);
		} catch (Exception e) {
			//
		}
		return null;
