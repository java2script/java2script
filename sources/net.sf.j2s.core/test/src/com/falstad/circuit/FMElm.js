...
var C$=Clazz.newClass(P$, "FMElm", null, 'com.falstad.circuit.CircuitElm');
...
return C$.superclazz.prototype.dump.apply(this, []) + " " + new Double(this.carrierfreq).toString() + " " + new Double(this.signalfreq).toString() + " " + new Double(this.maxVoltage).toString() + " " + new Double(this.deviation).toString() ;
});
...
var deltaT = (I$[0]||(I$[0]=Clazz.load('com.falstad.circuit.CircuitElm'))).sim.t - this.lasttime;
this.lasttime = (I$[0]||(I$[0]=Clazz.load('com.falstad.circuit.CircuitElm'))).sim.t;
var signalamplitude = Math.sin((2 * 3.141592653589793 * ((I$[0]||(I$[0]=Clazz.load('com.falstad.circuit.CircuitElm'))).sim.t - this.freqTimeZero) ) * this.signalfreq);
...
arr[5] = "dev =" + P$.CircuitElm.getUnitText$D$S(this.deviation, "Hz");
arr[6] = "Vmax = " + P$.CircuitElm.getVoltageText$D(this.maxVoltage);
});
...
if (n == 3) return Clazz.new_((I$[3]||(I$[3]=Clazz.load('com.falstad.circuit.EditInfo'))).c$$S$D$D$D,["Deviation (Hz)", this.deviation, 4, 500]);
return null;
});
