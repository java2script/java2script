...
Clazz.load (["sun.awt.geom.Curve"], "sun.awt.geom.Order3", ["java.awt.geom.QuadCurve2D", "sun.awt.geom.Order2"], function () {
...
}, sun.awt.geom, "Order3", sun.awt.geom.Curve);
c$.insert = Clazz.defineMethod (c$, "insert", 
function (curves, tmp, x0, y0, cx0, cy0, cx1, cy1, x1, y1, direction) {
var numparams = sun.awt.geom.Order3.getHorizontalParams (y0, cy0, cy1, y1, tmp);
if (numparams == 0) {
sun.awt.geom.Order3.addInstance (curves, x0, y0, cx0, cy0, cx1, cy1, x1, y1, direction);
return;
}tmp[3] = x0;
tmp[4] = y0;
tmp[5] = cx0;
...
tmp[1] = t;
t = tmp[0];
}sun.awt.geom.Order3.split (tmp, 3, t);
if (numparams > 1) {
t = (tmp[1] - t) / (1 - t);
sun.awt.geom.Order3.split (tmp, 9, t);
}var index = 3;
if (direction == -1) {
index += numparams * 6;
}while (numparams >= 0) {
sun.awt.geom.Order3.addInstance (curves, tmp[index + 0], tmp[index + 1], tmp[index + 2], tmp[index + 3], tmp[index + 4], tmp[index + 5], tmp[index + 6], tmp[index + 7], direction);
numparams--;
if (direction == 1) {
index += 6;
} else {
...
if (y0 > y1) {
curves.add ( new sun.awt.geom.Order3 (x1, y1, cx1, cy1, cx0, cy0, x0, y0, -direction));
} else if (y1 > y0) {
curves.add ( new sun.awt.geom.Order3 (x0, y0, cx0, cy0, cx1, cy1, x1, y1, direction));
...
Clazz.superConstructor (this, sun.awt.geom.Order3, [direction]);
if (cy0 < y0) cy0 = y0;
if (cy1 > y1) cy1 = y1;
this.x0 = x0;
this.y0 = y0;
...
if (false && t >= 0 && t <= 1) {
y = this.YforT (t);
var tdiff = sun.awt.geom.Curve.diffbits (t, origt);
var ydiff = sun.awt.geom.Curve.diffbits (y, origy);
var yerr = sun.awt.geom.Curve.diffbits (y, target);
...
var tlow = sun.awt.geom.Curve.prev (t);
var ylow = this.YforT (tlow);
var thi = sun.awt.geom.Curve.next (t);
...
sun.awt.geom.Order3.split (eqn, 0, t1);
}var i;
if (t0 <= 0) {
i = 0;
} else {
sun.awt.geom.Order3.split (eqn, 0, t0 / t1);
i = 6;
}return  new sun.awt.geom.Order3 (eqn[i + 0], ystart, eqn[i + 2], eqn[i + 3], eqn[i + 4], eqn[i + 5], eqn[i + 6], yend, dir);
}, "~N,~N,~N");
Clazz.overrideMethod (c$, "getReversedCurve", 
function () {
return  new sun.awt.geom.Order3 (this.x0, this.y0, this.cx0, this.cy0, this.cx1, this.cy1, this.x1, this.y1, -this.direction);
});
...
function () {
return (("(" + sun.awt.geom.Curve.round (this.getCX0 ()) + ", " + sun.awt.geom.Curve.round (this.getCY0 ()) + "), ") + ("(" + sun.awt.geom.Curve.round (this.getCX1 ()) + ", " + sun.awt.geom.Curve.round (this.getCY1 ()) + "), "));
});
});
