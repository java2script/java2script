...
this.offset = 0;
this.count = 0;
this.value =  Clazz.newArray$('CA', 1, [0]);
}, 1);
...
var value =  Clazz.newArray$('CA', 1, [count]);
if (hibyte == 0) {
for (var i = count; i-- > 0; ) {
...
if (otherLen == 0) {
return this;
}var buf =  Clazz.newArray$('CA', 1, [this.count + otherLen]);
...
if (i < len) {
var buf =  Clazz.newArray$('CA', 1, [len]);
for (var j = 0; j < i; j++) {
buf[j] = val[off + j];
}
while (i < len) {
...
if (mapLen > srcCount) {
var result2 =  Clazz.newArray$('CA', 1, [result.length + mapLen - srcCount]);
...
var data =  Clazz.newArray$('CA', -1, [-1, [c]]);
...
"serialPersistentFields",  Clazz.newArray$('java_io_ObjectStreamFieldA', 1, [0]),
