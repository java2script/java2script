...
class J2SLegacyVisitor extends J2SKeywordVisitor {
	...
	//
	// final static int fiI = Integer.valueOf(3); // incorrectly not unboxed in
	// Legacy
	...
	// BH 2023.11.10 Clazz.pu$h)/p0p replaced by anonymous functions
	 ...
	 *         2006-12-3
	 */
	protected static class J2STypeHelper {
	...
	protected static class ReferenceASTVisitor extends ASTVisitor {
		...
		public boolean isReferenced(Expression initializer) {
			isReferenced = false;
	...
	private J2STypeHelper typeHelper = new J2STypeHelper();
	...
	private final static String emptyFunction = "Clazz.decorateAsClass (function () {\r\n"
			+ "Clazz.instantialize (this, arguments);\r\n}, ";

	public boolean visit(AnonymousClassDeclaration node) {
		ITypeBinding binding = node.resolveBinding();
		J2STypeHelper typeHelper = this.typeHelper;
	...
	public boolean visit(ClassInstanceCreation node) {
		...
		if (anonDeclare == null) {
			...
			buffer.append("(");
			VariableHelper variableVisitor = variableHelper;
	...
	public boolean visit(EnumDeclaration node) {
		ITypeBinding binding = node.resolveBinding();
		J2STypeHelper typeVisitor = typeHelper;
...
//	}
//
	protected static boolean isMethodNativeIgnored(MethodDeclaration node) {
...
//			}
	...
	public boolean visit(MethodDeclaration node) {
		...
		if ((node.getModifiers() & Modifier.PRIVATE) != 0 && mBinding != null
				&& !MethodReferenceASTVisitor.checkReference(node.getRoot(), mBinding.getKey())) {
		...
		if (node.isConstructor()) {
			...
			} else if (MethodReferenceASTVisitor.canAutoOverride(node)) {
				buffer.append("Clazz.overrideMethod (");
			} else {
		...
		if (node.isConstructor()) {
			...
			if (checkKeywordViolation(identifier)) {
				buffer.append('$');
			}
		...
		if (node.isConstructor()) {
			...
			if (binding != null) {
				...
				if (superclass != null) {
					String qualifiedName = BindingHelper.discardGenericType(superclass.getQualifiedName());
...
//		} else {
//			/*
//			 * If all method invocations before super call is filtered, then super call is
//			 * still considered as the only one.
//			 * 
//			 * For example, the filtered methods may be: checkWidget(); checkDevice();
//			 */
...
//											break;
//										}
//									} else if (BindingHelper.isMethodInvoking(methodBinding, filterMethods[j],
//											filterMethods[j + 1])) {
//										isFiltered = true;
//										break;
//									}
...
//		return isOnlyOneCall;
	...
	public boolean visit(MethodInvocation node) {
		...
		if (MethodReferenceASTVisitor.isMethodRegistered(methodName)
			...
			if (binding != null && binding instanceof IMethodBinding) {
				...
				String propertyName = MethodReferenceASTVisitor.translate(className, methodName);
			...
			if (variableHelper.isFinalSensible && (varBinding.getModifiers() & Modifier.FINAL) != 0
				...
				if (methodDeclareStack.size() == 0 || !key.equals(methodDeclareStack.peek())) {
					...
					if (currentBlockForVisit != -1) {
						...
						List<FinalVariable> visitedVars = variableHelper.visitedVars;
						int size = finalVars.size();
						for (int i = 0; i < size; i++) {
							FinalVariable vv = finalVars.get(size - i - 1);
			...
			if (element instanceof TypeDeclaration) {
				...
				if ((field.getModifiers() & Modifier.STATIC) != 0) {
					...
					for (int j = 0; j < fragments.size(); j++) {
						...
						if (initializer != null) {
							if (refVisitor.isReferenced(initializer)) {
