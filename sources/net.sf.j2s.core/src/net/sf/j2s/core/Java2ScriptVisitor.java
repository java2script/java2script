...
//	}
	...
	public boolean visit(ClassInstanceCreation node) {
		...
		if (binding.isTopLevel() || isStatic(binding)) {
			...
			addInnerTypeInstance(node, getJavaClassNameQualified(binding), node.getExpression(), null,
	 ...
	 */
	private void openNew(String javaClassName, String anonJavaName, IMethodBinding constructorMethodBinding,
			int lambdaArity) {
		buffer.append("Clazz.new_(");
...
//		localName = null;
		...
		addInnerTypeInstance(node, javaInnerClassName, null, finals, constructorDeclaration, anonymousSuperclassName,
				anonName);
	 ...
	 * Then create a package_finalVars entry for this name if it is final or effectively final.
	 *  
	 * @param name
	...
	private void removeVariableFinals(IMethodBinding mBinding, List<ASTNode> parameters) {
		...
		for (int i = parameters.size() - 1; i >= 0; i--) {
			...
			if (binding != null) {
				...
				LocalVariable f = new LocalVariable(package_blockLevel + 1, identifier, methodSig, getFinalVarName(identifier));
...
//		}
	...
	private void addClassOrInterface(ASTNode node, ITypeBinding binding, List<?> bodyDeclarations, char type) {
		...
		List<FieldDeclaration> fields = (isInterface || isLambda || isEnum || class_annotations == null ? null : new ArrayList<>());
		...
		if (isLambda) {
			...
			for (Iterator<?> iter = bodyDeclarations.iterator(); iter.hasNext();) {
				...
				if (element instanceof MethodDeclaration) {
					...
					processMethodDeclaration(method, mnode.parameters(), mnode.getBody(), mnode.isConstructor(), NOT_LAMBDA);
					if (defpt >= 0) {
		...
		if (class_jaxbAccessorType != JAXB_TYPE_UNKNOWN) {
			ClassAnnotation.addClassAnnotations(class_jaxbAccessorType, class_annotations, enums, fields, methods, trailingBuffer);
...
//				anonDeclare.accept(this);
	 ...
	 *                               = new MouseAdapter() {....}
	 */
	private void addInnerTypeInstance(ASTNode node, String javaInnerClassName, Expression outerClassExpr, String finals,
			IMethodBinding constructorMethodDeclaration, String superAnonName, String anonName) {
		openNew((superAnonName == null ? javaInnerClassName : superAnonName), anonName, constructorMethodDeclaration,
				-1);
...
//          }			
	...
	private String getFinalNameForNode(SimpleName node, char lastBufferChar, IVariableBinding varBinding) {
		...
		if (isStatic(varBinding)) {
			...
			if (lastBufferChar != '.' && lastBufferChar != '"' && lastBufferChar != '\'') {
				...
				qualifier = getFinalJ2SClassNameQualifier(null, getJavaClassNameQualified(declaringClass),
						FINAL_ESCAPECACHE) + ".";
			}
	...
	private String getFinalInnerClassList(String javaClassName) {
		...
		String[] parts = removeBracketsAndFixNullPackageName(javaClassName).split("\\.");
		String s = parts[0];
		int i = 1;
	 ...
	 * shortening the name with C$. and/or with P$. Brackets are removed.
	 * "java.lang." is removed where appropriate.
	 * 
	 * @param javaClassName
...
//		}
	...
	private void addDummyClassForPackageOnlyFile() {
		...
		ClassAnnotation.addClassAnnotations(class_jaxbAccessorType, class_annotations, null, null, null, trailingBuffer);
	...
	private static class LocalVariable {
		...
		/**
		 * Check for final-variable access, adding this to visitedVariables if 
		 * need be.
		 * 
		 * @param name
		 * @param block
	...
	static class ClassAnnotation {
		...
		@SuppressWarnings("unchecked")
		public static void addClassAnnotations(int accessType, List<ClassAnnotation> class_annotations, List<EnumConstantDeclaration> enums, List<FieldDeclaration> fields,
			...
			if (pt > 0) {
				addTrailingFragments(fragments, trailingBuffer, ptBuf);
				addImplicitJAXBFieldsAndMethods(accessType, trailingBuffer, enums, fields, methods, propOrder);
				trailingBuffer.append("]]];\n");
			}
		}
		...
		private static void addImplicitJAXBFieldsAndMethods(int accessType, TrailingBuffer trailingBuffer,
				List<EnumConstantDeclaration> enums, List<FieldDeclaration> fields, List<IMethodBinding> methods, String propOrder) {
			if (accessType == JAXB_TYPE_ENUM) {
				for (int j = 0; j < enums.size(); j++) {
					EnumConstantDeclaration con = enums.get(j);
					IVariableBinding var = con.resolveVariable();
					String varName = var.getName();
					ITypeBinding type = var.getType();
					addAnnotation(type, varName, "@XmlEnumValue", trailingBuffer);
				}
				return;
			}
			...
			if (accessType != JAXB_TYPE_FIELD) {
				for (int i = 0; i < methods.size(); i++) {
					...
					if (var2 == null || publicOnly && !Modifier.isPublic(var.getModifiers()) && !Modifier.isPublic(var2.getModifiers()))
						continue;
					String varName = var.getName();
					if (varName.startsWith("set"))
						varName = (var = var2).getName();
					ITypeBinding type = var.getReturnType();
					addAnnotation(type, "M:" + varName, "@XmlElement", trailingBuffer);
				}
			}
		}

		private static void addAnnotation(ITypeBinding type, String varName, String str, TrailingBuffer trailingBuffer) {
