...
void createHandle() {
	...
	if ((style & SWT.HORIZONTAL) != 0) {
		handle.className = "sash-vertical-default";
	} else {
		handle.className = "sash-horizontal-default";
	}
...
//			}
			...
			if (event.doit) {
				if ((style & SWT.SMOOTH) != 0) {
					...
					if (isHorizontal) {
						handle.style.left = lastX + "px";
					} else {
						handle.style.top = lastY + "px";
					}
...
String windowClass () {
	return "DIV"; //display.windowClass;
}
