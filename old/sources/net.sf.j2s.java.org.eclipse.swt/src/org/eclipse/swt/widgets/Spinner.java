...
public class Spinner extends Composite {
	...
	int minimum, maximum;
	int increment;
	Element textHandle, updownHandle;
...
protected void createHandle () {
	...
	*/
	...
	updownHandle = document.createElement("DIV");
	handle.appendChild(updownHandle);
	updownHandle.className = "spinner-up-down-default";
	upBtnHandle = document.createElement("BUTTON");
	updownHandle.appendChild(upBtnHandle);
	upBtnHandle.className = "spinner-up-button-default";
	...
	downBtnHandle.className = "spinner-down-button-default";
	...
	textHandle.className = "spinner-text-default";

	textInputHandle = document.createElement("INPUT");
	textInputHandle.className = "spinner-text-field-default";
	 if ((style & SWT.READ_ONLY) != 0) {
		 textInputHandle.readOnly = true;
	 }
	
	if ((style & SWT.BORDER) == 0) {
		textInputHandle.className += " spinner-text-field-no-border";
	}
...
public Point computeSize (int wHint, int hHint, boolean changed) {
	...
	if (wHint == SWT.DEFAULT || hHint == SWT.DEFAULT) {
		...
		*/
		height = 13;//OS.getContainerHeight(textHandle);
		String string = "" + maximum;
		if (digits > 0) {
			int leading = maximum / (int) Math.pow(10, digits);
			String buffer = "" + leading;
			buffer += getDecimalSeparator ();
			int count = digits - buffer.length () + 1;
			while (count >= 0) {
				buffer += "0";
				count--;
			}
		...
		width = OS.getStringPlainWidth(string);
	}
...
public Rectangle computeTrim (int x, int y, int width, int height) {
	...
	*/
	int margins = 0x10000;//OS.SendMessage (hwndText, OS.EM_GETMARGINS, 0, 0);
	x -= margins & 0xFFFF;
	width += (margins & 0xFFFF) + ((margins >> 16) & 0xFFFF);
	if ((style & SWT.BORDER) != 0) {
		x -= 1;
...
//			width += 2;
	...
	width += 16;//OS.GetSystemMetrics (OS.SM_CXVSCROLL);
	return new Rectangle (x, y, width, height);
}
...
public int getSelection () {
	...
	*/
	String value = textInputHandle.value;
	int idx = value.indexOf(".");
	if (idx != -1) {
		if (value.substring(idx + 1).length() > digits) {
			value = value.substring(0, idx + 1 + digits);
		}
	}
...
protected void releaseHandle() {
	...
	if (updownHandle != null){
		OS.destroyHandle(updownHandle);
		updownHandle = null;	
	}
...
void setSelection (int value, boolean notify) {
	...
	if (hooks (SWT.Verify) || filters (SWT.Verify)) {
		int length = textInputHandle.value.length(); // OS.GetWindowTextLength (hwndText);
		string = verifyText (string, 0, length, null);
		if (string == null) return;
	}
	if (textInputHandle != null) {
		textInputHandle.value = "" + string;
	}
	/*
...
/* (non-Javadoc)
 * @see org.eclipse.swt.widgets.Composite#setSize(int, int)
 */
public void setSize(int width, int height) {
	...
	textInputHandle.style.width = width + "px";
	textInputHandle.style.height = height + "px";
}
...
protected boolean SetWindowPos(Object hWnd, Object hWndInsertAfter, int X, int Y, int cx, int cy, int uFlags) {
	...
	textInputHandle.style.width = (cx - 16 > 0 ? cx - 16 : 0) + "px";
	textInputHandle.style.height = (cy > 0 ? cy : 0) + "px";
	
	if (OS.isIE) {
		upBtnHandle.style.height = (cy >> 1) + "px";
		downBtnHandle.style.height = (cy >> 1) + "px";
	}
	int h = (cy >> 2 << 1) + 1;
	OS.updateArrowSize(upBtnHandle.childNodes[0], SWT.UP, 16, h);
	OS.updateArrowSize(downBtnHandle.childNodes[0], SWT.DOWN, 16, h);
