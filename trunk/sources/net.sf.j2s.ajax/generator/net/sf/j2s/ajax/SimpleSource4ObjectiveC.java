...
public class SimpleSource4ObjectiveC {
	...
	public static void main(String[] args) {
		...
		
		{
			...
			buffer.append("#import \"SimpleFactory.h\"\r\n");
			buffer.append("\r\n");
			...
			buffer.append("@interface ");
			buffer.append(simpleClazzName);
			buffer.append(" : SimpleFactory {\r\n\r\n\r\n}\r\n\r\n");
			buffer.append("- (id) createInstanceByClassName:(NSString *) className;\r\n");
			buffer.append("\r\n");
			buffer.append("@end\r\n");
			
			FileOutputStream fos = null;
			try {
				fos = new FileOutputStream(new File(targetFolder, simpleClazzName + ".h"));
		...
		
		{
				...
				try {
					...
					if (inst instanceof SimpleSerializable) {
						if (!(inst instanceof SimpleRPCRunnable)) {
							...
							buffer.append("    if ([className compare:@\"");
							buffer.append(j2sSimpleClazz);
							buffer.append("\"] == 0) {\r\n");
							buffer.append("        return [[[");
							buffer.append(simpleName);
							buffer.append(" alloc] init] autorelease];\r\n");
							buffer.append("    }\r\n");
						}
					}
				} catch (Throwable e) {
			...
			FileOutputStream fos = null;
			try {
				fos = new FileOutputStream(new File(targetFolder, simpleClazzName + ".m"));
				fos.write(buffer.toString().getBytes());
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
