...
public Point getMinimumSize () {
	...
	if ((style & SWT.NO_TRIM) == 0 && (style & trim) != 0) {
		width = Math.max (width, 80);//OS.GetSystemMetrics (OS.SM_CXMINTRACK));
	}
	int height = Math.max (0, minHeight);
	if ((style & SWT.NO_TRIM) == 0 && (style & trim) != 0) {
		if ((style & SWT.RESIZE) != 0) {
			height = Math.max (height, 24);//OS.GetSystemMetrics (OS.SM_CYMINTRACK));
		} else {
			/*
			RECT rect = new RECT ();
			...
			*/
			height = Math.max (height, 24);//rect.bottom - rect.top);
		}
	}
	if ((style & SWT.NO_TRIM) != 0) {
		return new Point (minWidth, Math.max(minHeight - 24, 0));
	}
