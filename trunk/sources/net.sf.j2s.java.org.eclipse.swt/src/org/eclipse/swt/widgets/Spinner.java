	...
	public Rectangle computeTrim (int x, int y, int width, int height) {
		...
		/*
		 /* Get the trim of the text control *-/
		   ...
		   */
		int margins = 4;//OS.SendMessage (hwndText, OS.EM_GETMARGINS, 0, 0);
		x -= margins & 0xFFFF;
		width += (margins & 0xFFFF) + ((margins >> 16) & 0xFFFF);
		if ((style & SWT.BORDER) != 0) {
			x -= 1;
		...
		width += 2;//OS.GetSystemMetrics (OS.SM_CXVSCROLL);
		return new Rectangle (x, y, width, height);
	}
...
//		return min [0];
	...
	public int getSelection () {
		 ...
		 */
		return Integer.parseInt(textInputHandle.value);
	}
