...
public boolean equals (Object object) {
	...
	if (device != color.device)
		return false;
	if (cssHandle != null && color.cssHandle != null) {
		return cssHandle == color.cssHandle;
	} else if (cssHandle != null) {
		return (rgbHandleFromCSS(cssHandle) & 0xFFFFFF) == (color.handle & 0xFFFFFF);
	} else if (color.cssHandle != null) {
		return (rgbHandleFromCSS(color.cssHandle) & 0xFFFFFF) == (handle & 0xFFFFFF);
	} else {
		return (handle & 0xFFFFFF) == (color.handle & 0xFFFFFF);
	}
}
...
public int getBlue () {
	...
	if (handle < 0) handle = rgbHandleFromCSS(cssHandle);
...
public String toString () {
	...
	if (cssHandle != null) return "Color {\"" + cssHandle + "\"}"; //$NON-NLS-1$//$NON-NLS-2$
...
public Color(Device device, String handle) {
	if (device == null) device = Device.getDevice();
	if (device == null) SWT.error(SWT.ERROR_NULL_ARGUMENT);
	cssHandle = handle;
	this.handle = -2;
...
private int rgbHandleFromCSS(String cssHandle) {
	...
	if (cssHandle != null) {
		...
		if (cssHandle.length() != 0 && cssHandle.startsWith("rgb")) {
			String str = cssHandle.substring(3).trim();
			if (str.startsWith("(")) {
				str = str.substring(1);
			}
			if (str.endsWith(")")) {
			...
			split[0] = str.substring(0, idx1);
			int idx2 = str.indexOf(',', idx1 + 1);
			split[1] = str.substring(idx1 + 1, idx2);
			split[2] = str.substring(idx2 + 1, str.length());
			try {
...
public String getCSSHandle() {
	if (cssHandle != null) return cssHandle;
	return "rgb(" + getRed() + ", " + getGreen() + ", " + getBlue() + ")"; //$NON-NLS-1$//$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
}

public boolean isPlatformSpecific() {
	if ((handle < 0 || handle == 0x0F000000) && cssHandle != null) {
		return rgbHandleFromCSS(cssHandle) == 0x0F000000;
	}
